	...
	if ($valid_user < 1)
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "Ora avete registrati";
				$LOGtimeMESSAGE = "Si è connessi a $NOW_TIME";
			...
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">Torna ad Amministrazione</font></A>";}
			if ($referrer=='welcome') 
