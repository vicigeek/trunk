...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- 版本: $version     版次: $build    MEETME: $conf_exten   server_ip: $server_ip-->\n";
	echo "<title>確認會議中分機";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'refresh')
	...
	if (strlen($conf_exten)<1)
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 是無效的\n";
		exit;
		}
	else
		{
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03007',$user,$server_ip,$session_name,$one_mysql_log);}
				$row=mysqli_fetch_row($rslt);
				$RingCalls=$row[0];
				if ($RingCalls > 0) {$RingCalls = "<font class=\"queue_text_red\">佇列中等待電話: $RingCalls</font>";}
				else {$RingCalls = "<font class=\"queue_text\">佇列中等待電話: $RingCalls</font>";}
...
if ($ACTION == 'register')
	...
	if ( (strlen($conf_exten)<1) || (strlen($exten)<1) )
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 是無效的 or Exten $exten 是無效的\n";
		exit;
		}
	else
		{
