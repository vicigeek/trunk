...
<title>QC ModifyLead: <?php echo "$lead_id - ".urldecode($lead_name).urldecode(" (Εκστρατεία: $campaign_id - $campaign_name)"); ?></title>
...
if ($end_call > 0) {
	...
	$rslt=mysql_to_mysqli($stmt, $link);
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        $row=mysqli_fetch_assoc($rslt);
        //QUALITY CONTROL CHANGE LOG BEGIN - CHANGE ΕΚΔΟΣΗ (VIEW ΕΚΔΟΣΗ IS BELOW, WHICH CREATES THE RECORD, THIS ONE MERELY MODIFIES THE EXISTING RECORD)
        $new_record=$row;
	...
	echo "<br>οι πληροφορίες τροποποιήθηκαν<BR><BR>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	echo "<tr><td colspan=2>Fronter: <A HREF=\"user_stats.php?user=$tsr\">$tsr</A> &nbsp; &nbsp; ID Λίστας: $list_id &nbsp; &nbsp; Called Count: $called_count</td></tr>\n";
	...
	while ($row_count > $o)
		...
		$rowx=mysqli_fetch_row($rslt);
             	echo "<tr><td align=right>$rowx[0] : </td><td align=left><hr>$rowx[1]<br><b>ID Λίστας:</b> $rowx[2]; <b>ID Εκστρατείας:</b> $rowx[3]<br>$rowx[4]</td></tr>\n";
		$o++;
		}
