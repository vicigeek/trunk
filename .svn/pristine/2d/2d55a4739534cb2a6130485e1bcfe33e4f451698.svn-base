...
function LoadLists(FromBox) {
	...
	for(i = 0; i < document.getElementById('group').options.length; i++) {
		if (document.getElementById('group').options[i].selected) {
			selectedKampagnen += document.getElementById('group').options[i].value+"|";
		} 
	}
	...
	for (j=0; j<campaign_id_ary.length; j++) {
		...
		var campaign_matches = selectedKampagnen.match(campaignID);
		if (campaign_matches) {
...
if (preg_match('/MSIE/i', $_SERVER['HTTP_USER_AGENT'])) {
	$JS_events="onBlur='LoadListen(this.form.group)' onKeyUp='LoadListen(this.form.group)'";
} else {
	$JS_events="onMouseUp='LoadListen(this.form.group)' onBlur='LoadListen(this.form.group)' onKeyUp='LoadListen(this.form.group)'";
}
...
echo "Listen: <font size=1>(optional, möglicherweise langsam)</font><BR>\n";
echo $list_options;
echo "</TD><TD VALIGN=TOP>";
...
if (strlen($group[0]) > 1)
	{
	echo " <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">ÄNDERN Sie</a> | \n";
...
if (strlen($group[0]) < 1)
	...
	$OUToutput .= "Gesamtanrufe gesetzt von dieser Kampagne:                       $TOTALcalls\n";
	$OUToutput .= "Durchschnittliche Anruf-Länge für alle benennt in den Sekunden: $average_call_seconds\n";
	if (preg_match("/YES/i",$include_rollover))
		{$OUToutput .= "Calls that went to rollover In-Gruppe:        $inTOTALcalls\n";}


	$OUToutput .= "\n";
	$OUToutput .= "---------- MENSCHLICHE ANTWORTS\n";
	...
	$OUToutput .= "Total Human Antworted calls for this Kampagne: $CIcalls\n";
	$OUToutput .= "Average Call Length for all HA in Sekunden:    $average_ci_seconds     GesamtTime: $CIsec\n";
	...
	$DROPANTWORTpercent = (MathZDC($DROPcallsRAW, $ANTWORTcalls) * 100);
	...
	$OUToutput .= "Percent of DROP Calls taken out of Answers:   $DROPcalls / $ANTWORTcalls  $DROPANTWORTpercent%\n";

	if (preg_match("/YES/i",$include_rollover))
		{
		$inDROPANTWORTpercent = (MathZDC($DROPcallsRAW, $CIcallsRAW) * 100);
		$inDROPANTWORTpercent = round($inDROPANTWORTpercent, 2);

		$OUToutput .= "Percent of DROP/Answer Calls with Rollover:   $DROPcalls / $CIcallsRAW  $inDROPANTWORTpercent%\n";
		}

	$OUToutput .= "Durchschnittliche Länge für TROPFEN Anrufe in den Sekunden:     $average_hold_seconds\n";
	...
	$AVG_ANTWORTagent_non_pause_sec = (MathZDC($TOTALanswers, $agent_non_pause_sec) * 60);
	...
	if ($skip_productivity_calc) {
		...
		$OUToutput .= "Productivity Rating:                          $AVG_ANTWORTagent_non_pause_sec\n";
	}
	...
	$OUToutput .= "Durchschnittliche Anruf-Länge für Na benennt in den Sekunden:   $average_na_seconds\n";


	##############################
	#########  CALL HANGUP GRUND STATS

	$TOTALcalls = 0;

	$ASCII_text .= "\n";
	$ASCII_text .= "---------- CALL HANGUP GRUND STATS\n";
	$ASCII_text .= "+----------------------+------------+\n";
	...
	$GRAPH.="<caption align='top'>CALL HANGUP GRUND STATS</caption>";
	$GRAPH.="<tr>\n";
	...
	while ($i < $reasons_to_print)
		...
		if (preg_match('/NONE/',$reason))	{$reason = 'NO ANTWORT           ';}
	...
	$CALLSHOUR_agent_graph=$graph_header."<th class='thgraph' scope='col'>MITTELCALLS/HR</th></tr>";
	###########################
...
#	$ASCII_text .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	if ($presets_enabled > 0)
		...
		$ASCII_text .= "\n";
		$ASCII_text .= "---------- MITTELPRESET DIALS\n";
		$ASCII_text .= "+------------------------------------------+------------+\n";
		$ASCII_text .= "| PRESET NAME                              | CALLS      |\n";
		$ASCII_text .= "+------------------------------------------+------------+\n";

		$GRAPH="</PRE><table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"white\" summary=\"DID Summary\" class=\"horizontalgraph\">\n";
		$GRAPH.="<caption align='top'>MITTELPRESET DIALS</caption>";
		$GRAPH.="<tr>\n";
		...
		$GRAPH.="  <tr>\n";
		$GRAPH.="	<th class=\"thgraph\" scope=\"col\">GESAMTANRUFE:</th>\n";
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- CUSTOM STATUS KATEGORIE STATS\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	$ASCII_text .= "| KATEGORIE             | CALLS      | DESCRIPTION                    |\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- MITTELSTATS\n";
	$ASCII_text .= "+--------------------------+------------+------------+--------+\n";
	$ASCII_text .= "| MITTEL                   | CALLS      | TIME H:M:S |AVERAGE |\n";
	$ASCII_text .= "+--------------------------+------------+------------+--------+\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>MITTELSTATS</caption><tr><th class='thgraph' scope='col'>STATUS</th>";
	$CALLS_graph=$graph_header."<th class='thgraph' scope='col'>CALLS </th></tr>";
	$TOTALTIME_graph=$graph_header."<th class='thgraph' scope='col'>TOTAL TIME</th></tr>";
	$AVGTIME_graph=$graph_header."<th class='thgraph' scope='col'>AVG TIME</th></tr>";
	###########################
