...
$report_name = 'Agente Tiempo Detalle';
$db_source = 'M';
...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		{
		$ASCII_text.="\nAgente Tiempo Detalle                     $NOW_TIME\n";
		$ASCII_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";

		$GRAPH.="Agente Tiempo Detalle                     $NOW_TIME\n";
		$GRAPH.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		}
	else
		{
		$file_output .= "Agente Tiempo Detalle                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>AGENTE TIME BREAKDOWN</caption><tr><th class='thgraph' scope='col'>STATUS</th>";
	...
	# Mostrars park info - need to run regardless so HTML view will populate
	$park_array=array();
...
echo "<INPUT TYPE=Submit NAME=ENVIAR VALUE=ENVIAR>$NWB#agent_time_detail$NWE\n";
