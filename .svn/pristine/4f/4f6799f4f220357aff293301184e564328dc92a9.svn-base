...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- ΕΚΔΟΣΗ: $version     ΔΗΜΙΟΥΡΓΙΑ: $build    ACTION: $ACTION   server_ip: $server_ip-->\n";
	echo "<title>Ο διαχτής στέλνει: ";
...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten δεν ισχύει or queryCID $queryCID δεν ισχύει, Originate εντολή που δεν έγινε εισαγωγή\n";
		}
	else
		{
		...
		echo "Originate εντολή που στέλνεται για Exten $exten Κανάλι $channel στο $server_ip\n";
		}
	}
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		...
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR Δεν επιτρέπεται να καλέσετε σε άλλες συνεδρίες χειριστή $exten\n";
			exit;
			}
		...
		echo "Originate εντολή που στέλνεται για Exten $exten Κανάλι $channel στο $server_ip |$account|$variable|\n";
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten δεν ισχύει or ext_context $ext_context or queryCID $queryCID δεν ισχύει, Hangup εντολή που δεν έγινε εισαγωγή\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
#			}
		...
		if ($channel_live==1)
			...
			echo "Hangup εντολή που στέλνεται για Κανάλι $channel στο $call_server_ip\n";
			}
		}
	}
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference στο $server_ip, Redirect εντολή που δεν έγινε εισαγωγή\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Δεν υπάρχει κενή διάσκεψη στο $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel δεν είναι ενεργό στο $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel δεν είναι ενεργό στο $server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel στο $call_server_ip, Hungup $extrachannel στο $server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channel στο $server_ip";}
			}
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $exten στο $server_ip";}
					}
				else
					{
					$S='*';
...
if ($ACTION=="Redirect")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		if ($channel_live==1)
			...
			echo "Redirect εντολή που στέλνεται για Κανάλι $channel στο $server_ip\n";
			}
		}
	}
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		...
		if ($channel_live==1)
			...
			echo "$ACTION εντολή που στέλνεται για Κανάλι $channel στο $server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n";
			}
		}
	}
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		...
		##### StopMonitorConf steps #####
			...
			# find and hang up all recordings going στο in this conference # and extension = '$exten' 
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "Διάσκεψη $exten, Stage $stage δεν ισχύει or queryCID $queryCID δεν ισχύει, Originate εντολή που δεν έγινε εισαγωγή\n";
		}
	else
		{
		...
		echo "Volume εντολή που στέλνεται για Διάσκεψη $exten, Stage $stage Κανάλι $channel στο $server_ip\n";
		}
	}
