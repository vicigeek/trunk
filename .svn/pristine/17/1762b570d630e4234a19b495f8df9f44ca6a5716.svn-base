...
<title>ADMINISTRATION: Dialer champs de compte de messagerie
<?php 
...
if ($SSemail_enabled < 1)
	{
	echo "ERREUR: emails entrants ne sont pas actifs sur ce système\n";
	exit;
	}
...
if (($VALIDER=="VALIDER" || $VALIDER=="UPDATE") && $email_account_id) 
	{
	$error_msg="";
	if (!$list_id) {$error_msg.="- Default list ID is invalide ou nulle<BR/>";}
	if (!$email_account_id) {$error_msg.="- Email account ID is invalide ou nulle<BR/>";}
	if (!$email_account_name) {$error_msg.="- Email account name is invalide ou nulle<BR/>";}
	if (!$email_account_server) {$error_msg.="- Email account server is invalide ou nulle<BR/>";}
	if (!$email_account_user) {$error_msg.="- Email account user is invalide ou nulle<BR/>";}
	if (!$email_account_pass) {$error_msg.="- Email account password is invalide ou nulle<BR/>";}
	if (!filter_var($email_replyto_address, FILTER_VALIDATE_EMAIL)) {$error_msg.="- Email reply-to address is invalide ou nulle<BR/>";}
	...
	if (!$error_msg) 
		...
		if ($VALIDER=="VALIDER") 
			...
			if ($add_copy_disabled > 0)
				{
				echo "<br>Vous n'avez pas la permission d'ajouter des enregistrements sur ce système -system_settings-\n";
				}
			else
				{
				...
				if (mysqli_affected_rows($link)==0) 
					...
					{
					$message="NOUVEAU COMPTE $email_account_id Créé avec succès";
					$eact="";
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="ACCOUNT $email_account_id Modifié avec succès";
				# $eact="";
	...
	if (mysqli_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysqli_affected_rows($link)==0) 
				{
				$error_msg.="- Il y avait une erreur inconnue lors de la tentative de copier le nouveau compte<BR/>";
				if($DB>0) {$error_msg.="<B>$ins_stmt</B><BR>";}
				}
			else
				{
				$message="NOUVEAU COMPTE $new_account_id SUCCES copiés à partir $source_email_account<BR/>";
				$eact="";
		...
		{
		$error_msg="- Error - mail source n'existe pas<BR/>";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "Compte pas copié<BR/>Les données présentées ont les erreurs suivantes:<BR/>$error_msg";}
		echo "<br>Copiez Nouveau compte Compte existant<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_VALIDER>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>ID du compte à copier à partir: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nouveau numéro de compte: </td><td align=left><input type=text name=new_account_id value='$new_account_id' size=10 maxlength=20></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nouveau nom de compte: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100>$NWB#email_accounts-carrier_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=VALIDER value='COPY'></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "</TD></TR></TABLE>\n";
		}
	else 
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** Il n'y a pas les comptes existants pour copier à partir de ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "Compte non INSERTED<BR/>Les données présentées ont les erreurs suivantes:<BR/>$error_msg";}
		echo "<br>Ajouter un nouveau compte de courrier entrant<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Nom du compte: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Actif: </td><td align=left><select size=1 name=active><option>Y</option><option SELECTED>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Description de compte: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Type de compte: </td><td align=left><select size=1 name=email_account_type><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Protocole de compte: </td><td align=left><select size=1 name=protocol><option SELECTED>IMAP</option><option>POP3</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Adresse de réponse: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Serveur: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Utilisateur Account: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Mot de passe: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">ID en groupe</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
		echo "</select>$NWB#email_accounts-in_group$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">ID par défaut de la liste</a>: </td><td align=left><input type=text name=default_list_id size=20 maxlength=255 value='$default_list_id'>$NWB#email_accounts-default_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>En groupe Agent Méthode de recherche:</td><td align=left><select size=1 name=agent_search_method><option value=\"LB\">LB - Load Balanced</option><option value=\"LO\">LO - Load Balanced Overflow</option><option value=\"SO\">SO - Server Only</option><option SELECTED>$agent_search_method</option></select>$NWB#email_accounts-agent_search_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Liste du groupe ID:</td><td align=left><input type=text name=list_id size=14 maxlength=14 value=\"$list_id\">$NWB#email_accounts-ingroup_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=VALIDER VALUE=VALIDER><input type=hidden name='eact' value='ADD'></td></tr>\n";
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>Confirmer la suppression du compte de messagerie INBOUND $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Cliquez sur pour supprimer un compte $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_to_mysqli($stmt, $link);
		...
		if ($error_msg) {echo "Compte non JOUR<BR/>Les données présentées ont les erreurs suivantes:<BR/>$error_msg";}
		echo "<br>Mettre à jour un compte de messagerie ENTRANT EN VIGUEUR<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Nom du compte: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Actif: </td><td align=left><select size=1 name=active>";
		echo "<option value='$active' selected>$active</option>";
		echo "<option>Y</option><option>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Description de compte: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Type de compte: </td><td align=left><select size=1 name=email_account_type><option value='$email_account_type' selected>$email_account_type</option><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Groupe d'utilisateurs d'administration: </td><td align=left><select size=1 name=user_group>\n";
		echo "<option value='$user_group' selected>$user_group</option>";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Tous les groupes d'utilisateurs d'administration</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Protocole de compte: </td><td align=left><select size=1 name=protocol><option>IMAP</option><option>POP3</option><option SELECTED>$protocol</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Adresse de réponse: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Serveur: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Utilisateur Account: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Mot de passe: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "</select>$NWB#email_accounts-email_frequency_check_mins$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=3811&group_id=$group_id\">ID en groupe</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
...
###############
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Les e-mails non traités: </td><td align=left><B>$unhandled_emails</B></td></tr>\n";
################
		
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=VALIDER VALUE=UPDATE><input type=hidden name='eact' value='UPDATE'></td></tr>\n";
		echo "</TABLE><BR><BR>";
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Cliquez ici pour voir les changements d'administration de ce disque</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>ENTRANTS BIENS compte de messagerie:\n";
	...
	echo "<TD><font size=1 color=white>Courriers non lus</TD>\n";
	echo "<TD><font size=1 color=white>MODIFIER</TD></tr>\n";
	...
	while ($accounts_to_print > $o) 
		...
		echo "<td><font size=1><a href=\"$PHP_SELF?eact=UPDATE&email_account_id=$row[email_account_id]\">MODIFIER</a></font></td></tr>\n";
		$o++;
		}
