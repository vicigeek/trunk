...
if ($user_territories_active < 1)
	{
	echo "ERROR: Χρήστης Εδάφη δεν δραστηριοποιούνται σε αυτό το σύστημα\n";
	exit;
	}
...
if( (strlen($USER)<2) or (strlen($PASS)<2) or (!$auth) or (!$modify_users_auth))
	...
	echo "Ακυρο Ονομα Χρήστη/Κωδικός Πρόσβασης: |$USER|$PASS|$auth_message|\n";
	exit;
	}
...
<title>ADMINISTRATION: Χρήστης Εδάφη
<?php
...
if ( ($action == "CHANGE_TERRITORY_OWNER_ACCOUNT") and ($enable_vtiger_integration > 0) )
	...
	echo "<br>Vtiger Αλλαγή Επικράτεια Ιδιοκτήτης<form action=$PHP_SELF method=POST>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>New Ιδιοκτήτης:</td><td align=left><select size=1 name=user>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Update ViciDial List Ιδιοκτήτης:</td><td align=left><select size=1 name=vl_owner>\n";
...
if ( ($action == "PROCESS_CHANGE_TERRITORY_OWNER_ACCOUNT") and ($enable_vtiger_integration > 0) )
	...
	if ( (strlen($accountid)<1) or (strlen($user)<1) )
		{
		echo "ERROR: Account ID and Χρήστης must be filled in<BR>\n";
		}
	else
		{
		if (preg_match("/YES/i",$batch))
		...
		if ($vat_ct > 0)
			...
			if ($row[0] < 1)
				...
				echo "NOTICE: Έπρεπε να προσθέσετε έδαφος χρήστη: $user $territory<BR>\n";
			...
			if ($vtu_ct > 0)
				...
				echo "Vtiger Επικράτεια Ιδιοκτήτης Changed: $user $territory<BR>\n";
				echo "Εγγραφές Changed: $changed<BR>\n";
...
if ( ($action == "CHANGE_TERRITORY_OWNER") and ($enable_vtiger_integration > 0) )
	...
	echo "<center><TABLE width=$section_width cellspacing=3>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Επικράτεια: </td><td align=left><select size=1 name=territory>\n";
...
if ( ($action == "PROCESS_CHANGE_TERRITORY_OWNER") and ($enable_vtiger_integration > 0) )
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		{
		echo "ERROR: Έδαφος και χρήστη πρέπει να συμπληρωθεί<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($vtu_ct > 0)
			...
			echo "Vtiger Επικράτεια Ιδιοκτήτης Changed: $user $territory &nbsp; &nbsp; &nbsp; Εγγραφές Changed: $changed - $Achanged - $Cchanged - $Vchanged<BR>\n";
...
if ($action == "ADD_USER_TERRITORY")
	...
	echo "<br>Προσθήκη Χρήστη Επικράτεια<form action=$PHP_SELF method=POST>\n";
...
if ($action == "PROCESS_ADD_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		{
		echo "ERROR: Έδαφος, χρηστών και το επίπεδο πρέπει να συμπληρωθεί<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($row[0] > 0)
			{
			echo "ERROR: αυτός ο χρήστης έδαφος είναι ήδη στο σύστημα<BR>\n";
			}
		else
			{
			...
			echo "Χρήστης Επικράτεια Added: $user $territory<BR>\n";
...
if ($action == "PROCESS_MODIFY_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		...
		if ($row[0] < 0)
			{
			echo "ERROR: Μέλους έδαφος δεν είναι στο σύστημα<BR>\n";
			}
		else
			{
			...
			echo "Χρήστη Επικράτεια Τροποποιημένο: $user $territory<BR>\n";
...
if ($action == "DELETE_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		...
		if ($row[0] < 0)
			...
			echo "Χρήστη Επικράτεια Διαγράφεται: $user $territory<BR>\n";
...
if ($action == "ADD_TERRITORY")
	...
	echo "<br>Προσθήκη Επικράτεια<form action=$PHP_SELF method=POST>\n";
	echo "<input type=hidden name=action value=PROCESS_ADD_TERRITORY>\n";
	echo "<center><TABLE width=$section_width cellspacing=3>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Επικράτεια: </td><td align=left><input type=text name=territory size=30 maxlength=100></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Περιγραφή επικράτειας: </td><td align=left><input type=text name=territory_description size=50 maxlength=255></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ΕΠΙΒΕΒΑΙΩΣΗ value=ΕΠΙΒΕΒΑΙΩΣΗ></td></tr>\n";
	echo "</TABLE></center>\n";
	}
...
if ($action == "PROCESS_ADD_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		{
		echo "ERROR: Έδαφος και Επικράτεια Περιγραφή πρέπει να συμπληρωθεί<BR>\n";
		}
	else
		{
		...
		if ($row[0] > 0)
			{
			echo "ERROR: υπάρχει ήδη στο έδαφος του συστήματος με αυτό το όνομα<BR>\n";
			}
		else
			{
			...
			echo "Έδαφος Added: $territory<BR>\n";
...
if ($action == "DELETE_TERRITORY")
	...
	if (strlen($territory)<1)
		{
		echo "ERROR: Έδαφος πρέπει να συμπληρωθεί<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_territories where territory='$territory';";
		...
		if ($row[0] < 0)
			{
			echo "ERROR: Αυτό το έδαφος δεν είναι το σύστημα με αυτό το όνομα<BR>\n";
			}
		else
			{
			...
			echo "Έδαφος Διαγράφεται: $territory<BR>\n";
...
if ($action == "PROCESS_MODIFY_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		...
		if ($row[0] < 1)
			...
			echo "Έδαφος Τροποποιημένο: $territory<BR>\n";
...
if ($action == "MODIFY_TERRITORY")
	...
	if ($territories_to_print > 0) 
		...
		echo "<br>Τροποποίηση Επικράτεια<form action=$PHP_SELF method=POST>\n";
		echo "<input type=hidden name=action value=PROCESS_MODIFY_TERRITORY>\n";
		echo "<input type=hidden name=territory value=\"$territory\">\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Επικράτεια: </td><td align=left><B>$rowx[0]</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Περιγραφή επικράτειας: </td><td align=left><input type=text name=territory_description size=50 maxlength=255 value=\"$rowx[1]\"></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Πράκτορες: </td><td align=left><B>$user_count</B></td></tr>";
		...
		if ($enable_vtiger_integration > 0)
			...
			echo "<tr bgcolor=#B6D3FC><td align=right>Vtiger Λογαριασμοί: </td><td align=left><B>$vtiger_count</B></td></tr>";
			}
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ΕΠΙΒΕΒΑΙΩΣΗ value=ΕΠΙΒΕΒΑΙΩΣΗ></form></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "<BR><BR>\n";

		echo "<TABLE CELLPADDING=3 CELLSPACING=1 WIDTH=600><TR><TD COLSPAN=5><FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>Χρήστες in this Επικράτεια:</TD></TR>\n";
		...
		#	echo "<TD>Accounts: $owner_count</TD>";
			...
			echo "</form>";
			echo "</TD>";
			echo "<TD><a href=\"$PHP_SELF?action=DELETE_USER_TERRITORY&territory=$territory&user=$Tuser[$o]\">ΔΙΑΓΡΑΦΗ</a></TD>";
			echo "</TR>\n";
			$o++;
			}
		...
		echo "<a href=\"$PHP_SELF?action=DELETE_TERRITORY&territory=$territory\">Διαγραφή Αυτό Επικράτεια</a>\n";
		echo "<BR><BR>\n";
		}
	else
		{
		echo "ERROR: Έδαφος δεν βρέθηκε: $territory<BR>\n";
		}
	}
...
<BR><font size=1>Χρήστης Εδάφη &nbsp; &nbsp; ΕΚΔΟΣΗ: <?php echo $version ?> &nbsp; &nbsp; ΔΗΜΙΟΥΡΓΙΑ: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
