...
if ($ADD==341111111111111)
	...
	if ( ($LOGmodify_servers==1) and ($SSqc_features_active > 0) )
		...
		echo "<tr><td>STATUS CODE</td><td>ΠΕΡΙΓΡΑΦΗ</td><td>QC ΚΑΤΗΓΟΡΙΑ</td><td>MODIFY/DELETE</td></tr>\n";
		...
		if ($rowx[0] > 0)
			...
			while ($statuses_to_print > $o)
				...
				if (preg_match("/^B$|^NA$|^DNC$|^NA$|^DROP$|^INCALL$|^QUEUE$|^NEW$/i",$rowx[0]))
					...
					{
					echo "<a href=\"$PHP_SELF?ADD=441111111111111&code=$rowx[0]&stage=delete\">ΔΙΑΓΡΑΦΗ</a>\n";
					}
				echo "</form></td></tr>\n";
				}
			}
		...
		echo "Περιγραφή:<input type=text name=code_name size=25 maxlength=30> \n";
                echo "QC Κατηγορία:<select name=qc_category><option selected value=''>-Choose-</option><option value='PASS'>PASS</option><option value='FAIL'>FAIL</option><option value='CANCEL'>CANCEL</option><option value='COMMIT'>COMMIT</option><BR>\n";
