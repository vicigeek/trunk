...
if( (strlen($PHP_AUTH_USER)<2) or (strlen($PHP_AUTH_PW)<2) or (!$auth))
    ...
    Header("WWW-Authenticate: Basic realm=\"CONTACT-CENTER-ADMIN\"");
    Header("HTTP/1.0 401 Unauthorized");
    echo "Nome ou Senha inválidos: |$PHP_AUTH_USER|$PHP_AUTH_PW|$auth_message|\n";
    exit;
	}
else
	{
...
if (!$ADD)			{$ADD="31111";}
if ($ADD==31111)	{echo "Alterar Agentes Remotos";}
if ($ADD==41111)	{echo "Alterar Agentes Remotos";}
...
if ($ADD==31111)
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Início do ID do Usuário: </td><td align=left>$user_start</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Número de linhas: </td><td align=left><input type=text name=number_of_lines size=3 maxlength=3 value=\"$number_of_lines\"> (somente números)</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>IP do Servidor: </td><td align=left>$row[3]</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Extensão Externa: </td><td align=left><input type=text name=conf_exten size=20 maxlength=20 value=\"$conf_exten\"> (number dialed to reach agents [i.e. 913125551212])</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Status: </td><td align=left><select size=1 name=status><option SELECTED>ATIVO</option><option>INACTIVE</option><option SELECTED>$status</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Campanha: </td><td align=left>$campaign_id</td></tr>\n";
...
if ($ADD==41111)
	...
	if ( (strlen($number_of_lines) < 1) or (strlen($conf_exten) < 2) )
		{echo "<br>AGENTES REMOTOS NÃO ALTERADOS - Por favor volte e verifique os dados digitados\n";}
	else
		{
...
#		echo "$stmt\n";
		echo "<br>AGENTES REMOTOS ALTERADOS\n";
...
if ($ADD==61111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		{echo "<br>ERRO DE AGENTES REMOTOS – Por favor volte e verifique os dados digitados \n";}
	else
		{
		$users_list = '';
		$k=0;
		...
		if ($talking_to_print > 0)
			...
			echo "  <SPAN class=\"yellow\"><B>          </SPAN> - Acima de 5 minutos em chamada</B>\n";
			echo "  <SPAN class=\"orange\"><B>          </SPAN> - Acima de 10 minutos em chamada</B>\n";
			}
		else
			{
			echo "**************************************************************************************\n";
...
if ($ADD==71111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		...
		echo "Estatísticas de entrada para Agentes Remotos                               $NOW_TIME\n\n";
		echo "\n";
