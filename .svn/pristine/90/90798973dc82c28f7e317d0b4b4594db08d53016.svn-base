	...
	if ($valid_user < 1)
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "Vous avez maintenant connecté";
				$LOGtimeMESSAGE = "Vous connecté à $NOW_TIME";
			...
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">RETOUR à l administration</font></A>";}
			if ($referrer=='welcome') 
