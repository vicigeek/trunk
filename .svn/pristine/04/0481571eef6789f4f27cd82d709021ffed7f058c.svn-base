...
438 => "Ακυρο Identity Header.",
470 => "Consent Needed.",
480 => "Temporarily Unavailable.",
481 => "Call/Transaction Does Not Exist.",
482 => "Loop Detected..",
483 => "Too Many Hops.",
484 => "Διεύθυνση Incomplete.",
485 => "Ambiguous.",
486 => "Busy Here.",
...
500 => "Κεντρικός υπολογιστής Internal Error.",
501 => "Not Implemented.",
502 => "Bad Gateway.",
503 => "Service Unavailable.",
504 => "Κεντρικός υπολογιστής Time-out.",
...
if ($format=='debug')
	...
	echo "<title>VICIDiaL Κώδικας ερώτησης βάσεων δεδομένων";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		if ($cl_user_ct > 0)
			...
			### CHECK TO SEE IF πράκτορας IS LOGGED IN TO TIMECLOCK, IF NOT, OUTPUT ERROR
	...
	if ($show_campaign_list > 0)
		...
		echo "<option value=\"\">-- Επιλέξτε μια καμπάνια --</option>\n";
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Επιλογή ομάδας $closer_choice δεν ισχύει\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Επιλογή ομάδας $closer_choice έχει καταχωρηθεί στο χρήστη $user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories δεν ισχύει\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories έχει καταχωρηθεί στο χρήστη $user\n";
	}
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
			...
			if (strlen($phone_number)>3)
				...
				if (preg_match("/ENABLED/",$manual_dial_call_time_check))
					...
					if ($dialable < 1)
						...
						echo "OUTSIDE OF LOCAL ΚΛΗΣΗ TIME   $VMDQaffected_rows|$VLAEDaffected_rows\n";
						exit;
						}
					}
				...
				#	$DB=1;
					if (strlen($dial_statuses)>2)
						...
						if (preg_match("/UP τελευταίος NAME/i",$lead_order)){$order_stmt = "order by last_name desc, $last_order";}
						if (preg_match("/DOWN τελευταίος NAME/i",$lead_order)){$order_stmt = "order by last_name, $last_order";}
						...
						if (preg_match("/UP τελευταία κλήση TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
						if (preg_match("/DOWN τελευταία κλήση TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				{
				//Added by Poundteam for Audited Σχόλια
				...
				//END Added by Poundteam for Audited Σχόλια
				}
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "τηλεφώνου και μετά του χρόνου κωδικό αναντιστοιχία ζώνη! ";

					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY')
						{
						$post_phone_time_diff_alert_message='';
						if ($PHONEdialable < 1)
							{$post_phone_time_diff_alert_message .= " Κώδικας Τηλέφωνο περιοχή εκτός της ζώνης Dialable $PHONEgmt_offset ";}
						if ($POSTdialable < 1)
							{$post_phone_time_diff_alert_message .= " Τ.Κ. Έξω Dialable Ζώνη $POSTgmt_offset";}
						}
					}
			...
			#	$Local_persist = '/n';
				...
				### check for extension append στην εκστρατεία
				$use_eac=0;
	...
	#				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
				...
				if (strlen($dial_ingroup) < 1)
					...
					if ($enable_queuemetrics_logging > 0)
						...
						# ΚΛΗΣΗOUTBOUND (formerly ENTERQUEUE)
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "ΕΝΑΛΛΑΚΤΙΚΟ ΤΗΛΕΦΩΝΟ ΑΡΙΘΜΟΣ ΚΑΤΑΣΤΑΣΗ NOT CHANGED\n";
		...
		echo "ΕΝΑΛΛΑΚΤΙΚΟ ΤΗΛΕΦΩΝΟ ΑΡΙΘΜΟΣ ΚΑΤΑΣΤΑΣΗ CHANGED\n";
		}
}
...
if ($ACTION == 'manDiaLonly')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1) || (strlen($ext_context)<1) || (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		...
		### check for manual dial filter and extension append settings στην εκστρατεία
		$use_eac=0;
		$use_custom_cid=0;
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		if ($affected_rows > 0)
			{
			echo "VICIDiaL_LOG Εισαγωγή : $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			if ( ($call_type=='OUT') or ($call_type=='OUTBALANCE') )
				...
				### if web form is set then send στο to vicidial.php for override of WEB_FORM address
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups στο vicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row στο selected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Εκστρατεία $campaign δεν ισχύει or lead_id $lead_id δεν ισχύει\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user στο $server_ip\n";
			exit;
			}
		}
	}
		...
		if ($vla_delete > 0) 
			...
			#	$affected_rows = mysqli_affected_rows($linkB);
				...
				if ($queuemetrics_addmember_enabled > 0)
					...
					### add the logged-στην εκστρατεία as well
					$AMqueue[$i] = $campaign;
					$i++;
					$amq_conf_ct++;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id δεν ισχύει\n";
		exit;
		}
	else
		{
		...
		//Added by Poundteam Incorporated for Audit Σχόλια Package
		require_once('audit_comments.php');
		audit_comments($lead_id,$list_id,$format,$user,$mel,$NOW_TIME,$link,$server_ip,$session_name,$one_mysql_log,$campaign);

		// JOEJ - Ηλεκτρονικό ταχυδρομείο feature - may not be necessary if vicidial_email_list doesn't need a status column.
		if ($email_enabled>0) 
			{
		...
		if ( (preg_match('/ACCTID/',$vtiger_search_category)) or (preg_match('/ACCOUNT/',$vtiger_search_category)) )
			...
			if ($VIDcount > 0)
				...
				if (preg_match('/Y/',$vtiger_status_call))
					...
					# Date of Επίθετο Attempt, Date of Επίθετο Non-Contact, Date of Επίθετο Contact, Date of Επίθετο Sale
	...
	echo 'Lead ' . $lead_id . ' έχει αλλαχτεί ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id δεν ισχύει\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue στο to that action without printing output
		$stage = 0;
		$status = $sub_status;
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list δεν ισχύει\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		{
		echo "agent_log_id $agent_log_id or pause_code $status δεν ισχύει\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-ΕΤΟΙΜΟ &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-Incall &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-ΠΑΥΣΗ &nbsp;\n";
	if (preg_match("/NOT-LOGGED-IN-AGENTS/",$agent_status_viewable_groups))
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		...
		### grab the basic data στο calls in the queue for this agent
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τηλέφωνο &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ονοματεπώνυμο &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; περιμένετε &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; πράκτορας &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; καλέστε την ομάδα &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τύπος &nbsp; </font></TD>";
		echo "</TR>";

		### Print call information and gather more info στο the calls as they are printed
		$loop_count=0;
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">TAKE ΚΛΗΣΗ</a> &nbsp; </TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<a href=\"#\" onclick=\"VieWCalLLoG('','form');return false;\">πάμε</a> &nbsp;  &nbsp; &nbsp; ";
	echo "<a href=\"#\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> κοντά </a>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
	echo "<TR>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> &nbsp; # &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Ημερομηνία\/ώρα &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; μήκος &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΑΤΑΣΤΑΣΗ &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τηλέφωνο &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ονοματεπώνυμο &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; εκστρατεία &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; μέσα και έξω &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; αναπληρωματικό &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ανωμαλίας &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; info &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΛΗΣΗ &nbsp; </font></TD>";
	echo "</TR>";
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>Δεν υπάρχουν κλήσεις για αυτή την ημέρα</td></tr>";}
	...
	while ($g > $u) 
		...
		if ($manual_dial_filter > 0)
			{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ΚΛΗΣΗ </A> </td>\n";}
		else
			{echo "<td align=right><font size=2> ΚΛΗΣΗ </td>\n";}
		echo "</tr>\n";
		}

	echo "</TABLE>";
	echo "<BR>";
	echo "<a href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">Κλείστε Πρόσκληση Σύνδεση</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			{
			echo "error: θα πρέπει να εισάγετε όρους αναζήτησης, ένα από αυτά πρέπει να είναι πυκνοκατοικημένη: μόλυβδος id, id πωλητή, τον αριθμό τηλεφώνου, το επώνυμο\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Επιστροφή</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<CENTER>\n";
			echo "<font style=\"font-size:14px;font-family:sans-serif;\"><B>";
			echo "Αποτελέσματα: $search_result_count";
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			echo "<TR>";
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τηλέφωνο &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΑΤΑΣΤΑΣΗ &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τελευταία κλήση &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; πόλη &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; κατάσταση &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr bgcolor=white><td colspan=10 align=center>Δεν βρέθηκαν αποτελέσματα</td></tr>";}
				...
				while ($g > $u) 
					...
					if ($inbound_lead_search < 1)
						{
						if ($manual_dial_filter > 0)
							{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ΚΛΗΣΗ </A> </td>\n";}
						else
							{echo "<td align=right><font size=2> ΚΛΗΣΗ </td>\n";}
						}
					else
						{
						echo "<td align=right><font size=2> <a href=\"#\" onclick=\"LeaDSearcHSelecT('$ALLlead_id[$i]');return false;\">επιλέξτε</A> </td>\n";
						}
					echo "</tr>\n";
					}
				...
				$stmtL="UPDATE vicidial_lead_search_log set δευτερόλεπτα='$search_seconds' where search_log_id='$search_log_id';";
				if ($DB) {echo "|$stmtL|\n";}
				$rslt=mysql_to_mysqli($stmtL, $link);
			...
			{
			echo "error: υπήρχε ένα πρόβλημα με δικούς σας όρους αναζήτησης\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Επιστροφή</a>";
			echo "</CENTER>";
			exit;
		...
		{
		echo "ERROR: Εκστρατεία not found\n";
		echo "<BR><BR>";
		echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Επιστροφή</a>";
		echo "</CENTER>";
		exit;
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			{
			echo "error: θα πρέπει να εισάγετε όρους αναζήτησης, μία από αυτές θα πρέπει να συμπληρωθεί: αριθμός του γραφείου, το επώνυμο, το όνομα\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">Επιστροφή</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0)
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τελευταίος NAME &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					echo "</tr>\n";
					echo "<tr $bgcolor>";
					echo "<td colspan=8><font size=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <B>Dept:</B> $ALLdepartment[$i] &nbsp; &nbsp; &nbsp; <B>Group:</B> $ALLgroup_name[$i] &nbsp; &nbsp; &nbsp; <B>Job:</B> $ALLjob_title[$i] &nbsp; &nbsp; &nbsp; <B>τοποθεσία:</B> $ALLlocation[$i]</td>";
					echo "</tr>\n";
					}
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Κατάσταση επανάκλησης: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Επανάκληση Κατάσταση μόλυβδο: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Χρόνος εισόδου επανάκλησης: &nbsp; </td><td ALIGN=left><font size=2>$row[1]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Επανάκληση χρόνο ενεργοποίησης: &nbsp; </td><td ALIGN=left><font size=2>$row[2]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Σχόλια επανάκλησης: &nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Κατάσταση: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Χρονική ζώνη: &nbsp; </td><td ALIGN=left><font size=2>$row[3]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>ζήτησε από τον τελευταίο μηδενισμό: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ΚΛΗΣΗ </a>";}
					else
						{$INFOout .= " ΚΛΗΣΗ ";}
					}
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>dial link: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ΚΛΗΣΗ </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>dial link: &nbsp; </td><td ALIGN=left><font size=2> ΚΛΗΣΗ ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right colspan=2><font size=2><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">επιλέξτε αυτό το προβάδισμα</a></td></tr>";}
			...
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\"> ΚΛΗΣΗ </a>";}
					else
						{$INFOout .= " ΚΛΗΣΗ ";}
					}
				}
			...
			if ($hide_call_log_info=='N')
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>αριθμό των κλήσεων: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";}
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>τελευταία τοπική ώρα κλήσης: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			{
			if ($search != 'logfirst')
				{$NOTESout .= "<CENTER>Κλήσεων ΓΙΑ ΑΥΤΟ ΤΟ ΜΟΛΥΒΔΟ:<br>\n";}
			...
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Ημερομηνία\/ώρα &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; πράκτορας &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; μήκος &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΑΤΑΣΤΑΣΗ &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; τηλέφωνο &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; εκστρατεία &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; μέσα και έξω &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; αναπληρωματικό &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ανωμαλίας &nbsp; </font></TD>";
		#	$NOTESout .= "</TR><TR>";
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ονοματεπώνυμο &nbsp; </font></TD>";
			$NOTESout .= "</TR>";
			...
			if ($g > 0)
				...
				{$NOTESout .= "<tr bgcolor=white><td colspan=11 align=center>Δεν υπάρχουν Προκηρύξεις</td></tr>";}
		...
		### BEGIN Ηλεκτρονικό ταχυδρομείο log
		if ($allow_emails>0)
			{
			$NOTESout .= "<CENTER>log email για αυτό το προβάδισμα:<br>\n";
			$NOTESout .= "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> # </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Ημερομηνία\/ώρα </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; πράκτορας </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; εκστρατεία </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; email στο </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; συνημμένα </B></font></td>";
			$NOTESout .= "</tr>\n";
		...
		### END Ηλεκτρονικό ταχυδρομείο Log ##
...
function hangup_cause_description($code)
	...
	else { return "αγνώστων κωδικό αιτία ανωμαλίας."; }
	}
