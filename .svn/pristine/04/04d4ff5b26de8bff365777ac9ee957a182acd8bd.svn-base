...
if ($admin_viewable_groupsALL > 0)
	{$UUgroups_list .= "<option value=\"---ALL---\">Todos os Grupos de Usuários de Admin</option>\n";}
...
<title>ADMINISTRATION: Dialer-mail campos Conta
<?php 
...
if ($SSemail_enabled < 1)
	{
	echo "ERRO: e-mails de entrada não estão ativos no sistema\n";
	exit;
	}
...
if (($ENVIAR=="ENVIAR" || $ENVIAR=="UPDATE") && $email_account_id) 
	{
	$error_msg="";
	if (!$list_id) {$error_msg.="- Default list ID is inválida ou nula<BR/>";}
	if (!$email_account_id) {$error_msg.="- Email account ID is inválida ou nula<BR/>";}
	if (!$email_account_name) {$error_msg.="- Email account name is inválida ou nula<BR/>";}
	if (!$email_account_server) {$error_msg.="- Email account server is inválida ou nula<BR/>";}
	if (!$email_account_user) {$error_msg.="- Email account user is inválida ou nula<BR/>";}
	if (!$email_account_pass) {$error_msg.="- Email account password is inválida ou nula<BR/>";}
	if (!filter_var($email_replyto_address, FILTER_VALIDATE_EMAIL)) {$error_msg.="- Email reply-to address is inválida ou nula<BR/>";}
	...
	if (!$error_msg) 
		...
		if ($ENVIAR=="ENVIAR") 
			...
			if ($add_copy_disabled > 0)
				{
				echo "<br>Você não tem permissão para adicionar registros no sistema -system_settings-\n";
				}
			else
				{
				...
				if (mysqli_affected_rows($link)==0) 
					...
					{
					$message="NOVA CONTA $email_account_id CRIADO COM SUCESSO";
					$eact="";
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="ACCOUNT $email_account_id SUCESSO MODIFICADO";
				# $eact="";
	...
	if (mysqli_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysqli_affected_rows($link)==0) 
				{
				$error_msg.="- Ocorreu um erro desconhecido ao tentar copiar a nova conta<BR/>";
				if($DB>0) {$error_msg.="<B>$ins_stmt</B><BR>";}
				}
			else
				{
				$message="NOVA CONTA $new_account_id SUCESSO copiado de $source_email_account<BR/>";
				$eact="";
		...
		{
		$error_msg="- Error - e-mail de origem não existe<BR/>";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "Conta não COPIADO<BR/>Os dados apresentados têm os seguintes erros:<BR/>$error_msg";}
		echo "<br>Copie Nova Conta de Conta Existente<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_ENVIAR>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>ID da conta para copiar de: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>New ID Conta: </td><td align=left><input type=text name=new_account_id value='$new_account_id' size=10 maxlength=20></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Novo Nome da conta: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100>$NWB#email_accounts-carrier_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ENVIAR value='COPY'></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "</TD></TR></TABLE>\n";
		}
	else 
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** Não há contas existentes para copiar a partir de ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "Conta não INSERIDO<BR/>Os dados apresentados têm os seguintes erros:<BR/>$error_msg";}
		echo "<br>Adicionar Nova Conta-mails recebidos<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>Email Conta ID: </td><td align=left><input type=text name=email_account_id size=15 maxlength=20 value='$email_account_id'>$NWB#email_accounts-email_account_id$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nome da conta de e-mail: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Ativo: </td><td align=left><select size=1 name=active><option>Y</option><option SELECTED>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Descrição da Conta: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Tipo de Conta: </td><td align=left><select size=1 name=email_account_type><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Grupo de Usuários do Administrador: </td><td align=left><select size=1 name=user_group>\n";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Todos os Grupos de Usuários de Admin</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Conta Protocolo: </td><td align=left><select size=1 name=protocol><option SELECTED>IMAP</option><option>POP3</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Endereço de Resposta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email conta do servidor: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail de Conta de Usuário: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail a senha da conta: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">No Grupo ID</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
		echo "</select>$NWB#email_accounts-in_group$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">Padrão Lista de ID</a>: </td><td align=left><input type=text name=default_list_id size=20 maxlength=255 value='$default_list_id'>$NWB#email_accounts-default_list_id$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>Método de Manuseio de Chamada Entrante:</td><td align=left><select size=1 name=call_handle_method><option>EMAIL</option><option>EMAILLOOKUP</option><option>EMAILLOOKUPRL</option><option>EMAILLOOKUPRC</option><option SELECTED>$call_handle_method</option></select>$NWB#email_accounts-call_handle_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Método de Busca de Agente p/ Ch. Entr.:</td><td align=left><select size=1 name=agent_search_method><option value=\"LB\">LB - Load Balanced</option><option value=\"LO\">LO - Load Balanced Overflow</option><option value=\"SO\">SO - Server Only</option><option SELECTED>$agent_search_method</option></select>$NWB#email_accounts-agent_search_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>ID da Lista de Entrada:</td><td align=left><input type=text name=list_id size=14 maxlength=14 value=\"$list_id\">$NWB#email_accounts-ingroup_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ENVIAR VALUE=ENVIAR><input type=hidden name='eact' value='ADD'></td></tr>\n";
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>Confirmar a exclusão do INBOUND conta de email $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Clique para excluir conta $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_to_mysqli($stmt, $link);
		...
		if ($error_msg) {echo "Conta não ATUALIZADA<BR/>Os dados apresentados têm os seguintes erros:<BR/>$error_msg";}
		echo "<br>Atualizar uma conta existente e-mails recebidos<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>Email Conta ID: </td><td align=left><input type=hidden name=email_account_id value='$email_account_id'><B>$email_account_id$NWB#email_accounts-email_account_id$NWE</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nome da conta de e-mail: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Ativo: </td><td align=left><select size=1 name=active>";
		echo "<option value='$active' selected>$active</option>";
		echo "<option>Y</option><option>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Descrição da Conta: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Tipo de Conta: </td><td align=left><select size=1 name=email_account_type><option value='$email_account_type' selected>$email_account_type</option><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Grupo de Usuários do Administrador: </td><td align=left><select size=1 name=user_group>\n";
		echo "<option value='$user_group' selected>$user_group</option>";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Todos os Grupos de Usuários de Admin</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Conta Protocolo: </td><td align=left><select size=1 name=protocol><option>IMAP</option><option>POP3</option><option SELECTED>$protocol</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Endereço de Resposta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email conta do servidor: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail de Conta de Usuário: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail a senha da conta: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "</select>$NWB#email_accounts-email_frequency_check_mins$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=3811&group_id=$group_id\">No Grupo ID</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
...
###############
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Os e-mails não-tratados: </td><td align=left><B>$unhandled_emails</B></td></tr>\n";
################
		
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ENVIAR VALUE=UPDATE><input type=hidden name='eact' value='UPDATE'></td></tr>\n";
		echo "</TABLE><BR><BR>";
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Clique aqui para ver as mudanças de administrador para este registro</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>ENTRADA EMAIL LISTAS CONTA:\n";
	...
	echo "<TD><font size=1 color=white>DESCRIÇÃO</TD>\n";
	...
	echo "<TD><font size=1 color=white>-Mails não lidos</TD>\n";
	echo "<TD><font size=1 color=white>ALTERAR</TD></tr>\n";
	...
	while ($accounts_to_print > $o) 
		...
		echo "<td><font size=1><a href=\"$PHP_SELF?eact=UPDATE&email_account_id=$row[email_account_id]\">ALTERAR</a></font></td></tr>\n";
		$o++;
		}
