...
28 => "Nesprávny number format (address incomplete).",
29 => "Facilities rejected.",
30 => "Response to STAV INQUIRY.",
...
81 => "Nesprávny call reference value.",
88 => "Incompatible destination.",
95 => "Nesprávny message, unspecified.",
...
100 => "Nesprávny information element contents.",
101 => "Message not compatible with call state.",
102 => "Recovery na timer expiry.",
103 => "Parameter non-existent or not implemented - passed na (national use).",
...
438 => "Nesprávny Identity Header.",
470 => "Consent Needed.",
480 => "Temporarily Unavailable.",
481 => "Call/Transaction Does Not Exist.",
482 => "Loop Detected..",
483 => "Too Many Hops.",
484 => "Adresa Incomplete.",
485 => "Ambiguous.",
486 => "Busy Here.",
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
		{
		echo "Nesprávny Užívateľské meno/Heslo: |$user|$pass|$auth_message|\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		if ($cl_user_ct > 0)
			...
			### CHECK TO SEE IF zástupca IS LOGGED IN TO TIMECLOCK, IF NOT, OUTPUT ERROR
	...
	### CHECK TO SEE IF zástupca IS WITHIN THEIR SHIFT IF RESTRICTED, IF NOT, OUTPUT ERROR
	if ( ( (preg_match("/START|ALL/",$shift_enforcement)) and (!preg_match("/OFF/",$VU_agent_shift_enforcement_override)) ) or (preg_match("/START|ALL/",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>ERROR: Nie sú k dispozícii žiadne Posuvy povolený pre užívateľské skupiny</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) )
				{
				$VDdisplayMESSAGE = "<B>ERROR: Nemáte možnosť prihlásiť sa mimo vašej smeny</B>\n";
				$VDloginDISPLAY=1;
				}
			}
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=submit NAME=POTVRDIŤ VALUE=POTVRDIŤ></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		echo "<option value=\"\">-- Vyberte KAMPAŇ --</option>\n";
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- Musíte sa prihlásiť do TIMECLOCK PRVEJ --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories Nie je správny\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories bol zaregistrovaný pre užívateľa $user\n";
	}
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
			...
			if (strlen($phone_number)>3)
				...
				if (preg_match("/ENABLED/",$manual_dial_call_time_check))
					...
					if ($dialable < 1)
						...
						echo "OUTSIDE OF LOCAL HOVOR TIME   $VMDQaffected_rows|$VLAEDaffected_rows\n";
						exit;
						}
					}
				...
				#	$DB=1;
					if (strlen($dial_statuses)>2)
						...
						if (preg_match("/UP posledný NAME/i",$lead_order)){$order_stmt = "order by last_name desc, $last_order";}
						if (preg_match("/DOWN posledný NAME/i",$lead_order)){$order_stmt = "order by last_name, $last_order";}
						...
						if (preg_match("/UP Posledná výzva TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
						if (preg_match("/DOWN Posledná výzva TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				{
				//Added by Poundteam for Audited Komentár
				...
				//END Added by Poundteam for Audited Komentár
				}
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "telefón a poštové smerovacie číslo časové pásmo nesúlad! ";

					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY')
						{
						$post_phone_time_diff_alert_message='';
						if ($PHONEdialable < 1)
							{$post_phone_time_diff_alert_message .= " Telefón Area Code Mimo Dialable zóny $PHONEgmt_offset ";}
						if ($POSTdialable < 1)
							{$post_phone_time_diff_alert_message .= " PSČ Mimo Dialable zóny $POSTgmt_offset";}
						}
					}
			...
			#	$Local_persist = '/n';
				...
				### check for extension append v kampani
				$use_eac=0;
	...
	#				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
				...
				if (strlen($dial_ingroup) < 1)
					...
					if ($enable_queuemetrics_logging > 0)
						...
						# HOVOROUTBOUND (formerly ENTERQUEUE)
...
if ($ACTION == 'manDiaLonly')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1) || (strlen($ext_context)<1) || (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		...
		### check for manual dial filter and extension append settings v kampani
		$use_eac=0;
		$use_custom_cid=0;
...
if ($ACTION == 'manDiaLlookCaLL')
	...
	if (strlen($MDnextCID)<18)
		...
		if ($VM_mancall_ct > 0)
			...
			{
			### after 10 sekúnd, start checking for call termination in the carrier log
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups na vicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row na selected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Kampaň $campaign Nie je správny or lead_id $lead_id Nie je správny\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user na $server_ip\n";
			exit;
			}
		}
	}
		...
		if ($vla_delete > 0) 
			...
			#	$affected_rows = mysqli_affected_rows($linkB);
				...
				if ($queuemetrics_addmember_enabled > 0)
					...
					### add the logged-v kampani as well
					$AMqueue[$i] = $campaign;
					$i++;
					$amq_conf_ct++;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		...
		//Added by Poundteam Incorporated for Audit Komentár Package
		require_once('audit_comments.php');
		...
		if ( (preg_match('/ACCTID/',$vtiger_search_category)) or (preg_match('/ACCOUNT/',$vtiger_search_category)) )
			...
			if ($VIDcount > 0)
				...
				if (preg_match('/Y/',$vtiger_status_call))
					...
					# Date of Posledný Attempt, Date of Posledný Non-Contact, Date of Posledný Contact, Date of Posledný Sale
	...
	echo 'Lead ' . $lead_id . ' bol zmenený pre ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue na to that action without printing output
		$stage = 0;
		$status = $sub_status;
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-READY &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-Incall &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-pozastavené &nbsp;\n";
	if (preg_match("/NOT-LOGGED-IN-AGENTS/",$agent_status_viewable_groups))
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		{
		echo "Hovorov v zobrazení frontu Zakázané pre túto kampaň\n";
		exit;
		}
	else
		{
		...
		### grab the basic data na calls in the queue for this agent
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefón &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; meno a priezvisko &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; počkajte &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; zástupca &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; skupiny volajúcich &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; typ &nbsp; </font></TD>";
		echo "</TR>";

		### Print call information and gather more info na the calls as they are printed
		$loop_count=0;
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">TAKE HOVOR</a> &nbsp; </TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<a href=\"#\" onclick=\"VieWCalLLoG('','form');return false;\">Prejsť</a> &nbsp;  &nbsp; &nbsp; ";
	echo "<a href=\"#\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> zavrieť </a>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
	echo "<TR>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> &nbsp; # &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Dátum\/čas &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; dĺžka &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STAV &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefón &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; meno a priezvisko &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; kampaň &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; dovnútra a von &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; striedavý &nbsp; </font></TD>";
	...
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; VOLAŤ &nbsp; </font></TD>";
	echo "</TR>";
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>Žiadne vyzýva tento deň</td></tr>";}
	...
	while ($g > $u) 
		...
		if ($manual_dial_filter > 0)
			{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> VOLAŤ </A> </td>\n";}
		else
			{echo "<td align=right><font size=2> VOLAŤ </td>\n";}
		echo "</tr>\n";
		}

	echo "</TABLE>";
	echo "<BR>";
	echo "<a href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">Zavrieť Call Log</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			{
			echo "Chyba: musíte zadať hľadané výrazy, musí byť jeden z nich je obývaných: olovo id, zákaznícke, telefónne číslo, priezvisko\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Vrátiť späť</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<CENTER>\n";
			echo "<font style=\"font-size:14px;font-family:sans-serif;\"><B>";
			echo "Nájdené: $search_result_count";
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			echo "<TR>";
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefón &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STAV &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Posledná výzva &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; město &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; stav &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; zips &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; info &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; VOLAŤ &nbsp; </font></TD>";
			echo "</TR>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr bgcolor=white><td colspan=10 align=center>Neboli nájdené žiadne výsledky</td></tr>";}
				...
				while ($g > $u) 
					...
					if ($inbound_lead_search < 1)
						{
						if ($manual_dial_filter > 0)
							{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> VOLAŤ </A> </td>\n";}
						else
							{echo "<td align=right><font size=2> VOLAŤ </td>\n";}
						}
					else
						{
						echo "<td align=right><font size=2> <a href=\"#\" onclick=\"LeaDSearcHSelecT('$ALLlead_id[$i]');return false;\">Vyberte</A> </td>\n";
						}
					echo "</tr>\n";
					}
				...
				$stmtL="UPDATE vicidial_lead_search_log set sekúnd='$search_seconds' where search_log_id='$search_log_id';";
				if ($DB) {echo "|$stmtL|\n";}
				$rslt=mysql_to_mysqli($stmtL, $link);
			...
			{
			echo "Chyba: tam bol problém s svoje vyhľadávacie termíny\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Vrátiť späť</a>";
			echo "</CENTER>";
			exit;
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			{
			echo "Chyba: musíte zadať hľadané výrazy, musí byť jeden z nich sa vyplní: kancelária číslo, priezvisko, meno\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">Vrátiť späť</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0)
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; posledný NAME &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					echo "</tr>\n";
					echo "<tr $bgcolor>";
					echo "<td colspan=8><font size=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <B>Dept:</B> $ALLdepartment[$i] &nbsp; &nbsp; &nbsp; <B>Group:</B> $ALLgroup_name[$i] &nbsp; &nbsp; &nbsp; <B>Job:</B> $ALLjob_title[$i] &nbsp; &nbsp; &nbsp; <B>umiestnenie:</B> $ALLlocation[$i]</td>";
					echo "</tr>\n";
					}
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Spätné volanie Stav: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Spätné volanie Lead stavu: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Spätné volanie Vstup čas: &nbsp; </td><td ALIGN=left><font size=2>$row[1]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Spätné volanie oneskorenie: &nbsp; </td><td ALIGN=left><font size=2>$row[2]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Komentáre spätné volanie: &nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Stav: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>časové pásmo: &nbsp; </td><td ALIGN=left><font size=2>$row[3]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>volal od posledného vynulovania: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> VOLAŤ </a>";}
					else
						{$INFOout .= " VOLAŤ ";}
					}
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>vytočiť odkaz: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> VOLAŤ </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>vytočiť odkaz: &nbsp; </td><td ALIGN=left><font size=2> VOLAŤ ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right colspan=2><font size=2><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">vyberte túto vedenie</a></td></tr>";}
			...
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\"> VOLAŤ </a>";}
					else
						{$INFOout .= " VOLAŤ ";}
					}
				}
			...
			if ($hide_call_log_info=='N')
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>počet hovorov: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";}
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>posledný miestnej pohotovosti: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			{
			if ($search != 'logfirst')
				{$NOTESout .= "<CENTER>VÝZVA PRE LOG tohto kábla:<br>\n";}
			...
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Dátum\/čas &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; zástupca &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; dĺžka &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STAV &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefón &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; kampaň &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; dovnútra a von &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; striedavý &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; hangup &nbsp; </font></TD>";
		#	$NOTESout .= "</TR><TR>";
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; meno a priezvisko &nbsp; </font></TD>";
			$NOTESout .= "</TR>";
			...
			if ($g > 0)
				...
				{$NOTESout .= "<tr bgcolor=white><td colspan=11 align=center>Žiadne volanie nájdené</td></tr>";}
		...
		if ($allow_emails>0)
			{
			$NOTESout .= "<CENTER>email Prihlásiť sa k tomuto vedeniu:<br>\n";
			$NOTESout .= "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> # </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Dátum\/čas </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; zástupca </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; kampaň </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; e-mail </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Príslušenstvo </B></font></td>";
			$NOTESout .= "</tr>\n";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (preg_match('/NONE/i',$view_calls_in_queue)) or (preg_match('/N/i',$grab_calls_in_queue)) )
		{
		echo "ERROR: Hovorov v zobrazení frontu Zakázané pre túto kampaň\n";
		exit;
		}
	else
		{
...
function hangup_cause_description($code)
	...
	else { return "Neznáma príčina hangup kód."; }
	}
