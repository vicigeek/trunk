...
if ($function == 'version')
	{
	$data = "VERSION: $version|CONSTRUCTION: $build|DATE: $NOW_TIME|EPOCH: $StarTtime";
	$result = 'SUCCESS';
	echo "$data\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if(strlen($source)<2)
		...
		if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0) or ($auth_api==0))
			{
			$result = 'ERROR';
			$result_reason = "Login ou mot de passe invalide";
...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- VERSION: $version     CONSTRUCTION: $build    USER: $user\n";
