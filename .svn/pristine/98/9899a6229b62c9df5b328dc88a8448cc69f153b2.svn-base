...
if ($INVIA && $old_user && $new_user && $confirm_transfer) {
...
if ($INVIA && $old_user && $new_user && !$confirm_transfer) 
	...
	echo "<a href='$PHP_SELF?DB=$DB&old_user=$old_user&new_user=$new_user&confirm_transfer=1&INVIA=1'>CLICK TO CONFIRM</a><BR><BR>";
	...
	if ($DB) {echo "$stmt\n";}
	# <tr><td align='right'>
	echo "Agenti with callbacks:<BR><select name='old_user' size=5>\n";
	...
	echo "</select>";
	echo "<BR/><BR/><input type='submit' name='INVIA' value='  TRANSFER TO '><BR/><BR/>";
