...
if ($format=='debug')
	...
	echo "<title>Conferentie Extentie Controle";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'refresh')
	...
	if (strlen($conf_exten)<1)
		...
		#	$AexternalDEAD=$row[0];
			...
			##### BEGIN check op calls in queue, number of active calls in the campaign
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03007',$user,$server_ip,$session_name,$one_mysql_log);}
				$row=mysqli_fetch_row($rslt);
				$RingCalls=$row[0];
				if ($RingCalls > 0) {$RingCalls = "<font class=\"queue_text_red\">Klanten in de Wachtrij: $RingCalls</font>";}
				else {$RingCalls = "<font class=\"queue_text\">Klanten in de Wachtrij: $RingCalls</font>";}
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03008',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			##### END check op calls in queue, number of active calls in the campaign
