...
$report_name = 'Listas Pass Report';
$db_source = 'M';
...
if ($use_lists > 0)
	{
	$MAIN.="</TD><TD VALIGN=TOP> Listas:<BR>";
	...
	$MAIN.="</SELECT>\n<BR>\n";
	$MAIN.="<a href=\"$PHP_SELF?use_lists=0&DB=$DB\">SWITCH TO CAMPAÑAS</a>";
	}
else
	{
	$MAIN.="</TD><TD VALIGN=TOP> Campañas:<BR>";
	$MAIN.="<SELECT SIZE=5 NAME=group[] multiple>\n";
	if  (preg_match('/\-\-ALL\-\-/',$group_string))
		{$MAIN.="<option value=\"--ALL--\" selected>-- ALL CAMPAÑAS --</option>\n";}
	else
		{$MAIN.="<option value=\"--ALL--\">-- ALL CAMPAÑAS --</option>\n";}
	$o=0;
	while ($campaigns_to_print > $o)
		{
		if (preg_match("/$groups[$o]\|/i",$group_string)) {$MAIN.="<option selected value=\"$groups[$o]\">$groups[$o] - $group_names[$o]</option>\n";}
...
if (strlen($group[0]) > 1)
	{
	$MAIN.=" <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">MODIFICAR</a> | \n";
	$MAIN.=" <a href=\"./admin.php?ADD=999999\">INFORMES</a> </FONT>\n";
	}
else
	{
...
if (strlen($group[0]) < 1)
	{
	$MAIN.="\n\n";
	$MAIN.="POR FAVOR SELECCIONE ARRIBA UNA CAMPAÑA Y FECHA ARRIBA Y CLICK EN REMITIR\n";
	}

else
	{
	$OUToutput = '';
	$OUToutput .= "Listas Pass Report                             $NOW_TIME\n";

	$OUToutput .= "\n";

	##############################
	#########  LISTA  ID BREAKDOWN STATS

	$TOTALleads = 0;

	$OUToutput .= "\n";
	$OUToutput .= "---------- LISTA  ID SUMMARY     <a href=\"$PHP_SELF?DB=$DB$groupQS&REMITIR=$REMITIR&file_download=1\">DESCARGAR</a>\n";
	...
	$OUToutput .= "|  LOAD DATE | LISTA  ID and NAME                         | CAMPAIGN | COUNT      | ACTIVE   |";
	...
	$GRAPH="</PRE><table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"white\" summary=\"LISTA  ID Summary\" class=\"horizontalgraph\">\n";
	$GRAPH.="<caption align='top'>LISTA  ID SUMMARY</caption>";
	$GRAPH.="<tr>\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>LISTA  ID SUMMARY</caption><tr><th class='thgraph' scope='col'>LISTS</th>";
