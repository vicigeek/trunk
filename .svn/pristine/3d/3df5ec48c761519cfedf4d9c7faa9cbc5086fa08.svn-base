...
if ($function == 'version')
	{
	$data = "VERSIÓN: $version|CONSTRUCCION: $build|DATE: $NOW_TIME|EPOCH: $StarTtime";
	$result = 'SUCCESS';
	echo "$data\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if(strlen($source)<2)
		...
		if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0) or ($auth_api==0))
			{
			$result = 'ERROR';
			$result_reason = "Nombre y contraseña inválidos del usuario";
...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- VERSIÓN: $version     CONSTRUCCION: $build    USER: $user\n";
