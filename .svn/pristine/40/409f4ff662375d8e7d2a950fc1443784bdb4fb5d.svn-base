...
<title>ADMINISTRATION: Τροποποίηση οδηγού</title>
...
<?php 
echo "<a href=\"./admin.php?ADD=100\">ADMINISTRATION</a>: Τροποποίηση οδηγού<BR>\n";
...
if ($lead_id == 'NEW')
	...
	if ($affected_rows > 0)
		...
		echo "Ο μόλυβδος έχει προστεθεί: $lead_id<BR><BR>\n";
		$end_call=0;
		}
	else
		{echo "ΣΦΑΛΜΑ: Μόλυβδος δεν έχουν προστεθεί, παρακαλώ πηγαίνετε πίσω και να δούμε τι έχετε εισαγάγει<BR><BR>\n";}
	}
else
	{
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	if ($LOGadmin_hide_phone_data != '0')
		{
		if ($DB > 0) {echo "ΑΠΟΚΡΥΨΗPHONEDATA|$row[11]|$LOGadmin_hide_phone_data|\n";}
		$phone_temp = $row[11];
	...
	if ($LOGadmin_hide_lead_data != '0')
		{
		if ($DB > 0) {echo "ΑΠΟΚΡΥΨΗLEADDATA|$row[5]|$row[6]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";}
		if (strlen($row[5]) > 0)
	...
	if ($lead_id == 'NEW')
		{echo "<br><b>Προσθέστε μια νέα πρωτοβουλία πρωτοποριακής</B>\n";}
	else
		{echo "<br>Πληροφορίες Κλήσης: $first_name $last_name - $phone_number\n";}
	...
	echo "<tr><td colspan=2>Lead ID: $lead_id &nbsp; &nbsp; ID Λίστας: $list_id</td></tr>\n";
	...
	if ($LOGadmin_hide_lead_data != '0')
		...
		echo "<tr><td align=right>Χώρα    : </td><td align=left>$country_code</td></tr>\n";
		...
		echo "<tr><td align=right>Χώρα    : </td><td align=left><input type=text name=country_code size=3 maxlength=3 value=\"".htmlentities($country_code)."\"></td></tr>\n";
	...
	if ($lead_id != 'NEW') 
		...
		echo "Callback Λεπτομέρειεςs:<BR><CENTER>\n";
		if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') or ($scheduled_callback == 'Y') )
			...
			if ($CB_to_print>0)
				...
				if ($rowx[9] == 'USERONLY')
					...
					echo "<input type=hidden name=lead_id value=\"$lead_id\">\n";
					echo "<input type=hidden name=callback_id value=\"$rowx[0]\">\n";
					echo "New Callback Owner ΧρήστηςID: <input type=text name=CBuser size=18 maxlength=20 value=\"$rowx[8]\"> \n";
				...
				<TR BGCOLOR="#E6E6E6">
				<TD align=center colspan=2>
				<FONT FACE="ARIAL,HELVETICA">Σχόλια: 
				...
				<input type=button value="ΕΠΙΒΕΒΑΙΩΣΗ" name=smt id=smt onClick="submit_form()">
				</TD>
				</TR>
			...
			{
			echo "<BR>Αν θέλετε να αλλάξετε αυτό οδήγησε σε μια προγραμματισμένη επανάκλησης, πρώτα να αλλάξετε την διάθεση να CBHOLD, τότε υποβάλλουν και θα είστε σε θέση να καθορίσει την ημερομηνία και την ώρα επανάκλησης.<BR>\n";
			}
		echo "</TD></TR></TABLE>\n";
		...
		if ($c > 0)
			{
			echo "<B>EXTENDED ALTERNATE ΤΗΛΕΦΩΝΑ ΓΙΑ ΤΟ ΠΑΡΟΝ ΜΟΛΥΒΔΟΥ:</B>\n";
		...
		if ($custom_fields_enabled > 0)
			...
			$stmt="ΕΜΦΑΝΙΣΗ ΠΙΝΑΚΑΣS LIKE \"custom_$CLlist_id\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_to_mysqli($stmt, $link);
			...
			if ($tablecount_to_print > 0) 
				...
				if ($fieldscount_to_print > 0) 
					...
					echo "<B>Προσαρμοσμένα Πεδία ΓΙΑ ΑΥΤΟ ΤΟ ΜΟΛΥΒΔΟ:</B><BR>\n";
		...
		echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=left><font size=2>LENGTH</td><td align=left><font size=2> STATUS</td><td align=left><font size=2> TSR</td><td align=right><font size=2>ΕΚΣΤΡΑΤΕΙΑ</td><td align=right><font size=2> LIST</td><td align=right><font size=2> LEAD</td><td align=right><font size=2> HANGUP REASON</td><td align=right><font size=2> PHONE</td></tr>\n";
		...
		echo "<tr><td><font size=1># </td><td align=left><font size=2>ΕΚΣΤΡΑΤΕΙΑ</td><td><font size=2>DATE/TIME </td><td align=left><font size=2>CALL MENU </td><td align=left><font size=2> &nbsp; ACTION</td></tr>\n";
	...
	##### vicidial agent outbound calls for this time period #####
		...
		if ($allow_emails>0) 
			...
			echo "<TABLE width=750 cellspacing=1 cellpadding=1>\n";
			echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=right><font size=2> USER</td><td align=right><font size=2>ΕΚΣΤΡΑΤΕΙΑ</td><td align=left><font size=2>EMAIL TO</td><td align=left><font size=2> MESSAGE</td><td align=right><font size=2> ATTACHMENTS</td></tr>\n";
		...
		if ($admin_display > 0)
			{
			echo "<a href=\"./admin.php?ADD=720000000000000&stage=$lead_id&category=LEADS\">Click here to see Lead Modifychanges to this lead</a>\n";
			}
                //Εμφάνιση link to QC if user has QC permissions
