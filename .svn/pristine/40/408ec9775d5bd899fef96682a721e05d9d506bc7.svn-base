...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		if ($commit == 'YES')
			...
			if ($referrer=='agent') 
				{$BACKlink = "<A HREF=\"./vicidial.php?pl=$phone_login&pp=$phone_pass&VD_login=$user\"><font color=\"#003333\">BACK TO Agent Início de uma sessão Screen</font></A>";}
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">Retornar para Administração</font></A>";}
			if ($referrer=='welcome') 
		...
		if ($status=='LOGIN')
			...
			$log_action = 'logout';
			$button_name = 'LOGOUT';
			$LOGtimeMESSAGE = "Você autenticado-nos em: $last_action_date<BR>Quantidade de tempo que tem sido registrado em:$totTIME_HMS<BR><BR>LOGOUT Clique abaixo para log-out";
			}
