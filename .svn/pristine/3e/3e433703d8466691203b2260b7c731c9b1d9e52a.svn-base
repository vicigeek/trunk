...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<TABLE><TR><TD></TD>\n";
	echo "<!-- ILPA -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- ВЕРСИЯ: $version     СБОРКА: $build      ADD: $ADD-->\n";
...
// ################################################################################
// ACTIVE EXTENSIONS LIST ОБНОВИТЬ FUNCTIONS
...
// ################################################################################
// BUSY TRUNK LIST ОБНОВИТЬ FUNCTIONS
...
// ################################################################################
// BUSY EXTENSIONS LIST ОБНОВИТЬ FUNCTIONS
...
// ################################################################################
// Check to see if user's extension is на a call right now, if it is, format and print it
	...
	//		alert(favorites_listEDIT);
		...
		while (loop_ct < favlistCT)
			...
			if (favorites_listEDIT.match(regFAVlist))
				{
				// do not print extension, it's на the favorites list
				}
			else
				{
...
<span style="position:absolute;left:0px;top:12px;z-index:29;" id="TrunkHangupBox">
    <table border=1 bgcolor="#CDE0C2" width=600 height=500><TR><TD> TRUNK зависание <BR><BR>
