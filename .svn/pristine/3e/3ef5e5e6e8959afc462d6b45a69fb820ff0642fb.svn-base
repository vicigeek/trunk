...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		echo"<HTML><HEAD>\n";
		echo"<TITLE>Agent Reloj de Control de Asistencia</TITLE>\n";
		...
		echo "<TD ALIGN=CENTER VALIGN=MIDDLE><B> Reloj de Control de Asistencia </B></TD>";
		echo "</TR>\n";
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "Usted ahora tiene registros";
				$LOGtimeMESSAGE = "Está conectado a $NOW_TIME";
			...
			if ( ($status=='LOGIN') and ($stage=='logout') )
				{
				$VDdisplayMESSAGE = "Ahora ha cerrado su sesión";
				$LOGtimeMESSAGE = "Cerró su sesión en$NOW_TIME<BR>El tiempo durante el cual tuvo su sesión abierta:$totTIME_HMS";
			...
			if ( ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='logout') ) or ( ($status=='LOGIN') and ($stage=='login') ) )
				{echo "ERROR: ya se ha registrado en el Reloj de Control de Asistencia:$status|$stage";  exit;}
		...
		if ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') )
			...
			$log_action = 'login';
			$button_name = 'LOGIN';
			$LOGtimeMESSAGE = "Usted cerró sesión por última vez en:$last_action_date<BR><BR>Click en INICIAR SESIÓN para acceder al sistema";
			}
		if ($status=='LOGIN')
			{
			...
			$log_action = 'logout';
			$button_name = 'LOGOUT';
			$LOGtimeMESSAGE = "Inició sesión en: $last_action_date<BR>Tiempo transcurrido desde que inició sesión:$totTIME_HMS<BR><BR>Haga click abajo en CERRAR SESIÓN para salir del sistema";
			}
