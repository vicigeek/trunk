...
if ($user_territories_active < 1)
	{
	echo "ERROR: User Gebiete sind nicht aktiv auf diesem System\n";
	exit;
	}
...
if( (strlen($USER)<2) or (strlen($PASS)<2) or (!$auth) or (!$modify_users_auth))
	...
	echo "Unzulässiges Username/Kennwort:|$USER|$PASS|$auth_message|\n";
	exit;
	}
...
if ( ($action == "CHANGE_TERRITORY_OWNER_ACCOUNT") and ($enable_vtiger_integration > 0) )
	...
	echo "<br>Vtiger ändern Territory Besitzer<form action=$PHP_SELF method=POST>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>New Besitzer:</td><td align=left><select size=1 name=user>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Update ViciDial List Besitzer:</td><td align=left><select size=1 name=vl_owner>\n";
	...
	if ( (strlen($accountid)<1) or (strlen($user)<1) )
		...
		if ($vat_ct > 0)
			...
			if ($row[0] < 1)
				...
				echo "NOTICE: Had to user Gebiet hinzufügen: $user $territory<BR>\n";
			...
			if ($vtu_ct > 0)
				...
				echo "Vtiger Territory Besitzer gewechselt: $user $territory<BR>\n";
				echo "Geändert Records: $changed<BR>\n";
...
<TR BGCOLOR=#E6E6E6>
<TD ALIGN=LEFT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>">US-Territorien</a></TD>
<TD ALIGN=RIGHT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>?action=ADD_TERRITORY">Add Territory</a></TD>
<TD ALIGN=RIGHT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>?action=ADD_USER_TERRITORY">Benutzer hinzufügen Territory</a></TD>
<?php
if ($enable_vtiger_integration > 0)
	{ ?>
...
if ( ($action == "PROCESS_CHANGE_TERRITORY_OWNER") and ($enable_vtiger_integration > 0) )
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		{
		echo "ERROR: Territorium und Benutzer müssen ausgefüllt werden<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($vtu_ct > 0)
			...
			echo "Vtiger Territory Besitzer gewechselt: $user $territory &nbsp; &nbsp; &nbsp; Geändert Records: $changed - $Achanged - $Cchanged - $Vchanged<BR>\n";
...
if ($action == "ADD_USER_TERRITORY")
	...
	echo "<br>Benutzer hinzufügen Territory<form action=$PHP_SELF method=POST>\n";
...
if ($action == "PROCESS_ADD_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		{
		echo "ERROR: Territory, Benutzer-und Level müssen ausgefüllt werden<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($row[0] > 0)
			{
			echo "ERROR: dieses Gebiet Benutzer ist bereits im System<BR>\n";
			}
		else
			{
...
if ($action == "PROCESS_MODIFY_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		...
		if ($row[0] < 0)
			{
			echo "ERROR: dieses Gebiet Benutzer ist nicht im System<BR>\n";
			}
		else
			{
			...
			echo "User Territory Geändert: $user $territory<BR>\n";
...
if ($action == "DELETE_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		...
		if ($row[0] < 0)
			...
			echo "User Territory entfällt: $user $territory<BR>\n";
...
if ($action == "ADD_TERRITORY")
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Territory Beschreibung: </td><td align=left><input type=text name=territory_description size=50 maxlength=255></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=SUBMIT value=SUBMIT></td></tr>\n";
	echo "</TABLE></center>\n";
	}
...
if ($action == "PROCESS_ADD_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		{
		echo "ERROR: Territorium und Territory Beschreibung müssen ausgefüllt werden<BR>\n";
		}
	else
		{
		...
		if ($row[0] > 0)
			{
			echo "ERROR: gibt es bereits ein Gebiet, in das System mit diesem Namen<BR>\n";
			}
		else
			{
...
if ($action == "DELETE_TERRITORY")
	...
	if (strlen($territory)<1)
		{
		echo "ERROR: Territory müssen ausgefüllt werden<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_territories where territory='$territory';";
		...
		if ($row[0] < 0)
			{
			echo "ERROR: Dieses Gebiet ist nicht in das System mit diesem Namen<BR>\n";
			}
		else
			{
			...
			echo "Gebiet entfällt: $territory<BR>\n";
...
if ($action == "PROCESS_MODIFY_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		...
		if ($row[0] < 1)
			...
			echo "Geändert Territory: $territory<BR>\n";
...
if ($action == "MODIFY_TERRITORY")
	...
	if ($territories_to_print > 0) 
		...
		echo "<br>Ändern Territory<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Territory Beschreibung: </td><td align=left><input type=text name=territory_description size=50 maxlength=255 value=\"$rowx[1]\"></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Bevollmächtigte: </td><td align=left><B>$user_count</B></td></tr>";
		...
		if ($enable_vtiger_integration > 0)
			...
			echo "<tr bgcolor=#B6D3FC><td align=right>Vtiger-Konten: </td><td align=left><B>$vtiger_count</B></td></tr>";
			}
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=SUBMIT value=SUBMIT></form></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "<BR><BR>\n";

		echo "<TABLE CELLPADDING=3 CELLSPACING=1 WIDTH=600><TR><TD COLSPAN=5><FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>Benutzer in this Territory:</TD></TR>\n";
		...
		#	echo "<TD>Accounts: $owner_count</TD>";
			...
			echo "</form>";
			echo "</TD>";
			echo "<TD><a href=\"$PHP_SELF?action=DELETE_USER_TERRITORY&territory=$territory&user=$Tuser[$o]\">LÖSCHUNG</a></TD>";
			echo "</TR>\n";
			$o++;
			}
		...
		echo "<a href=\"$PHP_SELF?action=DELETE_TERRITORY&territory=$territory\">Löschen Dieses Gebiet</a>\n";
		echo "<BR><BR>\n";
		}
	else
		{
		echo "ERROR: Gebiet nicht gefunden: $territory<BR>\n";
		}
	}
...
if ($action == "LIST_ALL_TERRITORIES")
	...
	echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=1 color=white>BESCHREIBUNG</B></TD>\n";
...
<BR><font size=1>User Territories &nbsp; &nbsp; VERSION: <?php echo $version ?> &nbsp; &nbsp; BAU: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
