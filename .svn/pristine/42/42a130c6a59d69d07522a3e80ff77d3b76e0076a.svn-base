...
28 => "Ongeldig number format (address incomplete).",
...
81 => "Ongeldig call reference value.",
88 => "Incompatible destination.",
95 => "Ongeldig message, unspecified.",
...
100 => "Ongeldig information element contents.",
101 => "Message not compatible with call state.",
102 => "Recovery op timer expiry.",
103 => "Parameter non-existent or not implemented - passed op (national use).",
...
438 => "Ongeldig Identity Header.",
470 => "Consent Needed.",
480 => "Temporarily Unavailable.",
481 => "Call/Transaction Does Not Exist.",
482 => "Loop Detected..",
483 => "Too Many Hops.",
484 => "Adres Incomplete.",
485 => "Ambiguous.",
486 => "Busy Here.",
...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- VERSIE: $version     BUILD: $build    USER: $user   server_ip: $server_ip-->\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( ( (preg_match("/START|ALL/",$shift_enforcement)) and (!preg_match("/OFF/",$VU_agent_shift_enforcement_override)) ) or (preg_match("/START|ALL/",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>FOUT: Er zijn geen shifts ingeschakeld voor je gebruikersgroep</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
		...
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=submit NAME=OPSLAAN VALUE=OPSLAAN></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		echo "<option value=\"\">-- Selecteer een Campagne --</option>\n";
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Groepskeuze $closer_choice is niet geldig\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Groepskeuze $closer_choice is geregistreerd voor gebruiker $user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories is niet geldig\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories is geregistreerd voor gebruiker $user\n";
	}
...
if ($ACTION == 'manDiaLnextCaLL')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
		...
		$channel_live=0;
		echo "HOPPER LEEG\n";
		echo "Conf Exten $conf_exten or campaign $campaign or ext_context $ext_context is niet geldig\n";
		exit;
		}
	else
		{
			...
			if (strlen($phone_number)>3)
				...
				if (preg_match("/ENABLED/",$manual_dial_call_time_check))
					...
					if ($dialable < 1)
						...
						echo "OUTSIDE OF LOCAL BEL TIME   $VMDQaffected_rows|$VLAEDaffected_rows\n";
						exit;
						}
					}
				...
				#	$DB=1;
					if (strlen($dial_statuses)>2)
						...
						if (preg_match("/UP laatste NAME/i",$lead_order)){$order_stmt = "order by last_name desc, $last_order";}
						if (preg_match("/DOWN laatste NAME/i",$lead_order)){$order_stmt = "order by last_name, $last_order";}
						...
						if (preg_match("/UP laatste gesprek TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
						if (preg_match("/DOWN laatste gesprek TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				{
				//Added by Poundteam for Audited Opmerkingen
				...
				//END Added by Poundteam for Audited Opmerkingen
				}
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "Telefoon en postcode tijdzone komen niet overeen! ";

					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY')
						{
						$post_phone_time_diff_alert_message='';
						if ($PHONEdialable < 1)
							{$post_phone_time_diff_alert_message .= " Telefoon landcode valt buiten de toegestane Tijdzone $PHONEgmt_offset ";}
						if ($POSTdialable < 1)
							{$post_phone_time_diff_alert_message .= " Postcode valt buiten de toegestande Tijdzone $POSTgmt_offset";}
						}
					}
			...
			#	$Local_persist = '/n';
				...
				### check for extension append in campagne
				$use_eac=0;
	...
	#				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
				...
				if (strlen($dial_ingroup) < 1)
					...
					if ($enable_queuemetrics_logging > 0)
						...
						# BELOUTBOUND (formerly ENTERQUEUE)
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "TELEFOON2 NUMMER STATUS NOT CHANGED\n";
		echo "$phone_number $stage $lead_id or $called_count is niet geldig\n";
		exit;
		}
	else
		{
		...
		echo "TELEFOON2 NUMMER STATUS CHANGED\n";
		}
}
...
if ($ACTION == 'manDiaLonly')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1) || (strlen($ext_context)<1) || (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		...
		### check for manual dial filter and extension append settings in campagne
		$use_eac=0;
		$use_custom_cid=0;
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		echo "LOG NIET INGEVOERD\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id or list_id: $list_id or phone_number: $phone_number or campaign: $campaign is niet geldig\n";
		exit;
		}
	else
		{
		...
		if ($affected_rows > 0)
			{
			echo "VICIDiaL_LOG Ingevoerd: $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
	...
	#	if ($affected_rows > 0)
	#		{
	#		echo "CALL_LOG Ingevoerd: $uniqueid|$channel|$NOW_TIME";
	#		}
	#	else
	#		{
	#		echo "LOG NIET INGEVOERD\n";
...
if ($stage == "end")
	...
	if ( (strlen($uniqueid)<1) or (strlen($lead_id)<1) )
		{
		echo "LOG NIET INGEVOERD\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id is niet geldig\n";
		$log_no_enter=1;
		}
	else
		{
...
if ($ACTION == 'VDADREcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || (strlen($lead_id)<1) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Campagne $campaign is niet geldig\n";
		echo "lead_id $lead_id is niet geldig\n";
		exit;
		}
	else
		{
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			if ( ($call_type=='OUT') or ($call_type=='OUTBALANCE') )
				...
				### if web form is set then send op to vicidial.php for override of WEB_FORM address
			...
			{
			echo "0\n";
		#	echo "No calls in QUEUE for $user op $server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups op vicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row op selected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Campagne $campaign is niet geldig or lead_id $lead_id is niet geldig\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user op $server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		{
		echo "NO\n";
		echo "campaign $campaign or conf_exten $conf_exten is niet geldig\n";
		exit;
		}
	else
		{
		...
		if ($no_delete_sessions < 1)
			{
			##### Remove the reservation op the vicidial_conferences meetme room
		...
		if ($vla_delete > 0) 
			...
			#	$affected_rows = mysqli_affected_rows($linkB);
				...
				if ($queuemetrics_addmember_enabled > 0)
					...
					### add the logged-in campagne as well
					$AMqueue[$i] = $campaign;
					$i++;
					$amq_conf_ct++;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id is niet geldig\n";
		exit;
		}
	else
		{
		...
		//Added by Poundteam Incorporated for Audit Opmerkingen Package
		require_once('audit_comments.php');
		...
		if ( (preg_match('/ACCTID/',$vtiger_search_category)) or (preg_match('/ACCOUNT/',$vtiger_search_category)) )
			...
			if ($VIDcount > 0)
				...
				if (preg_match('/Y/',$vtiger_status_call))
					...
					# Date of Laatste Attempt, Date of Laatste Non-Contact, Date of Laatste Contact, Date of Laatste Sale
	...
	echo 'Lead ' . $lead_id . ' is veranderd naar ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id is niet geldig\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if ( (strlen($stage)<2) || (strlen($server_ip)<1) )
		{
		echo "stage $stage is niet geldig\n";
		exit;
		}
	else
		{
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue op to that action without printing output
		$stage = 0;
		$status = $sub_status;
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list is niet geldig\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		{
		echo "agent_log_id $agent_log_id or pause_code $status is niet geldig\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-KLAAR &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-INCALL &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-PAUSED &nbsp;\n";
	if (preg_match("/NOT-LOGGED-IN-AGENTS/",$agent_status_viewable_groups))
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		{
		echo "Bellers in de Wachtrij bekijken is uitgeschakeld voor deze Campagne\n";
		exit;
		}
	else
		{
		...
		### grab the basic data op calls in the queue for this agent
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefoon &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; volledige naam &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; wachten &nbsp; </font></TD>";
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; bel groep &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; type &nbsp; </font></TD>";
		echo "</TR>";

		### Print call information and gather more info op the calls as they are printed
		$loop_count=0;
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">TAKE BEL</a> &nbsp; </TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<a href=\"#\" onclick=\"VieWCalLLoG('','form');return false;\">ga</a> &nbsp;  &nbsp; &nbsp; ";
	echo "<a href=\"#\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> sluiten </a>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
	echo "<TR>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> &nbsp; # &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; datum\/tijd &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; lengte &nbsp; </font></TD>";
	...
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; in en uit &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; afwisselend &nbsp; </font></TD>";
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>Geen gesprekken op deze dag</td></tr>";}
	...
	echo "</TABLE>";
	echo "<BR>";
	echo "<a href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">Sluit Logboek</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			{
			echo "Foutmelding: Een van deze zoekvelden moet gebruikt worden: Ares ID, Leverancier ID, Telefoon1, Achternaam\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Ga Terug</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<CENTER>\n";
			echo "<font style=\"font-size:14px;font-family:sans-serif;\"><B>";
			echo "Zoekresultaten: $search_result_count";
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			echo "<TR>";
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefoon &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STATUS &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; laatste gesprek &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; stad &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; staat &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ritssluiting &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; info &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; DIAL &nbsp; </font></TD>";
			echo "</TR>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr bgcolor=white><td colspan=10 align=center>Er zijn geen zoekresultaten</td></tr>";}
				...
				while ($g > $u) 
					...
					if ($inbound_lead_search < 1)
						...
						{
						echo "<td align=right><font size=2> <a href=\"#\" onclick=\"LeaDSearcHSelecT('$ALLlead_id[$i]');return false;\">Selecteer</A> </td>\n";
						}
					echo "</tr>\n";
					}
				...
				$stmtL="UPDATE vicidial_lead_search_log set seconden='$search_seconds' where search_log_id='$search_log_id';";
				if ($DB) {echo "|$stmtL|\n";}
				$rslt=mysql_to_mysqli($stmtL, $link);
			...
			{
			echo "Foutmelding: Er is een probleem met de opgegeven zoekwoorden\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Ga Terug</a>";
			echo "</CENTER>";
			exit;
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			{
			echo "Foutmelding: Een van deze zoekvelden moet gebruikt worden: Telefoon2, Achternaam, Voornaam\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">Ga Terug</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0)
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; laatste NAME &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					echo "</tr>\n";
					echo "<tr $bgcolor>";
					echo "<td colspan=8><font size=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <B>Dept:</B> $ALLdepartment[$i] &nbsp; &nbsp; &nbsp; <B>Group:</B> $ALLgroup_name[$i] &nbsp; &nbsp; &nbsp; <B>Job:</B> $ALLjob_title[$i] &nbsp; &nbsp; &nbsp; <B>locatie:</B> $ALLlocation[$i]</td>";
					echo "</tr>\n";
					}
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Terugbel Status: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Status Terugbelafspraak: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Tijdstip Terugbelafspraak Gemaakt: &nbsp; </td><td ALIGN=left><font size=2>$row[1]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Terugbel Deadline: &nbsp; </td><td ALIGN=left><font size=2>$row[2]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Notities: &nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>tijdzone: &nbsp; </td><td ALIGN=left><font size=2>$row[3]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>genoemd sinds de laatste reset: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>dial koppeling: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> DIAL </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>dial koppeling: &nbsp; </td><td ALIGN=left><font size=2> DIAL ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right colspan=2><font size=2><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">Selecteer deze klant</a></td></tr>";}
			...
			if ($hide_call_log_info=='N')
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Aantal keren gebeld: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";}
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Laatst gebelde tijd: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			{
			if ($search != 'logfirst')
				{$NOTESout .= "<CENTER>Gesprekslog voor deze klant:<br>\n";}
			...
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; datum\/tijd &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; agent &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; lengte &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STATUS &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; telefoon &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; campagne &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; in en uit &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; afwisselend &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; hangup &nbsp; </font></TD>";
		#	$NOTESout .= "</TR><TR>";
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; volledige naam &nbsp; </font></TD>";
			$NOTESout .= "</TR>";
			...
			if ($g > 0)
				...
				{$NOTESout .= "<tr bgcolor=white><td colspan=11 align=center>Geen gesprekken gevonden</td></tr>";}
		...
		if ($allow_emails>0)
			{
			$NOTESout .= "<CENTER>e-logboek voor deze lead:<br>\n";
			$NOTESout .= "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> # </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; datum\/tijd </B></font></td>";
			...
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; e-mail naar </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; toebehoren </B></font></td>";
			$NOTESout .= "</tr>\n";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (preg_match('/NONE/i',$view_calls_in_queue)) or (preg_match('/N/i',$grab_calls_in_queue)) )
		{
		echo "ERROR: Bellers in de Wachtrij bekijken is uitgeschakeld voor deze Campagne\n";
		exit;
		}
	else
		{
...
function hangup_cause_description($code)
	...
	else { return "Oorzaak ophangen onbekend."; }
	}
