...
if ($format=='debug')
	...
	echo "<title>Verificação Da Extensão De Conf";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'refresh')
	...
	if (strlen($conf_exten)<1)
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten é inválido\n";
		exit;
		}
	else
		{
		...
		#	$AexternalDEAD=$row[0];
			...
			##### BEGIN check em calls in queue, number of active calls in the campaign
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03008',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			##### END check em calls in queue, number of active calls in the campaign
...
if ($ACTION == 'register')
	...
	if ( (strlen($conf_exten)<1) || (strlen($exten)<1) )
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten é inválido or Exten $exten é inválido\n";
		exit;
		}
	else
		{
	...
	echo "Conferência $conf_exten foi registado a $exten\n";
	}
