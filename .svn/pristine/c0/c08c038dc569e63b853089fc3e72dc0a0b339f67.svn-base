...
28 => "無効 number format (address incomplete).",
29 => "Facilities rejected.",
30 => "Response to ステータス INQUIRY.",
...
81 => "無効 call reference value.",
88 => "Incompatible destination.",
95 => "無効 message, unspecified.",
...
100 => "無効 information element contents.",
101 => "Message not compatible with call state.",
102 => "Recovery オンtimer expiry.",
103 => "Parameter non-existent or not implemented - passed オン(national use).",
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
		{
		echo "無効 ユーザ名/パスワード: |$user|$pass|$auth_message|\n";
		exit;
		}
	else
		{
		if( (strlen($server_ip)<6) or (!isset($server_ip)) or ( (strlen($session_name)<12) or (!isset($session_name)) ) )
			{
			echo "無効 server_ip: |$server_ip|  or  無効 session_name: |$session_name|\n";
			exit;
			}
		else
			{
			...
			$SNauth=$row[0];
			  if($SNauth==0)
				{
				echo "無効 session_name: |$session_name|$server_ip|\n";
				exit;
				}
			  else
				{
...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- バージョン: $version     ビルド: $build    USER: $user   server_ip: $server_ip-->\n";
	echo "<title>VICIDiaL データベースクエリースクリプト";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		if ($cl_user_ct > 0)
			...
			### CHECK TO SEE IF エージェント IS LOGGED IN TO TIMECLOCK, IF NOT, OUTPUT ERROR
	...
	### CHECK TO SEE IF エージェント IS WITHIN THEIR SHIFT IF RESTRICTED, IF NOT, OUTPUT ERROR
	if ( ( (preg_match("/START|ALL/",$shift_enforcement)) and (!preg_match("/OFF/",$VU_agent_shift_enforcement_override)) ) or (preg_match("/START|ALL/",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>エラー：あなたのユーザグループに有効化されたシフトはありません。</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) )
				{
				$VDdisplayMESSAGE = "<B>エラー：あなたのシフトの外部にログインすることは許可されていません。</B>\n";
				$VDloginDISPLAY=1;
				}
			}
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=submit NAME=送信 VALUE=送信></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		echo "<option value=\"\">-- キャンペーンを選択してください。 --</option>\n";
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- 最初にタイムクロックにログインしなければなりません。 --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "グループ選択 $closer_choice 有効ではない\n";
		exit;
		}
	else
		{
	...
	echo "Closer In グループ選択 $closer_choice ユーザに登録されています $user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories 有効ではない\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories ユーザに登録されています $user\n";
	}
...
if ($ACTION == 'manDiaLnextCaLL')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
		...
		$channel_live=0;
		echo "ホッパーが空です\n";
		echo "Conf Exten $conf_exten or campaign $campaign or ext_context $ext_context 有効ではない\n";
		exit;
		}
	else
		{
			...
			if (strlen($phone_number)>3)
				...
				if (preg_match("/ENABLED/",$manual_dial_call_time_check))
					...
					if ($dialable < 1)
						...
						echo "OUTSIDE OF LOCAL 電話 TIME   $VMDQaffected_rows|$VLAEDaffected_rows\n";
						exit;
						}
					}
				...
				#	$DB=1;
					if (strlen($dial_statuses)>2)
						...
						if (preg_match("/UP 最後 NAME/i",$lead_order)){$order_stmt = "order by last_name desc, $last_order";}
						if (preg_match("/DOWN 最後 NAME/i",$lead_order)){$order_stmt = "order by last_name, $last_order";}
						...
						if (preg_match("/UP ラストコール TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
						if (preg_match("/DOWN ラストコール TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				{
				//Added by Poundteam for Audited コメント
				...
				//END Added by Poundteam for Audited コメント
				}
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "電話や郵便番号のタイムゾーンの不一致! ";

					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY')
						{
						$post_phone_time_diff_alert_message='';
						if ($PHONEdialable < 1)
							{$post_phone_time_diff_alert_message .= " ダイヤルできるゾーン以外の電話エリアコード $PHONEgmt_offset ";}
						if ($POSTdialable < 1)
							{$post_phone_time_diff_alert_message .= " ダイヤルできるゾーン以外の郵便番号 $POSTgmt_offset";}
						}
					}
			...
			#	$Local_persist = '/n';
				...
				### check for extension append キャンペーンの
				$use_eac=0;
	...
	#				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
				...
				if (strlen($dial_ingroup) < 1)
					...
					if ($enable_queuemetrics_logging > 0)
						...
						# 電話OUTBOUND (formerly ENTERQUEUE)
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "代替電話 番号 ステータス NOT CHANGED\n";
		echo "$phone_number $stage $lead_id or $called_count 有効ではない\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'manDiaLonly')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1) || (strlen($ext_context)<1) || (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		...
		### check for manual dial filter and extension append settings キャンペーンの
		$use_eac=0;
		$use_custom_cid=0;
...
if ($ACTION == 'manDiaLlookCaLL')
	...
	if (strlen($MDnextCID)<18)
		{
		echo "NO\n";
		echo "MDnextCID $MDnextCID 有効ではない\n";
		exit;
		}
	else
		{
		...
		if ($VM_mancall_ct > 0)
			...
			{
			### after 10 秒, start checking for call termination in the carrier log
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		echo "ログが入力されていません\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id or list_id: $list_id or phone_number: $phone_number or campaign: $campaign 有効ではない\n";
		exit;
		}
	else
		{
		...
		if ($affected_rows > 0)
			{
			echo "VICIDiaL_LOG 挿入済み: $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
	...
	#	if ($affected_rows > 0)
	#		{
	#		echo "CALL_LOG 挿入済み: $uniqueid|$channel|$NOW_TIME";
	#		}
	#	else
	#		{
	#		echo "ログが入力されていません\n";
...
if ($stage == "end")
	...
	if ( (strlen($uniqueid)<1) or (strlen($lead_id)<1) )
		{
		echo "ログが入力されていません\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id 有効ではない\n";
		$log_no_enter=1;
		}
	else
		{
...
if ($ACTION == 'VDADREcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || (strlen($lead_id)<1) )
		...
		$channel_live=0;
		echo "0\n";
		echo "キャンペーン $campaign 有効ではない\n";
		echo "lead_id $lead_id 有効ではない\n";
		exit;
		}
	else
		{
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			if ( ($call_type=='OUT') or ($call_type=='OUTBALANCE') )
				...
				### if web form is set then send オンto vicidial.php for override of WEB_FORM address
			...
			{
			echo "0\n";
		#	echo "No calls in QUEUE for $user オン$server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups オンvicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row オンselected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "キャンペーン $campaign 有効ではない or lead_id $lead_id 有効ではない\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user オン$server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		{
		echo "NO\n";
		echo "campaign $campaign or conf_exten $conf_exten 有効ではない\n";
		exit;
		}
	else
		{
		...
		if ($no_delete_sessions < 1)
			{
			##### Remove the reservation オンthe vicidial_conferences meetme room
		...
		if ($vla_delete > 0) 
			...
			#	$affected_rows = mysqli_affected_rows($linkB);
				...
				if ($queuemetrics_addmember_enabled > 0)
					...
					### add the logged-キャンペーンの as well
					$AMqueue[$i] = $campaign;
					$i++;
					$amq_conf_ct++;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id 有効ではない\n";
		exit;
		}
	else
		{
		...
		//Added by Poundteam Incorporated for Audit コメント Package
		require_once('audit_comments.php');
		audit_comments($lead_id,$list_id,$format,$user,$mel,$NOW_TIME,$link,$server_ip,$session_name,$one_mysql_log,$campaign);

		// JOEJ - メール feature - may not be necessary if vicidial_email_list doesn't need a status column.
		if ($email_enabled>0) 
			{
		...
		if ( (preg_match('/ACCTID/',$vtiger_search_category)) or (preg_match('/ACCOUNT/',$vtiger_search_category)) )
			...
			if ($VIDcount > 0)
				...
				if (preg_match('/Y/',$vtiger_status_call))
					...
					# Date of 最後 Attempt, Date of 最後 Non-Contact, Date of 最後 Contact, Date of 最後 Sale
	...
	echo 'Lead ' . $lead_id . ' 変更されています ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id 有効ではない\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue オンto that action without printing output
		$stage = 0;
		$status = $sub_status;
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list 有効ではない\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		{
		echo "agent_log_id $agent_log_id or pause_code $status 有効ではない\n";
		exit;
		}
	else
		{
	...
	echo ' 一時停止Code ' . $status . " has been recorded\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-準備ができている &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-インコール &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-一時停止 &nbsp;\n";
	if (preg_match("/NOT-LOGGED-IN-AGENTS/",$agent_status_viewable_groups))
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		{
		echo "このキャンペーンに対し無効になっている列になっている電話ビュー\n";
		exit;
		}
	else
		{
		...
		### grab the basic data オンcalls in the queue for this agent
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 電話 &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; フルネーム &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 待ち時間 &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; エージェント &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; グループを呼び出す &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; タイプを呼び出す &nbsp; </font></TD>";
		echo "</TR>";

		### Print call information and gather more info オンthe calls as they are printed
		$loop_count=0;
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">TAKE 電話</a> &nbsp; </TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<a href=\"#\" onclick=\"VieWCalLLoG('','form');return false;\">行く</a> &nbsp;  &nbsp; &nbsp; ";
	echo "<a href=\"#\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> 閉じる</a>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
	echo "<TR>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> &nbsp; # &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 日付\/時刻 &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 長さ &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ステータス &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 電話 &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; フルネーム &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; キャンペーン &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; イン\/アウト &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 代替の &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ハングアップ &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 情報 &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ダイアル &nbsp; </font></TD>";
	echo "</TR>";
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>この日には電話がありません。</td></tr>";}
	...
	while ($g > $u) 
		...
		echo "<td align=right><font size=2> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i]);return false;\"> 情報 </A> </td>\n";
		if ($manual_dial_filter > 0)
			{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ダイアル </A> </td>\n";}
		else
			{echo "<td align=right><font size=2> ダイアル </td>\n";}
		echo "</tr>\n";
		}

	echo "</TABLE>";
	echo "<BR>";
	echo "<a href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">コールログを閉じる</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			{
			echo "エラー：あなたは、検索語に入力する必要があり、これらのいずれかが居住しなければなりません：リードID、ベンダーID、電話番号、姓\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">戻る</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<CENTER>\n";
			echo "<font style=\"font-size:14px;font-family:sans-serif;\"><B>";
			echo "結果が見つかりました。 $search_result_count";
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			echo "<TR>";
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 電話 &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ステータス &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ラストコール &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 都市 &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 状態 &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 郵便番号 &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 情報 &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ダイアル &nbsp; </font></TD>";
			echo "</TR>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr bgcolor=white><td colspan=10 align=center>結果が見つかりません。</td></tr>";}
				...
				while ($g > $u) 
					...
					echo "<td align=right><font size=2> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i],'','$inbound_lead_search');return false;\"> 情報 </A> </td>\n";
					if ($inbound_lead_search < 1)
						{
						if ($manual_dial_filter > 0)
							{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ダイアル </A> </td>\n";}
						else
							{echo "<td align=right><font size=2> ダイアル </td>\n";}
						}
					else
						{
						echo "<td align=right><font size=2> <a href=\"#\" onclick=\"LeaDSearcHSelecT('$ALLlead_id[$i]');return false;\">選択する</A> </td>\n";
						}
					echo "</tr>\n";
					}
				...
				$stmtL="UPDATE vicidial_lead_search_log set 秒='$search_seconds' where search_log_id='$search_log_id';";
				if ($DB) {echo "|$stmtL|\n";}
				$rslt=mysql_to_mysqli($stmtL, $link);
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			{
			echo "エラー：あなたは、検索語に入力する必要があり、これらのいずれかが移入されている必要がありますオフィス番号、姓、名\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">戻る</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0)
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 最後 NAME &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					echo "</tr>\n";
					echo "<tr $bgcolor>";
					echo "<td colspan=8><font size=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <B>Dept:</B> $ALLdepartment[$i] &nbsp; &nbsp; &nbsp; <B>Group:</B> $ALLgroup_name[$i] &nbsp; &nbsp; &nbsp; <B>Job:</B> $ALLjob_title[$i] &nbsp; &nbsp; &nbsp; <B>場所：</B> $ALLlocation[$i]</td>";
					echo "</tr>\n";
					}
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>コールバックステータス: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>コールバック見込み客ステータス: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>コールバックエントリー時間: &nbsp; </td><td ALIGN=left><font size=2>$row[1]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>コールバックトリガー時間: &nbsp; </td><td ALIGN=left><font size=2>$row[2]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>コールバックコメント: &nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>ステータス: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>タイムゾーン: &nbsp; </td><td ALIGN=left><font size=2>$row[3]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>最後のリセット以降に呼び出さ: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ダイアル </a>";}
					else
						{$INFOout .= " ダイアル ";}
					}
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>リンクをダイヤル: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ダイアル </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>リンクをダイヤル: &nbsp; </td><td ALIGN=left><font size=2> ダイアル ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right colspan=2><font size=2><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">このリードを選択する</a></td></tr>";}
			...
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\"> ダイアル </a>";}
					else
						{$INFOout .= " ダイアル ";}
					}
				}
			...
			if ($hide_call_log_info=='N')
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>カウントと呼ばれる: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";}
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>最後のローカル通話時間: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			{
			if ($search != 'logfirst')
				{$NOTESout .= "<CENTER>この見込み客に対する電話ログ:<br>\n";}
			...
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 日付\/時刻 &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; エージェント &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 長さ &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ステータス &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 電話 &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; キャンペーン &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; イン\/アウト &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 代替の &nbsp; </font></TD>";
			$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ハングアップ &nbsp; </font></TD>";
		#	$NOTESout .= "</TR><TR>";
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; フルネーム &nbsp; </font></TD>";
			$NOTESout .= "</TR>";
			...
			if ($g > 0)
				...
				{$NOTESout .= "<tr bgcolor=white><td colspan=11 align=center>電話がありません。</td></tr>";}
		...
		if ($allow_emails>0)
			{
			$NOTESout .= "<CENTER>この端子の電子メールログ:<br>\n";
			$NOTESout .= "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> # </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 日付\/時刻 </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; エージェント </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; キャンペーン </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; への電子メール </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 添付ファイル </B></font></td>";
			$NOTESout .= "</tr>\n";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (preg_match('/NONE/i',$view_calls_in_queue)) or (preg_match('/N/i',$grab_calls_in_queue)) )
		{
		echo "ERROR: このキャンペーンに対し無効になっている列になっている電話ビュー\n";
		exit;
		}
	else
		{
