...
<?php
echo "<title>AGENTES REMOTOS: $LOGfullname - $PHP_AUTH_USER   ";

if (!$ADD)			{$ADD="31111";}
if ($ADD==31111)	{echo "Modificar Agentes Remotos";}
if ($ADD==41111)	{echo "Modificar Agentes Remotos";}
...
if ($ADD==31111)
	...
	echo "<br>MODIFICAR UNA ENTRADA DE AGENTES REMOTOS: $row[0]<form action=$PHP_SELF method=POST>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Comienzo Del ID del usuario: </td><td align=left>$user_start</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Número de líneas: </td><td align=left><input type=text name=number_of_lines size=3 maxlength=3 value=\"$number_of_lines\"> (Solamente números)</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>IP del Servidor: </td><td align=left>$row[3]</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Extensión Externa: </td><td align=left><input type=text name=conf_exten size=20 maxlength=20 value=\"$conf_exten\"> (number dialed to reach agents [i.e. 913125551212])</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Estado: </td><td align=left><select size=1 name=status><option SELECTED>ACTIVO</option><option>INACTIVE</option><option SELECTED>$status</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Campaña: </td><td align=left>$campaign_id</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Grupos De entrada: </td><td align=left>\n";
	echo "$groups_list";
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=submit value=submit></td></tr>\n";
	echo "</TABLE></center>\n";
	echo "NOTA: Puede tomar hasta 30 segundos para que los cambios remitidos en esta pantalla se activen\n";
	}
...
if ($ADD==41111)
	...
	if ( (strlen($number_of_lines) < 1) or (strlen($conf_exten) < 2) )
		{echo "<br>AGENTES REMOTOS NO MODIFICADOS - Por favor regresar y revisar los datos incorporados\n";}
	else
		{
...
#		echo "$stmt\n";
		echo "<br>AGENTES REMOTOS MODIFICADOS\n";

		### LOG CHANGES TO LOG FILE ###
	#	$fp = fopen ("./admin_changes_log.txt", "a");
	#	fwrite ($fp, "$date|MODIFY AGENTES REMOTOS ENTRY     |$PHP_AUTH_USER|$ip|$stmt|\n");
	#	fclose($fp);
		}
...
if ($ADD==61111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		{echo "<br>ERROR AGENTES REMOTOS - Por favor regresar y revisar los datos incorporados\n";}
	else
		{
		$users_list = '';
		$k=0;
		...
		if ($talking_to_print > 0)
			...
			echo "+------------|--------+--------------+------------+--------+---------------------+---------+\n";
			echo "  $i agentes registrados en el servidor $server_ip\n\n";

			echo "  <SPAN class=\"yellow\"><B>          </SPAN> - Llamadas de 5 min o más</B>\n";
			echo "  <SPAN class=\"orange\"><B>          </SPAN> - Llamadas de mas de 10 min </B>\n";
			}
		else
			{
			echo "**************************************************************************************\n";
...
if ($ADD==71111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		...
		echo "EstadÍsticas de entradas de agente remoto                               $NOW_TIME\n\n";
		echo "\n";
...
#			echo "  <SPAN class=\"orange\"><B>          </SPAN> - Over 10 minutes on call</B>\n";
			...
			{
			echo "**************************************************************************************\n";
			echo "*********************************SIN LLAMADAS PARA ESTE DIA*******************************\n";
			echo "**************************************************************************************\n";
			}
		}
