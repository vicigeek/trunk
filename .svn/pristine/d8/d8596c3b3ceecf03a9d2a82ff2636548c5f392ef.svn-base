...
<title>ADMINISTRATION: Dialer πεδία λογαριασμό ηλεκτρονικού ταχυδρομείου
<?php 
...
if (($ΕΠΙΒΕΒΑΙΩΣΗ=="ΕΠΙΒΕΒΑΙΩΣΗ" || $ΕΠΙΒΕΒΑΙΩΣΗ=="UPDATE") && $email_account_id) 
	{
	$error_msg="";
	if (!$list_id) {$error_msg.="- Default list ID is άκυρη ή null<BR/>";}
	if (!$email_account_id) {$error_msg.="- Email account ID is άκυρη ή null<BR/>";}
	if (!$email_account_name) {$error_msg.="- Email account name is άκυρη ή null<BR/>";}
	if (!$email_account_server) {$error_msg.="- Email account server is άκυρη ή null<BR/>";}
	if (!$email_account_user) {$error_msg.="- Email account user is άκυρη ή null<BR/>";}
	if (!$email_account_pass) {$error_msg.="- Email account password is άκυρη ή null<BR/>";}
	if (!filter_var($email_replyto_address, FILTER_VALIDATE_EMAIL)) {$error_msg.="- Email reply-to address is άκυρη ή null<BR/>";}

	if (!$error_msg) 
		{
		if ($ΕΠΙΒΕΒΑΙΩΣΗ=="ΕΠΙΒΕΒΑΙΩΣΗ") 
			{
			if ($add_copy_disabled > 0)
				{
				echo "<br>Δεν έχετε άδεια για να προσθέσετε αρχεία σε αυτό το σύστημα -system_settings-\n";
				}
			else
				{
				...
				if (mysqli_affected_rows($link)==0) 
					...
					{
					$message="Νέος λογαριασμός $email_account_id Δημιουργήθηκε με επιτυχία";
					$eact="";
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="ACCOUNT $email_account_id Τροποποιήθηκε επιτυχώς";
				# $eact="";
	...
	if (mysqli_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="Νέος λογαριασμός $new_account_id Αντιγράφηκαν με επιτυχία από $source_email_account<BR/>";
				$eact="";
		...
		{
		$error_msg="- Error - email πηγή δεν υπάρχει<BR/>";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "Δεν λαμβάνουν υπόψη COPIED<BR/>Τα δεδομένα που υποβάλλονται έχει τα ακόλουθα σφάλματα:<BR/>$error_msg";}
		echo "<br>Αντιγράψτε νέου λογαριασμού από τον υπάρχοντα λογαριασμό<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_ΕΠΙΒΕΒΑΙΩΣΗ>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Αριθμός λογαριασμού για να αντιγράψει από: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** Δεν υπάρχουν υπάρχοντες λογαριασμούς να αντιγράψετε από ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "Δεν λαμβάνουν υπόψη ΕΙΣΗΓΜΕΝΗ<BR/>Τα δεδομένα που υποβάλλονται έχει τα ακόλουθα σφάλματα:<BR/>$error_msg";}
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>Επιβεβαιώστε τη διαγραφή των εισερχόμενων λογαριασμό email $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Κάντε κλικ για να διαγράψετε το λογαριασμό $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_to_mysqli($stmt, $link);
		...
		if ($error_msg) {echo "ΔΕΝ υπόψη τις ενημερωμένες<BR/>Τα δεδομένα που υποβάλλονται έχει τα ακόλουθα σφάλματα:<BR/>$error_msg";}
...
################
		...
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=ΕΠΙΒΕΒΑΙΩΣΗ VALUE=UPDATE><input type=hidden name='eact' value='UPDATE'></td></tr>\n";
		echo "</TABLE><BR><BR>";
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Κάντε κλικ εδώ για να δείτε Διαχειριστής αλλαγές σε αυτό το αρχείο</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>INBOUND ΑΚΙΝΗΤΑ λογαριασμού e-mail:\n";
	...
	echo "<TD><font size=1 color=white>Αδιάβασών ηλεκτρονικών ταχυδρομείων</TD>\n";
	...
	while ($accounts_to_print > $o) 
		...
		echo "<td><font size=1><a href=\"$PHP_SELF?eact=UPDATE&email_account_id=$row[email_account_id]\">ΤΡΟΠΟΠΟΙΗΣΗ</a></font></td></tr>\n";
		$o++;
		}
