...
28 => "Неверный number format (address incomplete).",
29 => "Facilities rejected.",
30 => "Response to СОСТОЯНИЕ INQUIRY.",
...
81 => "Неверный call reference value.",
88 => "Incompatible destination.",
95 => "Неверный message, unspecified.",
...
100 => "Неверный information element contents.",
101 => "Message not compatible with call state.",
102 => "Recovery на timer expiry.",
103 => "Parameter non-existent or not implemented - passed на (national use).",
...
438 => "Неверный Identity Header.",
470 => "Consent Needed.",
480 => "Temporarily Unavailable.",
481 => "Call/Transaction Does Not Exist.",
482 => "Loop Detected..",
483 => "Too Many Hops.",
484 => "Адрес Incomplete.",
485 => "Ambiguous.",
486 => "Busy Here.",
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
		{
		echo "Неверный Имя агента/Пароль: |$user|$pass|$auth_message|\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		if ($cl_user_ct > 0)
			...
			### CHECK TO SEE IF агент IS LOGGED IN TO TIMECLOCK, IF NOT, OUTPUT ERROR
	...
	### CHECK TO SEE IF агент IS WITHIN THEIR SHIFT IF RESTRICTED, IF NOT, OUTPUT ERROR
	if ( ( (preg_match("/START|ALL/",$shift_enforcement)) and (!preg_match("/OFF/",$VU_agent_shift_enforcement_override)) ) or (preg_match("/START|ALL/",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>ОШИБКА: для Вашей группы нет разрешенных смен</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) )
				{
				$VDdisplayMESSAGE = "<B>ОШИБКА: Вам не разрешено входить в систему вне своей смены</B>\n";
				$VDloginDISPLAY=1;
				}
			}
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=submit NAME=ВВОД VALUE=ВВОД></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		echo "<option value=\"\">-- ПОЖАЛУЙСТА ВЫБЕРИТЕ КАМПАНИЮ --</option>\n";
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- СНАЧАЛА НУЖНО ОТМЕТИТЬСЯ В ТАБЕЛЬНЫХ ЧАСАХ --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Выбор группы $closer_choice неверный\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Выбор группы $closer_choice был зарегистрирован $user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories неверный\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories был зарегистрирован $user\n";
	}
...
if ($ACTION == 'manDiaLnextCaLL')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
		...
		$channel_live=0;
		echo "БУФЕР ПУСТ\n";
		echo "Conf Exten $conf_exten or campaign $campaign or ext_context $ext_context неверный\n";
		exit;
		}
	else
		{
			...
			if (strlen($phone_number)>3)
				...
				if (preg_match("/ENABLED/",$manual_dial_call_time_check))
					...
					if ($dialable < 1)
						...
						echo "OUTSIDE OF LOCAL ВЫЗОВ TIME   $VMDQaffected_rows|$VLAEDaffected_rows\n";
						exit;
						}
					}
				...
				#	$DB=1;
					if (strlen($dial_statuses)>2)
						...
						if (preg_match("/UP последний NAME/i",$lead_order)){$order_stmt = "order by last_name desc, $last_order";}
						if (preg_match("/DOWN последний NAME/i",$lead_order)){$order_stmt = "order by last_name, $last_order";}
						...
						if (preg_match("/UP Последний звонок TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
						if (preg_match("/DOWN Последний звонок TIME/i",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				{
				//Added by Poundteam for Audited Заметки
				...
				//END Added by Poundteam for Audited Заметки
				}
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "телефон и почтовый индекс часовой пояс несоответствие! ";

					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY')
						{
						$post_phone_time_diff_alert_message='';
						if ($PHONEdialable < 1)
							{$post_phone_time_diff_alert_message .= " Телефон Код города Вне зоны Dialable $PHONEgmt_offset ";}
						if ($POSTdialable < 1)
							{$post_phone_time_diff_alert_message .= " Индекс Вне зоны Dialable $POSTgmt_offset";}
						}
					}
			...
			#	$Local_persist = '/n';
				...
				### check for extension append в кампании
				$use_eac=0;
	...
	#				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
				...
				if (strlen($dial_ingroup) < 1)
					...
					if ($enable_queuemetrics_logging > 0)
						...
						# ВЫЗОВOUTBOUND (formerly ENTERQUEUE)
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "ДОП.ТЕЛЕФОН НОМЕР СОСТОЯНИЕ NOT CHANGED\n";
		echo "$phone_number $stage $lead_id or $called_count неверный\n";
		exit;
		}
	else
		{
		...
		echo "ДОП.ТЕЛЕФОН НОМЕР СОСТОЯНИЕ CHANGED\n";
		}
}
...
if ($ACTION == 'manDiaLonly')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1) || (strlen($ext_context)<1) || (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		...
		### check for manual dial filter and extension append settings в кампании
		$use_eac=0;
		$use_custom_cid=0;
...
if ($ACTION == 'manDiaLlookCaLL')
	...
	if (strlen($MDnextCID)<18)
		{
		echo "NO\n";
		echo "MDnextCID $MDnextCID неверный\n";
		exit;
		}
	else
		{
		...
		if ($VM_mancall_ct > 0)
			...
			{
			### after 10 секунд, start checking for call termination in the carrier log
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		echo "ЖУРНАЛ НЕ УКАЗАН\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id or list_id: $list_id or phone_number: $phone_number or campaign: $campaign неверный\n";
		exit;
		}
	else
		{
		...
		if ($affected_rows > 0)
			{
			echo "VICIDiaL_LOG Добавлено: $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
	...
	#	if ($affected_rows > 0)
	#		{
	#		echo "CALL_LOG Добавлено: $uniqueid|$channel|$NOW_TIME";
	#		}
	#	else
	#		{
	#		echo "ЖУРНАЛ НЕ УКАЗАН\n";
...
if ($stage == "end")
	...
	if ( (strlen($uniqueid)<1) or (strlen($lead_id)<1) )
		{
		echo "ЖУРНАЛ НЕ УКАЗАН\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id неверный\n";
		$log_no_enter=1;
		}
	else
		{
...
if ($ACTION == 'VDADREcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || (strlen($lead_id)<1) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Кампания $campaign неверный\n";
		echo "lead_id $lead_id неверный\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups на vicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row на selected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Кампания $campaign неверный or lead_id $lead_id неверный\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user на $server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		{
		echo "NO\n";
		echo "campaign $campaign or conf_exten $conf_exten неверный\n";
		exit;
		}
	else
		{
		...
		if ($vla_delete > 0) 
			...
			#	$affected_rows = mysqli_affected_rows($linkB);
				...
				if ($queuemetrics_addmember_enabled > 0)
					...
					### add the logged-в кампании as well
					$AMqueue[$i] = $campaign;
					$i++;
					$amq_conf_ct++;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id неверный\n";
		exit;
		}
	else
		{
		...
		//Added by Poundteam Incorporated for Audit Заметки Package
		require_once('audit_comments.php');
		audit_comments($lead_id,$list_id,$format,$user,$mel,$NOW_TIME,$link,$server_ip,$session_name,$one_mysql_log,$campaign);

		// JOEJ - Эл.почта feature - may not be necessary if vicidial_email_list doesn't need a status column.
		if ($email_enabled>0) 
			{
		...
		if ( (preg_match('/ACCTID/',$vtiger_search_category)) or (preg_match('/ACCOUNT/',$vtiger_search_category)) )
			...
			if ($VIDcount > 0)
				...
				if (preg_match('/Y/',$vtiger_status_call))
					...
					# Date of Фамилия Attempt, Date of Фамилия Non-Contact, Date of Фамилия Contact, Date of Фамилия Sale
	...
	echo 'Lead ' . $lead_id . ' был изменен на ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id неверный\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue на to that action without printing output
		$stage = 0;
		$status = $sub_status;
		...
		{
		echo 'Agent ' . $user . ' теперь находится в состоянии ' . $stage . "\nNext agent_log_id:\n$agent_log_id\n";
		}
	}
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list неверный\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		{
		echo "agent_log_id $agent_log_id or pause_code $status неверный\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-ОЖИДАНИЕ &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-ВЫЗОВ &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-ПАУЗА &nbsp;\n";
	if (preg_match("/NOT-LOGGED-IN-AGENTS/",$agent_status_viewable_groups))
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		{
		echo "Для этой кампании просмотр вызовов в очереди запрещен\n";
		exit;
		}
	else
		{
		...
		### grab the basic data на calls in the queue for this agent
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; телефон &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; полное имя &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ждать &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; агент &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; называют группу &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; тип &nbsp; </font></TD>";
		echo "</TR>";

		### Print call information and gather more info на the calls as they are printed
		$loop_count=0;
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (preg_match('/Y/i',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"callinqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">TAKE ВЫЗОВ</a> &nbsp; </TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<a href=\"#\" onclick=\"VieWCalLLoG('','form');return false;\">Выбрать</a> &nbsp;  &nbsp; &nbsp; ";
	echo "<a href=\"#\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> близко </a>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
	echo "<TR>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> &nbsp; # &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; дата\/время &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; длина &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; СОСТОЯНИЕ &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; телефон &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; полное имя &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; кампания &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; в и &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; заместитель &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; зависание &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; информация &nbsp; </font></TD>";
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ВЫЗОВ &nbsp; </font></TD>";
	echo "</TR>";
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>Нет звонков в этот день</td></tr>";}
	...
	while ($g > $u) 
		...
		echo "<td align=right><font size=2> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i]);return false;\"> информация </A> </td>\n";
		if ($manual_dial_filter > 0)
			{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ВЫЗОВ </A> </td>\n";}
		else
			{echo "<td align=right><font size=2> ВЫЗОВ </td>\n";}
		echo "</tr>\n";
		}

	echo "</TABLE>";
	echo "<BR>";
	echo "<a href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">Закрыть журнал вызовов</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			{
			echo "Ошибка: Вы должны войти в поисках слагаемых, одно из них должно быть заполнено: свинец ID, Код п., номер телефона, фамилию\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Вернуться</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<CENTER>\n";
			echo "<font style=\"font-size:14px;font-family:sans-serif;\"><B>";
			echo "Результаты Найдено: $search_result_count";
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			echo "<TR>";
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; телефон &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; СОСТОЯНИЕ &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; Последний звонок &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; город &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; государство &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; застежка-молния &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; информация &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ВЫЗОВ &nbsp; </font></TD>";
			echo "</TR>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr bgcolor=white><td colspan=10 align=center>Ничего не найдено</td></tr>";}
				...
				while ($g > $u) 
					...
					echo "<td align=right><font size=2> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i],'','$inbound_lead_search');return false;\"> информация </A> </td>\n";
					if ($inbound_lead_search < 1)
						{
						if ($manual_dial_filter > 0)
							{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> ВЫЗОВ </A> </td>\n";}
						else
							{echo "<td align=right><font size=2> ВЫЗОВ </td>\n";}
						}
					else
						{
						echo "<td align=right><font size=2> <a href=\"#\" onclick=\"LeaDSearcHSelecT('$ALLlead_id[$i]');return false;\">выберите</A> </td>\n";
						}
					echo "</tr>\n";
					}
				...
				$stmtL="UPDATE vicidial_lead_search_log set секунд='$search_seconds' where search_log_id='$search_log_id';";
				if ($DB) {echo "|$stmtL|\n";}
				$rslt=mysql_to_mysqli($stmtL, $link);
			...
			{
			echo "Ошибка: была проблема с условия поиска\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Вернуться</a>";
			echo "</CENTER>";
			exit;
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			{
			echo "Ошибка: Вы должны войти в поисках слагаемых, одно из них должно быть заполнено: рабочий телефон, фамилию, имя, отчество\n";
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">Вернуться</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0)
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; последний NAME &nbsp; </font></TD>";
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					echo "</tr>\n";
					echo "<tr $bgcolor>";
					echo "<td colspan=8><font size=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <B>Dept:</B> $ALLdepartment[$i] &nbsp; &nbsp; &nbsp; <B>Group:</B> $ALLgroup_name[$i] &nbsp; &nbsp; &nbsp; <B>Job:</B> $ALLjob_title[$i] &nbsp; &nbsp; &nbsp; <B>Расположение:</B> $ALLlocation[$i]</td>";
					echo "</tr>\n";
					}
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Ответный статус: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Обратный вызов ведущий статус: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Время обратного вызова Вступление: &nbsp; </td><td ALIGN=left><font size=2>$row[1]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Обратный вызов Trigger время: &nbsp; </td><td ALIGN=left><font size=2>$row[2]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Ответный Комментарии: &nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Состояние: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>часовой пояс: &nbsp; </td><td ALIGN=left><font size=2>$row[3]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>называется с момента последнего сброса: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ВЫЗОВ </a>";}
					else
						{$INFOout .= " ВЫЗОВ ";}
					}
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>набрать ссылку: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> ВЫЗОВ </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>набрать ссылку: &nbsp; </td><td ALIGN=left><font size=2> ВЫЗОВ ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right colspan=2><font size=2><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">выберите этот провод</a></td></tr>";}
			...
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\"> ВЫЗОВ </a>";}
					else
						{$INFOout .= " ВЫЗОВ ";}
					}
				}
			...
			if ($hide_call_log_info=='N')
				{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>количество вызовов: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";}
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Последний вызов местному времени: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
		...
		if ($hide_call_log_info=='N')
			{
			if ($search != 'logfirst')
				{$NOTESout .= "<CENTER>Журнал вызовов У объявления:<br>\n";}
		...
		if ($allow_emails>0)
			{
			$NOTESout .= "<CENTER>Электронная почта Вход У объявления:<br>\n";
			$NOTESout .= "<TABLE CELLPADDING=0 CELLSPACING=1 Border=0 WIDTH=$stage>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\"><font style=\"font-size:10px;font-family:sans-serif;\"><B> # </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; дата\/время </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; агент </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; кампания </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; по электронной почте к </B></font></td>";
			$NOTESout .= "<td BGCOLOR=\"#CCCCCC\" align=left><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; вложения </B></font></td>";
			$NOTESout .= "</tr>\n";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (preg_match('/NONE/i',$view_calls_in_queue)) or (preg_match('/N/i',$grab_calls_in_queue)) )
		{
		echo "ERROR: Для этой кампании просмотр вызовов в очереди запрещен\n";
		exit;
		}
	else
		{
...
function hangup_cause_description($code)
	...
	else { return "неизвестный код причины зависания."; }
	}
