...
$report_name='Liste des distributeurs de fiches Report';
...
echo "<TITLE>Liste des distributeurs de fiches Report</TITLE></HEAD><BODY BGCOLOR=WHITE marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
...
if (!$group)
	...
	{
	echo "Liste des distributeurs de fiches en cours                      $NOW_TIME\n";
	...
	echo "+------+--------+-----------+------------+------------+-------+--------+-------+--------+-------+-------+----------------------+\n";
	echo "|ORDER |PRIORITÉ| LEAD ID   | ID DE LA LISTE    | PHONE NUM  | STATE | STATUS | COUNT | GMT    | ALT   | SOURCE| VENDOR LEAD CODE     |\n";
	echo "+------+--------+-----------+------------+------------+-------+--------+-------+--------+-------+-------+----------------------+\n";
	...
	while ($i < $users_to_print)
		...
		if ($LOGadmin_hide_lead_data != '0')
			{
			if ($DB > 0) {echo "CACHERLEADDATA|$row[2]|$LOGadmin_hide_lead_data|\n";}
			if (strlen($row[2]) > 0)
