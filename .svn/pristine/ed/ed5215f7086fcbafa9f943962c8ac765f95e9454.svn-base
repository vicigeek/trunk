...
echo "<!-- VERSION: <?php echo $version ?>     BAU: <?php echo $build ?> -->\n";
?>
...
window.onload = function() {
	...
	document.getElementById("enable_move_country_code").onclick = enableMoveLandCode;
	document.getElementById("enable_move_vendor_lead_code").onclick = enableMoveVendorLeadCode;
	document.getElementById("enable_move_source_id").onclick = enableMoveLandSourceId;
	...
	document.getElementById("enable_update_country_code").onclick = enableUpdateLandCode;
	document.getElementById("enable_update_vendor_lead_code").onclick = enableUpdateVendorLeadCode;
	document.getElementById("enable_update_source_id").onclick = enableUpdateLandSourceId;
	...
	document.getElementById("enable_delete_country_code").onclick = enableDeleteLandCode;
	document.getElementById("enable_delete_vendor_lead_code").onclick = enableDeleteVendorLeadCode;
	document.getElementById("enable_delete_source_id").onclick = enableDeleteLandSourceId;
...
if ($move_submit == "move" )
	...
	if (($enable_move_vendor_lead_code == "enabled") && ($move_vendor_lead_code != ''))
		...
		$move_parm = $move_parm . "&nbsp;&nbsp;&nbsp;&nbsp;Verkäuferleitung Code is like $move_vendor_lead_code<br />";
	...
	elseif ($enable_move_security_phrase == "enabled")
		{
		blank_field('Sicherheit Phrase',true);
		}
	...
	if ( $to_list_lead_count + $move_lead_count > $list_lead_limit )
		...
		echo "<html>\n";
		echo "<head>\n";
		echo "<!-- VERSION: $version     BAU: $build -->\n";
		echo "</head>\n";
		echo "<body>\n";
...
if ($update_submit == "update" )
	...
	if ($update_list == '') { missing_required_field('Liste Identifikation'); }
	...
	if (($enable_update_vendor_lead_code == "enabled") && ($update_vendor_lead_code != ''))
		...
		$update_parm = $update_parm . "&nbsp;&nbsp;&nbsp;&nbsp;Verkäuferleitung Code is like $update_vendor_lead_code<br />";
...
if ( ( $delete_submit == "delete" ) && ( $delete_lists > 0 ) )
	...
	if ($delete_list == '') { missing_required_field('Liste Identifikation'); }
	...
	if (($enable_delete_vendor_lead_code == "enabled") && ($delete_vendor_lead_code != ''))
		...
		$delete_parm = $delete_parm . "&nbsp;&nbsp;&nbsp;&nbsp;;Verkäuferleitung Code is like $delete_vendor_lead_code<br />";
...
if (
	...
	if ( preg_match("/ALL\-CAMPAIGNS/i",$allowed_campaigns) )
		{
		if ($DB) { echo "|VerarbeitungAll Kampagnen|\n"; }
	...
	echo "<p>The following are advanced lead management tools.  They will only work on unaktivierte Listen with less than $list_lead_limit leads in them. This is to avoid data inconsistencies.</p>";
	...
	echo "</select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=rightLandCode</td></td><td align=left>\n";
	...
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>ÄndernDate</td></td><td align=left>\n";
	...
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Sicherheit Phrase</td></td><td align=left>\n";
	...
	echo "<tr bgcolor=#015B91><td colspan=2 align=center><font color=white><b>Update Lead Status</b></font></td></tr>\n";
	...
	if ( $delete_lists > 0 )
		...
		echo "</table></center>\n";
		echo "<br /><center><table width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#015B91><td colspan=2 align=center><font color=white><b>LöschenLeads</b></font></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>List</td><td align=left>\n";
