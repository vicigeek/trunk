...
if( (strlen($PHP_AUTH_USER)<2) or (strlen($PHP_AUTH_PW)<2) or (!$auth))
    ...
    Header("WWW-Authenticate: Basic realm=\"CONTACT-CENTER-ADMIN\"");
    Header("HTTP/1.0 401 Unauthorized");
    echo "Unzulässiges Username/Kennwort:|$PHP_AUTH_USER|$PHP_AUTH_PW|$auth_message|\n";
    exit;
	}
else
	{
...
<?php
echo "<title>REMOTEMITTEL: $LOGfullname - $PHP_AUTH_USER   ";

if (!$ADD)			{$ADD="31111";}
if ($ADD==31111)	{echo "Ändern Sie Remotemittel";}
if ($ADD==41111)	{echo "Ändern Sie Remotemittel";}
...
if ($ADD==31111)
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Benutzernummer Anfang: </td><td align=left>$user_start</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Zeilenzahl: </td><td align=left><input type=text name=number_of_lines size=3 maxlength=3 value=\"$number_of_lines\"> (nur Zahlen)</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Bediener IP: </td><td align=left>$row[3]</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Externe Verlängerung: </td><td align=left><input type=text name=conf_exten size=20 maxlength=20 value=\"$conf_exten\"> (number dialed to reach agents [i.e. 913125551212])</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Status:</td><td align=left><select size=1 name=status><option SELECTED>AKTIV</option><option>INACTIVE</option><option SELECTED>$status</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Kampagne: </td><td align=left>$campaign_id</td></tr>\n";
...
#		echo "$stmt\n";
		echo "<br>REMOTEMITTEL GEÄNDERT\n";

		### LOG CHANGES TO LOG FILE ###
	#	$fp = fopen ("./admin_changes_log.txt", "a");
	#	fwrite ($fp, "$date|MODIFY REMOTEMITTEL ENTRY     |$PHP_AUTH_USER|$ip|$stmt|\n");
	#	fclose($fp);
		}
...
if ($ADD==61111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		{echo "<br>REMOTEMITTEL-STÖRUNG - gehen Sie bitte zurück und betrachten Sie dieDaten, die Sie eingaben\n";}
	else
		{
		$users_list = '';
		$k=0;
		...
		if ($talking_to_print > 0)
			...
			echo "  <SPAN class=\"yellow\"><B>          </SPAN> - 5 Minuten oder mehr beim Anruf</B>\n";
			echo "  <SPAN class=\"orange\"><B>          </SPAN> - Über 10 Minuten beim Anruf</B>\n";
			}
		else
			{
			echo "**************************************************************************************\n";
...
if ($ADD==71111)
	...
	if ( (strlen($server_ip) < 2) or (strlen($user) < 2) )
		...
		echo "Inbound Notfall des Remotemittels                               $NOW_TIME\n\n";
		echo "\n";
