...
if ($user_territories_active < 1)
	{
	echo "ERROR: Territori degli utenti non sono attivi su questo sistema\n";
	exit;
	}
...
if( (strlen($USER)<2) or (strlen($PASS)<2) or (!$auth) or (!$modify_users_auth))
	...
	echo "Utentename/Password non validi: |$USER|$PASS|$auth_message|\n";
	exit;
	}
...
<!-- VERSIONE: <?php echo $version ?>     BUILD: <?php echo $build ?> -->
<title>ADMINISTRATION: Utente Territori
<?php
...
if ( ($action == "CHANGE_TERRITORY_OWNER_ACCOUNT") and ($enable_vtiger_integration > 0) )
	...
	echo "<br>Vtiger Cambia Territorio Proprietario<form action=$PHP_SELF method=POST>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>New Proprietario:</td><td align=left><select size=1 name=user>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Update ViciDial List Proprietario:</td><td align=left><select size=1 name=vl_owner>\n";
...
if ( ($action == "PROCESS_CHANGE_TERRITORY_OWNER_ACCOUNT") and ($enable_vtiger_integration > 0) )
	...
	if ( (strlen($accountid)<1) or (strlen($user)<1) )
		{
		echo "ERROR: Account ID and Utente must be filled in<BR>\n";
		}
	else
		{
		if (preg_match("/YES/i",$batch))
		...
		if ($vat_ct > 0)
			...
			if ($row[0] < 1)
				...
				echo "NOTICE: Dovuto aggiungere territorio utente: $user $territory<BR>\n";
			...
			if ($vtu_ct > 0)
				...
				echo "Vtiger Territorio Proprietario Changed: $user $territory<BR>\n";
				echo "Record modificati: $changed<BR>\n";
...
<TR BGCOLOR=#E6E6E6>
<TD ALIGN=LEFT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>">Elenco Territori</a></TD>
<TD ALIGN=RIGHT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>?action=ADD_TERRITORY">Aggiungi Territorio</a></TD>
<TD ALIGN=RIGHT><FONT FACE="ARIAL,HELVETICA" SIZE=2><B> &nbsp; <a href="<?php echo "$PHP_SELF" ?>?action=ADD_USER_TERRITORY">Aggiungi Utente Territorio</a></TD>
<?php
if ($enable_vtiger_integration > 0)
	{ ?>
...
if ( ($action == "PROCESS_CHANGE_TERRITORY_OWNER") and ($enable_vtiger_integration > 0) )
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		{
		echo "ERROR: Territorio e degli utenti deve essere compilato in<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($vtu_ct > 0)
			...
			echo "Vtiger Territorio Proprietario Changed: $user $territory &nbsp; &nbsp; &nbsp; Record modificati: $changed - $Achanged - $Cchanged - $Vchanged<BR>\n";
...
if ($action == "ADD_USER_TERRITORY")
	...
	echo "<br>Aggiungi Utente Territorio<form action=$PHP_SELF method=POST>\n";
...
if ($action == "PROCESS_ADD_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		{
		echo "ERROR: Territorio, degli utenti e di livello deve essere compilato in<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_user_territories where territory='$territory' and user='$user';";
		...
		if ($row[0] > 0)
			{
			echo "ERROR: questo utente territorio è già nel sistema<BR>\n";
			}
		else
			{
			...
			echo "Utente Territorio Added: $user $territory<BR>\n";
...
if ($action == "PROCESS_MODIFY_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) or (strlen($level)<1) )
		...
		if ($row[0] < 0)
			{
			echo "ERROR: questo utente territorio non è nel sistema di<BR>\n";
			}
		else
			{
			...
			echo "Utente Territorio Modified: $user $territory<BR>\n";
...
if ($action == "DELETE_USER_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($user)<1) )
		...
		if ($row[0] < 0)
			...
			echo "Utente Territorio Soppresso: $user $territory<BR>\n";
...
if ($action == "ADD_TERRITORY")
	...
	echo "<br>Aggiungi Territorio<form action=$PHP_SELF method=POST>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Descrizione territorio: </td><td align=left><input type=text name=territory_description size=50 maxlength=255></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=INVIA value=INVIA></td></tr>\n";
	echo "</TABLE></center>\n";
	}
...
if ($action == "PROCESS_ADD_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		{
		echo "ERROR: Territorio e Territorio Descrizione deve essere compilato in<BR>\n";
		}
	else
		{
		...
		if ($row[0] > 0)
			{
			echo "ERROR: vi è già un territorio nel sistema con questo nome<BR>\n";
			}
		else
			{
			...
			echo "Territorio Added: $territory<BR>\n";
...
if ($action == "DELETE_TERRITORY")
	...
	if (strlen($territory)<1)
		{
		echo "ERROR: Territorio deve essere compilato in<BR>\n";
		}
	else
		{
		$stmt="SELECT count(*) from vicidial_territories where territory='$territory';";
		...
		if ($row[0] < 0)
			{
			echo "ERROR: Questo territorio non è presente nel sistema con questo nome<BR>\n";
			}
		else
			{
			...
			echo "Territorio Soppresso: $territory<BR>\n";
...
if ($action == "PROCESS_MODIFY_TERRITORY")
	...
	if ( (strlen($territory)<1) or (strlen($territory_description)<1) )
		...
		if ($row[0] < 1)
			...
			echo "Territorio Modified: $territory<BR>\n";
...
if ($action == "MODIFY_TERRITORY")
	...
	if ($territories_to_print > 0) 
		...
		echo "<br>Modifica Territorio<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Descrizione territorio: </td><td align=left><input type=text name=territory_description size=50 maxlength=255 value=\"$rowx[1]\"></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Agenti: </td><td align=left><B>$user_count</B></td></tr>";
		...
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=INVIA value=INVIA></form></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "<BR><BR>\n";

		echo "<TABLE CELLPADDING=3 CELLSPACING=1 WIDTH=600><TR><TD COLSPAN=5><FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>Utenti in this Territorio:</TD></TR>\n";
		...
		#	echo "<TD>Accounts: $owner_count</TD>";
			...
			echo "</form>";
			echo "</TD>";
			echo "<TD><a href=\"$PHP_SELF?action=DELETE_USER_TERRITORY&territory=$territory&user=$Tuser[$o]\">CANCELLA</a></TD>";
			echo "</TR>\n";
			$o++;
			}
		...
		echo "<a href=\"$PHP_SELF?action=DELETE_TERRITORY&territory=$territory\">Elimina Questo territorio</a>\n";
		echo "<BR><BR>\n";
		}
	else
		{
		echo "ERROR: Territorio non trovato: $territory<BR>\n";
		}
	}
...
<BR><font size=1>Utente Territori &nbsp; &nbsp; VERSIONE: <?php echo $version ?> &nbsp; &nbsp; BUILD: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
