...
$report_name = 'Agent Status Détail';
$db_source = 'M';
...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		{
		$ASCII_text.="<BR><BR>Agent Status Détail Report                     $NOW_TIME\n";
		$ASCII_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		$ASCII_text.="---------- AGENT Détails -------------\n\n";
		$GRAPH.="<BR><BR>Agent Status Détail Report                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>CALL STATS BREAKDOWN</caption><tr><th class='thgraph' scope='col'>ETAT</th>";
...
echo "<INPUT TYPE=Submit NAME=VALIDER VALUE=VALIDER>$NWB#agent_status_detail$NWE\n";
