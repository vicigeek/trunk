...
if ($format=='debug')
	...
	echo "<title>Kolla konfanknytningen";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'refresh')
	...
	if (strlen($conf_exten)<1)
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten är ej giltig\n";
		exit;
		}
	else
		{
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03007',$user,$server_ip,$session_name,$one_mysql_log);}
				$row=mysqli_fetch_row($rslt);
				$RingCalls=$row[0];
				if ($RingCalls > 0) {$RingCalls = "<font class=\"queue_text_red\">Samtal i kö: $RingCalls</font>";}
				else {$RingCalls = "<font class=\"queue_text\">Samtal i kö: $RingCalls</font>";}
...
if ($ACTION == 'register')
	...
	if ( (strlen($conf_exten)<1) || (strlen($exten)<1) )
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten är ej giltig or Exten $exten är ej giltig\n";
		exit;
		}
	else
		{
	...
	echo "Konferens $conf_exten har blivit registrerad $exten\n";
	}
