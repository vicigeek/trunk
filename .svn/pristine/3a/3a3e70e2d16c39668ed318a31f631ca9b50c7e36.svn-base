...
function LoadLists(FromBox) {
	...
	for(i = 0; i < document.getElementById('group').options.length; i++) {
		if (document.getElementById('group').options[i].selected) {
			selectedCampagnes += document.getElementById('group').options[i].value+"|";
		} 
	}

	// Supprimer la liste menu
	...
	for (j=0; j<campaign_id_ary.length; j++) {
		...
		var campaign_matches = selectedCampagnes.match(campaignID);
		if (campaign_matches) {
...
if (preg_match('/MSIE/i', $_SERVER['HTTP_USER_AGENT'])) {
	$JS_events="onBlur='LoadListes(this.form.group)' onKeyUp='LoadListes(this.form.group)'";
} else {
	$JS_events="onMouseUp='LoadListes(this.form.group)' onBlur='LoadListes(this.form.group)' onKeyUp='LoadListes(this.form.group)'";
}
...
echo "Listes: <font size=1>(en option, peut-être lente)</font><BR>\n";
echo $list_options;
echo "</TD><TD VALIGN=TOP>";
...
if (strlen($group[0]) > 1)
	{
	echo " <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">MODIFIER</a> | \n";
...
if (strlen($group[0]) < 1)
	...
	$OUToutput .= "Total des appels placés dans cette campagne: $TOTALcalls\n";
	$OUToutput .= "Durée moyenne des appels pour tous les appels en secondes: $average_call_seconds\n";
	if (preg_match("/YES/i",$include_rollover))
		{$OUToutput .= "Calls that went to rollover In-Groupe:         $inTOTALcalls\n";}


	$OUToutput .= "\n";
	$OUToutput .= "---------- RÉPONSE HUMAINES\n";
	...
	$OUToutput .= "Total Réponse de l'hommeed calls for this Campagne: $CIcalls\n";
	$OUToutput .= "Average Call Length for all HA en secondes:    $average_ci_seconds     TotalTime: $CIsec\n";


	$OUToutput .= "\n";
	$OUToutput .= "---------- ABANDONNES\n";
	...
	$DROPREPONSEpercent = (MathZDC($DROPcallsRAW, $REPONSEcalls) * 100);
	...
	$OUToutput .= "Percent of DROP Calls taken out of Answers:   $DROPcalls / $REPONSEcalls  $DROPREPONSEpercent%\n";

	if (preg_match("/YES/i",$include_rollover))
		{
		$inDROPREPONSEpercent = (MathZDC($DROPcallsRAW, $CIcallsRAW) * 100);
		$inDROPREPONSEpercent = round($inDROPREPONSEpercent, 2);

		$OUToutput .= "Percent of DROP/Answer Calls with Rollover:   $DROPcalls / $CIcallsRAW  $inDROPREPONSEpercent%\n";
		}

	$OUToutput .= "Durée moyenne pour les appels abandonnés en secondes:    $average_hold_seconds\n";
	...
	$AVG_REPONSEagent_non_pause_sec = (MathZDC($TOTALanswers, $agent_non_pause_sec) * 60);
	...
	if ($skip_productivity_calc) {
		...
		$OUToutput .= "Productivity Rating:                          $AVG_REPONSEagent_non_pause_sec\n";
	}
	...
	$OUToutput .= "Durée moyenne des appels pour les appels NA en secondes:  $average_na_seconds\n";


	##############################
	#########  FAIT APPEL Hangup STATS

	$TOTALcalls = 0;

	$ASCII_text .= "\n";
	$ASCII_text .= "---------- FAIT APPEL Hangup STATS\n";
	$ASCII_text .= "+----------------------+------------+\n";
	...
	$GRAPH.="<caption align='top'>FAIT APPEL Hangup STATS</caption>";
	$GRAPH.="<tr>\n";
	...
	while ($i < $reasons_to_print)
		...
		if (preg_match('/NONE/',$reason))	{$reason = 'NO REPONSE           ';}
	...
	$ASCII_text .= "| STATUS | DESCRIPTION          | CATÉGORIE             | CALLS      | TOTAL TIME | AVG TIME |CALLS/HOUR|CALLS/HOUR|\n";
	$ASCII_text .= "+--------+----------------------+----------------------+------------+------------+----------+----------+----------+\n";
...
#	$ASCII_text .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	if ($presets_enabled > 0)
		...
		$GRAPH.="  <tr>\n";
		$GRAPH.="	<th class=\"thgraph\" scope=\"col\">TOTAL DES APPELS:</th>\n";
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- CUSTOM STATUS CATÉGORIE STATS\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	$ASCII_text .= "| CATÉGORIE             | CALLS      | DESCRIPTION                    |\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>AGENT STATS</caption><tr><th class='thgraph' scope='col'>ETAT</th>";
	$CALLS_graph=$graph_header."<th class='thgraph' scope='col'>CALLS </th></tr>";
	$TOTALTIME_graph=$graph_header."<th class='thgraph' scope='col'>TOTAL TIME</th></tr>";
	$AVGTIME_graph=$graph_header."<th class='thgraph' scope='col'>AVG TIME</th></tr>";
	###########################
