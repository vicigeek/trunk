...
function LoadLists(FromBox) {
	...
	for(i = 0; i < document.getElementById('group').options.length; i++) {
		if (document.getElementById('group').options[i].selected) {
			selectedCampañas += document.getElementById('group').options[i].value+"|";
		} 
	}

	// Borrar lista menu
	document.getElementById('list_ids').options.length=0;
	var new_list = new Opción();
	new_list.value = "--ALL--";
	...
	for (j=0; j<campaign_id_ary.length; j++) {
		...
		var campaign_matches = selectedCampañas.match(campaignID);
		if (campaign_matches) {
...
if (preg_match('/MSIE/i', $_SERVER['HTTP_USER_AGENT'])) {
	$JS_events="onBlur='LoadListas(this.form.group)' onKeyUp='LoadListas(this.form.group)'";
} else {
	$JS_events="onMouseUp='LoadListas(this.form.group)' onBlur='LoadListas(this.form.group)' onKeyUp='LoadListas(this.form.group)'";
}
...
echo "Listas: <font size=1>(opcional, posiblemente lento)</font><BR>\n";
echo $list_options;
echo "</TD><TD VALIGN=TOP>";
...
if (strlen($group[0]) < 1)
	{
	echo "\n\n";
	echo "POR FAVOR SELECCIONE ARRIBA UNA CAMPAÑA Y FECHA ARRIBA Y CLICK EN REMITIR\n";
	}
	...
	$OUToutput .= "Total Llamadas realizadas en esta campaña:      $TOTALcalls\n";
	$OUToutput .= "Tiempo medio de todas las llamadas (seg):       $average_call_seconds\n";
	if (preg_match("/YES/i",$include_rollover))
		{$OUToutput .= "Calls that went to rollover In-Grupo:         $inTOTALcalls\n";}


	$OUToutput .= "\n";
	$OUToutput .= "---------- RESPUESTA HUMANOAS\n";
	...
	$OUToutput .= "Total Respuesta Humanaed calls for this Campaña: $CIcalls\n";
	$OUToutput .= "Average Call Length for all HA en segundos:    $average_ci_seconds     TotalTime: $CIsec\n";
	...
	$DROPRESPUESTApercent = (MathZDC($DROPcallsRAW, $RESPUESTAcalls) * 100);
	...
	$OUToutput .= "Percent of DROP Calls taken out of Answers:   $DROPcalls / $RESPUESTAcalls  $DROPRESPUESTApercent%\n";

	if (preg_match("/YES/i",$include_rollover))
		{
		$inDROPRESPUESTApercent = (MathZDC($DROPcallsRAW, $CIcallsRAW) * 100);
		$inDROPRESPUESTApercent = round($inDROPRESPUESTApercent, 2);

		$OUToutput .= "Percent of DROP/Answer Calls with Rollover:   $DROPcalls / $CIcallsRAW  $inDROPRESPUESTApercent%\n";
		}

	$OUToutput .= "Tiempo medio de llamadas Caidas (seg):          $average_hold_seconds\n";
	...
	$AVG_RESPUESTAagent_non_pause_sec = (MathZDC($TOTALanswers, $agent_non_pause_sec) * 60);
	...
	if ($skip_productivity_calc) {
		...
		$OUToutput .= "Productivity Rating:                          $AVG_RESPUESTAagent_non_pause_sec\n";
	}
	...
	$OUToutput .= "Tiempo medio de llamadas sin Respuesta (seg):   $average_na_seconds\n";


	##############################
	#########  ESTADÍSTICAS MOTIVOS CIERRE LLAMADAS

	$TOTALcalls = 0;

	$ASCII_text .= "\n";
	$ASCII_text .= "---------- ESTADÍSTICAS MOTIVOS CIERRE LLAMADAS\n";
	$ASCII_text .= "+----------------------+------------+\n";
	...
	$GRAPH.="<caption align='top'>ESTADÍSTICAS MOTIVOS CIERRE LLAMADAS</caption>";
	$GRAPH.="<tr>\n";
	...
	while ($i < $reasons_to_print)
		...
		if (preg_match('/NONE/',$reason))	{$reason = 'NO RESPUESTA           ';}
	...
	$CALLSHOUR_agent_graph=$graph_header."<th class='thgraph' scope='col'>AGENTE CALLS/HR</th></tr>";
	###########################
...
#	$ASCII_text .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	for ($d=0; $d<count($graph_stats); $d++) {
		...
		$CALLSHOUR_agent_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$d][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$d][5], $max_agentcallshr))."' height='16' />".$graph_stats[$d][5]."</td></tr>";
	}
	...
	if ($presets_enabled > 0)
		...
		$ASCII_text .= "\n";
		$ASCII_text .= "---------- AGENTE PRESET DIALS\n";
		$ASCII_text .= "+------------------------------------------+------------+\n";
		$ASCII_text .= "| PRESET NAME                              | CALLS      |\n";
		$ASCII_text .= "+------------------------------------------+------------+\n";

		$GRAPH="</PRE><table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"white\" summary=\"DID Summary\" class=\"horizontalgraph\">\n";
		$GRAPH.="<caption align='top'>AGENTE PRESET DIALS</caption>";
		$GRAPH.="<tr>\n";
		...
		$GRAPH.="  <tr>\n";
		$GRAPH.="	<th class=\"thgraph\" scope=\"col\">LLAMADAS TOTALES:</th>\n";
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- CUSTOM STATUS CATEGORÍA STATS\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	$ASCII_text .= "| CATEGORÍA             | CALLS      | DESCRIPTION                    |\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- AGENTE STATS\n";
	$ASCII_text .= "+--------------------------+------------+------------+--------+\n";
	$ASCII_text .= "| AGENTE                    | CALLS      | TIME H:M:S |AVERAGE |\n";
	$ASCII_text .= "+--------------------------+------------+------------+--------+\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>AGENTE STATS</caption><tr><th class='thgraph' scope='col'>ESTADO</th>";
	$CALLS_graph=$graph_header."<th class='thgraph' scope='col'>CALLS </th></tr>";
	$TOTALTIME_graph=$graph_header."<th class='thgraph' scope='col'>TOTAL TIME</th></tr>";
	$AVGTIME_graph=$graph_header."<th class='thgraph' scope='col'>AVG TIME</th></tr>";
	###########################
...
#######
	...
	if ($bottom_graph == 'YES')
		...
		echo "GRÁFICO EN INCREMENTOS DE 15 MINUTOS DEL TOTAL DE LLAMDAS REALIZADAS EN ESTA CAMPAÑA\n";
