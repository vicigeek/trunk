...
102 => "Recovery em timer expiry.",
103 => "Parameter non-existent or not implemented - passed em (national use).",
...
500 => "Usuário Internal Error.",
501 => "Not Implemented.",
502 => "Bad Gateway.",
503 => "Service Unavailable.",
504 => "Usuário Time-out.",
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
		{
		echo "Inválido Username/Senha: |$user|$pass|$auth_message|\n";
		exit;
		}
	else
		{
...
if ($format=='debug')
	...
	echo "<title>VICIDiaL Certificado Da Pergunta Da Base de dados";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( ( (preg_match("/START|ALL/",$shift_enforcement)) and (!preg_match("/OFF/",$VU_agent_shift_enforcement_override)) ) or (preg_match("/START|ALL/",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>ERRO: Não existem Shifts activado para o Grupo de Usuários</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) )
				{
				$VDdisplayMESSAGE = "<B>ERRO: Você não tem permissão para iniciar sessão no para fora do seu turno</B>\n";
				$VDloginDISPLAY=1;
				}
			}
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=submit NAME=SUBMETA VALUE=SUBMETA></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- É necessário efetuar login AO PRIMEIRO TIMECLOCK --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Escolha Do Grupo $closer_choice é inválido\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Escolha Do Grupo $closer_choice foi registado ao usuário $user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories é inválido\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories foi registado ao usuário $user\n";
	}
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "ALT DEL TELEFONO NÚMERO STATUS NOT CHANGED\n";
		...
		echo "ALT DEL TELEFONO NÚMERO STATUS CHANGED\n";
		}
}
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		if ($affected_rows > 0)
			{
			echo "VICIDiaL_LOG Introduzido: $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			if ( ($call_type=='OUT') or ($call_type=='OUTBALANCE') )
				...
				### if web form is set then send em to vicidial.php for override of WEB_FORM address
...
if ($ACTION == 'VDADcheckINCOMINGemail')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### Check for transfers
		#$upd_stmt="update vicidial_email_list inner join(select email_row_id from vicidial_email_list inner join vicidial_inbound_groups em vicidial_inbound_groups.group_id=vicidial_email_list.group_id order by queue_priority desc, email_date asc limit 1) as selected_row em selected_row.email_row_id=vicidial_email_list.email_row_id set status='QUEUE', user='$user'";
...
if ($ACTION == 'LeaDSearcHSelecTUpdatE')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || ($lead_id<0) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Campanha $campaign é inválido or lead_id $lead_id é inválido\n";
		exit;
		}
	else
		{
		...
		if ($queue_leadID_ct > 0)
			...
			{
			echo "0\n";
		#	echo "No leads to update for $user em $server_ip\n";
			exit;
			}
		}
	}
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		...
		if ($no_delete_sessions < 1)
			{
			##### Remove the reservation em the vicidial_conferences meetme room
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id é inválido\n";
		exit;
		}
	else
		{
	...
	echo 'Lead ' . $lead_id . ' foi mudado a ' . $dispo_choice . " Status\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id é inválido\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if (strlen($sub_status) > 0)
		{
		### if a pause code(sub_status) is sent with this pause request, continue em to that action without printing output
		$stage = 0;
		$status = $sub_status;
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list é inválido\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		{
		echo "agent_log_id $agent_log_id or pause_code $status é inválido\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		...
		### grab the basic data em calls in the queue for this agent
		...
		### Print call information and gather more info em the calls as they are printed
		$loop_count=0;
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; SELETOR &nbsp; </font></TD>";
	echo "</TR>";
	...
	while ($g > $u) 
		...
		if ($manual_dial_filter > 0)
			{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> SELETOR </A> </td>\n";}
		else
			{echo "<td align=right><font size=2> SELETOR </td>\n";}
		echo "</tr>\n";
		}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Vá Para trás</a>";
			echo "</CENTER>";
			exit;
			}
			...
			if ($search_result_count)
				...
				while ($g > $u) 
					...
					if ($inbound_lead_search < 1)
						{
						if ($manual_dial_filter > 0)
							{echo "<td align=right><font size=2> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"> SELETOR </A> </td>\n";}
						else
							{echo "<td align=right><font size=2> SELETOR </td>\n";}
						}
					else
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">Vá Para trás</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($info_to_print > 0)
			...
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> SELETOR </a>";}
					else
						{$INFOout .= " SELETOR ";}
					}
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>ligação discar: &nbsp; </td><td ALIGN=left><font size=2><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> SELETOR </a>";}
				else
					{$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>ligação discar: &nbsp; </td><td ALIGN=left><font size=2> SELETOR ";}
				}
			$INFOout .= "</td></tr>";
			if ($inbound_lead_search > 0)
			...
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				...
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
						{$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\"> SELETOR </a>";}
					else
						{$INFOout .= " SELETOR ";}
					}
				}
