...
if ($ACTION=="OriginateName")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		echo "Channel $channel deve ser mais grande de 2 caráteres\n";
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten é inválido or queryCID $queryCID é inválido, Originate comando não introduzido\n";
		}
	else
		{
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR Você não tem permissão para discar para sessões de outro agente $exten\n";
			exit;
			}
		...
		echo "Originate comando emitido para Exten $exten Canaleta $channel em $server_ip |$account|$variable|\n";
...
if ($ACTION=="Hangup")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) )
		{
		$channel_live=0;
		echo "Channel $channel é inválido or queryCID $queryCID é inválido, Hangup comando não introduzido\n";
		}
	else
		{
...
#			}
		if ( ($auto_dial_level > 0) and (strlen($CalLCID)>2) and (strlen($exten)>2) and ($secondS > 0))
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02012',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if ($row[0]==0)
					{
					$channel_live=0;
					echo "Channel $channel não está vivo em $call_server_ip, Hangup comando não introduzido $rowx[0]\n$stmt\n";
					}
				else
					{
					echo "$stmt\n";
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02083',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			if ($row[0] > 0)
				{
				$channel_live=0;
				echo "Channel $channel in use by another agent em $call_server_ip, Hangup comando não introduzido $rowx[0]\n$stmt\n";
...
if ($ACTION=="RedirectXtraCXNeW")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		echo "Uma destas variáveis é inválido:\n";
		echo "Channel $channel deve ser mais grande de 2 caráteres\n";
		echo "ExtraChannel $extrachannel deve ser mais grande de 2 caráteres\n";
		echo "queryCID $queryCID deve ser mais grande de 14 caráteres\n";
		echo "exten $exten deve ser ajustado\n";
		echo "ext_context $ext_context deve ser ajustado\n";
		echo "ext_priority $ext_priority deve ser ajustado\n";
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference em $server_ip, Redirect comando não introduzido\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Não pode encontrar a conferência vazia em $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveX=0;
				echo "Channel $channel não está vivo em $call_server_ip, Redirect comando não introduzido\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel não está vivo em $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveY=0;
				echo "Channel $channel não está vivo em $server_ip, Redirect comando não introduzido\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel não está vivo em $server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel em $call_server_ip, Hungup $extrachannel em $server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channel em $server_ip";}
			}
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $exten em $server_ip";}
					}
				else
					{
					$S='*';
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel é inválido or queryCID $queryCID é inválido or filename: $filename é inválido, $ACTION comando não introduzido\n";
		}
	else
		{
		$stmt="SELECT count(*) FROM live_channels where server_ip = '$server_ip' and channel='$channel';";
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_live=0;
				echo "Channel $channel não está vivo em $server_ip, $ACTION comando não introduzido\n";
				}	
			}
		if ($channel_live==1)
			{
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel é inválido or exten $exten é inválido or filename: $filename é inválido, $ACTION comando não introduzido\n";
		}
	else
		{
		$VDvicidial_id='';
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "Conferência $exten, Stage $stage é inválido or queryCID $queryCID é inválido, Originate comando não introduzido\n";
		}
	else
		{
		...
		echo "Volume comando emitido para Conferência $exten, Stage $stage Canaleta $channel em $server_ip\n";
		}
	}
