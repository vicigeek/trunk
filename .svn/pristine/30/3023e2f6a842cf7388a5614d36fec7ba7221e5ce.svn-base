...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<TABLE><TR><TD></TD>\n";
	echo "<!-- ILPA -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_nl/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Dutch <img src=\"../agc/images/nl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- VERSIE: $version     BUILD: $build      ADD: $ADD-->\n";
...
// ################################################################################
// ACTIVE EXTENSIONS LIST VERVERS FUNCTIONS
...
// ################################################################################
// BUSY TRUNK LIST VERVERS FUNCTIONS
...
// ################################################################################
// BUSY EXTENSIONS LIST VERVERS FUNCTIONS
	...
	function busyhangup_send_redirect(taskvar,tasktrunkaction) 
		...
		if (tasktrunkaction == 'LISTEN')
			...
			if (listenvalue.match(regLa))
				...
				{
				bsr_continue=0;
				alert("Je kunt alleen Zap kanalen monitoren:\n" + listenvalue);
				}
			}
		if (bsr_continue == '1')
			{
...
// ################################################################################
// Check to see if user's extension is op a call right now, if it is, format and print it
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	pause();
					...
					if (live_calls > 0)
						{
						var live_calls_HTML = "<font face=\"Arial,Helvetica\"><B>LIVE GESPREKKEN OP DEZE TELEFOON:</B></font><BR><table width=100%><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\">CLIENT KANAAL</td><td><font class=\"log_title\">REMOTE KANAAL</td><td><font class=\"log_title\">RECORD</td><td><font class=\"log_title\">OPHANGEN</td><td><font class=\"log_title\">XFER</td><td><font class=\"log_title\">PARK</td></tr>";
				...
				//	alert(xmlhttp.responseText);
					...
					var conf_chan_array = check_conf_array[1].split(" ~");
					   if (live_conf_calls > 0)
						...
						var conv_start=-1;
						var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>LIVE GESPREKKEN IN DEZE CONFERENTIE:</B></font><BR><TABLE WIDTH=500><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">REMOTE KANAAL</TD><TD><font class=\"log_title\">OPHANGEN</TD><TD><font class=\"log_title\">XFER</TD></TR>";
...
//('BuilD','ADD','REMOVE','UP','DOWN')
	...
	//		alert(favorites_listEDIT);
		...
		var favedit_HTML = "<center><table cellpadding=5 cellspacing=5 width=750><tr><td colspan=2 align=center bgcolor=\"#DDDD99\"><B>BESCHIKBARE EXTENTIES</B></td><td align=center bgcolor=\"#CCCC99\"><B> FAVORIETEN</B></td></tr><tr><td bgcolor=\"#DDDD99\" height=380 width=200 valign=top><font class=\"ss_text\"><span id=FavSelectA>";
		loop_ct = 0;
		while (loop_ct < favlistCT)
			...
			if (favorites_listEDIT.match(regFAVlist))
				{
				// do not print extension, it's op the favorites list
				}
			else
				{
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT()
		...
		document.getElementById("LogouTBoxLink").innerHTML = "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&user=" + user + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "\">KLIK HIER OM OPNIEUW IN TE LOGGEN</a>\n";
...
<a href="#" onclick="activeext_force_refresh();return false;">VERVERS</a> | 
...
<td>Lokale Extenties <BR>
<a href="#" onclick="busyext_force_refresh();return false;">VERVERS</a> | 
<span id="busyext_order"><a href="#" onclick="busyext_order_desc();return false;">SORTEER</a> | </span>
</td></tr>
