...
$report_name = 'Agent Status Detalle';
$db_source = 'M';
...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		{
		$ASCII_text.="<BR><BR>Agent Status Detalle Report                     $NOW_TIME\n";
		$ASCII_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		$ASCII_text.="---------- AGENTE Detalles -------------\n\n";
		$GRAPH.="<BR><BR>Agent Status Detalle Report                     $NOW_TIME\n";
		$GRAPH.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		$GRAPH.="---------- AGENTE Detalles -------------";
		}
	else
		{
		$file_output .= "Agent Status Detalle Report                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>CALL STATS BREAKDOWN</caption><tr><th class='thgraph' scope='col'>ESTADO</th>";
...
echo "<INPUT TYPE=Submit NAME=REMITIR VALUE=REMITIR>$NWB#agent_status_detail$NWE\n";
