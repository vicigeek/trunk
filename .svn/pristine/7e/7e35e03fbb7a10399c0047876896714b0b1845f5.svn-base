...
//	echo $stmt="SELECT conf_exten,server_ip,extension from vicidial_conferences order by conf_exten";
    ...
    echo "<td><font size=1 color=white align=left><B>ID Campaña</B></td>";
    ...
    while ($vicidialconf_to_print > $o) {
        ...
        echo "<td align=center><font size=1><a href=\"$PHP_SELF?ADD=31&campaign_id=$row[0]\">MODIFICAR</a></td></tr>\n";
        $o++;
    }
...
if (($ADD==881) && ($qc_auth=='1')) {
    ...
    echo "<br>$campaign_id - {$row[1]} - Control de Calidad Queue<br>QC statuses: $qc_status_list\n";
    ...
    foreach ( $qc_statuses as $qc_status ) {
        ...
        while ($vicidialconf_to_print > $o) {
            if($o==0){
                ...
                echo "<td align=center><font size=1 color=white><B>UsuarioID</B></td></tr>\n";
            }
            $row=mysqli_fetch_row($rslt);
		if (preg_match("/1$|3$|5$|7$|9$/i", $o))
				{
