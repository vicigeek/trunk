...
if (!$group)
	...
	$ASCII_text="---------- PERFORMANCE Dettagliatas -------------\n";
	...
	$TOTALI_array[0]="TOTALI";
	$graph_TOTALI_array[0]+="TOTALI";
	...
	for ($q=0; $q<count($rpt_date_array); $q++) 
		...
		$CSV_header2.='"CALLS","SALES","CONVERSIONE RATE TO CALLS","SALES PER HOUR","TIME",';
		...
		while (list($key, $val)=each($agent_performance_array)) { # CYCLE THROUGH EACH USER
			...
			for ($k=0; $k<5; $k++) 
				...
				$graph_TOTALI_array[($array_offset+$k)]+=$agent_performance_array[$key][($array_offset+$k)];
				}
			$j++;
			}
		...
		for ($j=0; $j<count($graph_stats); $j++) 
			...
			$CALLS_graph[$q].="  <tr><td class='chart_td$class'>".$graph_stats[$j][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(600*$graph_stats[$j][1], $max_stats[1]))."' height='16' />".$graph_stats[$j][1]."</td></tr>";
			$SALES_graph[$q].="  <tr><td class='chart_td$class'>".$graph_stats[$j][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(600*$graph_stats[$j][2], $max_stats[2]))."' height='16' />".$graph_stats[$j][2]."</td></tr>";
			$SALECONV_graph[$q].="  <tr><td class='chart_td$class'>".$graph_stats[$j][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(600*$graph_stats[$j][3], $max_stats[3]))."' height='16' />".$graph_stats[$j][3]."</td></tr>";
			$SPH_graph[$q].="  <tr><td class='chart_td$class'>".$graph_stats[$j][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(600*$graph_stats[$j][4], $max_stats[4]))."' height='16' />".$graph_stats[$j][4]."</td></tr>";
			$TIME_graph[$q].="  <tr><td class='chart_td$class'>".$graph_stats[$j][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(600*$graph_stats[$j][5], $max_stats[5]))."' height='16' />".sec_convert($graph_stats[$j][5], 'H')."</td></tr>";
			}

		$CALLS_graph[$q].="  <tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".$graph_TOTALI_array[($array_offset)]."</th></tr>";
		$SALES_graph[$q].="  <tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".$graph_TOTALI_array[($array_offset+1)]."</th></tr>";
		$SALECONV_graph[$q].="  <tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".$graph_TOTALI_array[($array_offset+2)]."</th></tr>";
		$SPH_graph[$q].="  <tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".$graph_TOTALI_array[($array_offset+3)]."</th></tr>";
		$TIME_graph[$q].="  <tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".sec_convert($graph_TOTALI_array[($array_offset+4)], 'H')."</th></tr>";
		}
	...
	while (list($key, $val)=each($agent_performance_array)) {
		...
		for ($q=0; $q<count($rpt_date_array); $q++) 
			...
			$TOTALI_array[$x]=sprintf("%0.2f", MathZDC(100*$TOTALI_array[($x-1)], $TOTALI_array[($x-2)]));
			...
			$TOTALI_array[$x-1]=sprintf("%0.2f", MathZDC($TOTALI_array[($x-3)], MathZDC($TOTALI_array[$x], 3600))); # Go back to get last result
			}
		$ASCII_text.="\n";
		$CSV_text.="\n";
		}
	...
	$CSV_text.='"","TOTALI",';
	$ASCII_text.="| ".sprintf("%26s", $TOTALI_array[0])." ||";
	$GRAPH_text.=$GRAPH.$GRAPH2.$GRAPH3;

	for ($i=1; $i<count($TOTALI_array); $i++) 
		{
		$CSV_text.="\"$TOTALI_array[$i]\",";
		switch($i%5) 
			{
			case "1":
			case "2":
				$ASCII_text.=" ".sprintf("%5s", $TOTALI_array[$i])." |";
				break;
			case "3":
				$ASCII_text.=" ".sprintf("%10s", $TOTALI_array[$i])."% |";
				break;
			case "4":
				$ASCII_text.=" ".sprintf("%12s", $TOTALI_array[$i])." |";
				break;
			case "0":
				$ASCII_text.=" ".sprintf("%9s", sec_convert($TOTALI_array[$i], 'H'))." ||";
				break;
			}
		}
$ASCII_text.="\n";
