...
echo "<TITLE>Server-Specific Tiempo Real Report</TITLE></HEAD><BODY BGCOLOR=WHITE marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
...
echo "Agentes Time On Calls           $NOW_TIME    <a href=\"$PHP_SELF?server_ip=$server_ip&DB=$DB&reset_counter=$reset_counter&closer_display=$closer_display_reverse\">$closer_reverse_link</a> | <a href=\"./admin.php?ADD=999999\">INFORMES</a>\n\n";
...
$talking_to_print = mysqli_num_rows($rslt);
	...
	#	echo "  <SPAN class=\"blue\"><B>          </SPAN> - Llamadas de 5 min o más</B>\n";
	#	echo "  <SPAN class=\"purple\"><B>          </SPAN> - Llamadas de mas de 10 min </B>\n";
		}
	else
		{
		echo "+------------+------------+--------+-----------+---------------------+--------+----------+---------+\n";
		echo "  $agentcount agentes registrados en el servidor $server_ip\n\n";

		echo "  <SPAN class=\"yellow\"><B>          </SPAN> - Agentes Pausados</B>\n";
		echo "  <SPAN class=\"blue\"><B>          </SPAN> - Llamadas de 5 min o más</B>\n";
		echo "  <SPAN class=\"purple\"><B>          </SPAN> - Llamadas de mas de 10 min </B>\n";
		}
...
echo "----------------------------------------------------------------------------------------";
echo "\n\n";
echo "Server-Specific Tiempo Real Report        TRUNK SHORT: $balanceSHORT          $NOW_TIME\n\n";
echo "+---------------------+--------+--------------+--------------------+----------+---------+\n";
...
$parked_to_print = mysqli_num_rows($rslt);
	...
	#	if ($call_time_M_int >= 6) {$G='<SPAN class="red"><B>'; $EG='</B></SPAN>';}
		...
		echo "  <SPAN class=\"green\"><B>          </SPAN> - LLAMADAS EN ESPERA ACTIVAS</B>\n";
