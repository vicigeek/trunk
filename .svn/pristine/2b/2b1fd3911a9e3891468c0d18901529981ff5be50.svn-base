...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- バージョン: $version     ビルド: $build    ACTION: $ACTION   server_ip: $server_ip-->\n";
	echo "<title>マネージャーが送信する: ";
...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten 有効ではない or queryCID $queryCID 有効ではない, Originate 挿入されていないコマンド\n";
		}
	else
		{
		...
		echo "Originate 送信されたコマンド Exten $exten チャンネル $channel オン$server_ip\n";
		}
	}
...
if ($ACTION=="OriginateName")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		{
		$channel_live=0;
		echo "これらの変数の1つ 有効ではない:\n";
		echo "Channel $channel 2文字以上でなければなりません。\n";
		echo "queryCID $queryCID 14文字以上でなければなりません。\n";
		echo "extenName $extenName 送信されなければなりません。\n";
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "\nOriginateName Action 送信されない\n";
		}
	else
		{
...
if ($ACTION=="OriginateNameVmail")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		echo "queryCID $queryCID 14文字以上でなければなりません。\n";
		echo "extenName $extenName 送信されなければなりません。\n";
		echo "exten $exten 送信されなければなりません。\n";
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "\nOriginateNameVmail Action 送信されない\n";
		}
	else
		{
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten 有効ではない or queryCID $queryCID 有効ではない, Originate 挿入されていないコマンド\n";
		}
	else
		{
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR 他のエージェントセッションにダイアルすることは許可されていません。 $exten\n";
			exit;
			}
		...
		echo "Originate 送信されたコマンド Exten $exten チャンネル $channel オン$server_ip |$account|$variable|\n";
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten 有効ではない or ext_context $ext_context or queryCID $queryCID 有効ではない, Hangup 挿入されていないコマンド\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
if ($ACTION=="Hangup")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) )
		{
		$channel_live=0;
		echo "Channel $channel 有効ではない or queryCID $queryCID 有効ではない, Hangup 挿入されていないコマンド\n";
		}
	else
		{
...
#			}
		if ( ($auto_dial_level > 0) and (strlen($CalLCID)>2) and (strlen($exten)>2) and ($secondS > 0))
			...
			if ($rowx[0]==0)
				{
				echo "Call $CalLCID $channel ライブではない オン$call_server_ip, Checking Live チャンネル...\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02012',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if ($row[0]==0)
					{
					$channel_live=0;
					echo "Channel $channel ライブではない オン$call_server_ip, Hangup 挿入されていないコマンド $rowx[0]\n$stmt\n";
					}
				else
					{
					echo "$stmt\n";
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02083',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			if ($row[0] > 0)
				{
				$channel_live=0;
				echo "Channel $channel in use by another agent オン$call_server_ip, Hangup 挿入されていないコマンド $rowx[0]\n$stmt\n";
		...
		if ($channel_live==1)
			...
			echo "Hangup 送信されたコマンド チャンネル $channel オン$call_server_ip\n";
			}
		}
	}
...
if ($ACTION=="RedirectVD")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($campaign)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($uniqueid)<2) or (strlen($lead_id)<1) )
		...
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "auto_dial_level $auto_dial_level 送信されなければなりません。\n";
		echo "campaign $campaign 送信されなければなりません。\n";
		echo "uniqueid $uniqueid 送信されなければなりません。\n";
		echo "lead_id $lead_id 送信されなければなりません。\n";
		echo "\nRedirectVD Action 送信されない\n";
		}
	else
		{
...
if ($ACTION=="RedirectToPark")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($extenName)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($parkedby)<1) )
		...
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "parkedby $parkedby 送信されなければなりません。\n";
		echo "\nRedirectToPark Action 送信されない\n";
		}
	else
		{
		if ($stage!="2NDXfeR")
...
if ($ACTION=="RedirectFromPark")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) )
		...
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "\nRedirectFromPark Action 送信されない\n";
		}
	else
		{
		if ($stage!="2NDXfeR")
...
if ($ACTION=="RedirectNameVmail")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "\nRedirectNameVmail Action 送信されない\n";
		}
	else
		{
		$stmt="SELECT voicemail_id FROM phones where server_ip = '$server_ip' and extension='$extenName';";
...
if ($ACTION=="RedirectXtraCXNeW")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		echo "これらの変数の1つ 有効ではない:\n";
		echo "Channel $channel 2文字以上でなければなりません。\n";
		echo "ExtraChannel $extrachannel 2文字以上でなければなりません。\n";
		echo "queryCID $queryCID 14文字以上でなければなりません。\n";
		echo "exten $exten 送信されなければなりません。\n";
		echo "ext_context $ext_context 送信されなければなりません。\n";
		echo "ext_priority $ext_priority 送信されなければなりません。\n";
		echo "\nRedirect Action 送信されない\n";
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference オン$server_ip, Redirect 挿入されていないコマンド\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "空の会議を見つけることができません。 オン$server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveX=0;
				echo "Channel $channel ライブではない オン$call_server_ip, Redirect 挿入されていないコマンド\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel ライブではない オン$call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveY=0;
				echo "Channel $channel ライブではない オン$server_ip, Redirect 挿入されていないコマンド\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel ライブではない オン$server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($rowx[0] < 1)
				{
				$channel_liveY=0;
				echo "No Local agent to send call to, Redirect 挿入されていないコマンド\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				echo "RedirectXtraCX 送信されたコマンド チャンネル $channel オン$call_server_ip and \nHungup $extrachannel オン$server_ip\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel オン$call_server_ip, Hungup $extrachannel オン$server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channel オン$server_ip";}
			}
...
if ($ACTION=="RedirectXtraNeW")
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			echo "ext_context $ext_context 送信されなければなりません。\n";
			echo "ext_priority $ext_priority 送信されなければなりません。\n";
			echo "session_id $session_id 送信されなければなりません。\n";
			echo "\nRedirect Action 送信されない\n";
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename))
				{
				if ($WeBRooTWritablE > 0)
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					echo "RedirectXtra 送信されたコマンド チャンネル $channel and \nExtraChannel $extrachannel\n to $exten オン$server_ip\n";
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $exten オン$server_ip";}
					}
				else
					{
					$S='*';
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02059',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					echo "RedirectXtra 送信されたコマンド チャンネル $channel オン$call_server_ip and \nExtraChannel $extrachannel\n to $exten オン$server_ip\n";
...
if ($ACTION=="Redirect")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		if ($channel_live==1)
			...
			echo "Redirect 送信されたコマンド チャンネル $channel オン$server_ip\n";
			}
		}
	}
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel 有効ではない or queryCID $queryCID 有効ではない or filename: $filename 有効ではない, $ACTION 挿入されていないコマンド\n";
		}
	else
		{
		$stmt="SELECT count(*) FROM live_channels where server_ip = '$server_ip' and channel='$channel';";
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_live=0;
				echo "Channel $channel ライブではない オン$server_ip, $ACTION 挿入されていないコマンド\n";
				}	
			}
		if ($channel_live==1)
			{
			...
			echo "$ACTION 送信されたコマンド チャンネル $channel オン$server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n";
			}
		}
	}
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel 有効ではない or exten $exten 有効ではない or filename: $filename 有効ではない, $ACTION 挿入されていないコマンド\n";
		}
	else
		{
		$VDvicidial_id='';
		...
		##### StopMonitorConf steps #####
			...
			# find and hang up all recordings going オンin this conference # and extension = '$exten' 
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "会議 $exten, Stage $stage 有効ではない or queryCID $queryCID 有効ではない, Originate 挿入されていないコマンド\n";
		}
	else
		{
		...
		echo "Volume 送信されたコマンド 会議 $exten, Stage $stage チャンネル $channel オン$server_ip\n";
		}
	}
