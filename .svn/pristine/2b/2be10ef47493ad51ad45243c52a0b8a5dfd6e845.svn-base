...
$report_name = 'Agent Ωρα Λεπτομέρειες';
$db_source = 'M';
...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		{
		$ASCII_text.="\nAgent Ωρα Λεπτομέρειες                     $NOW_TIME\n";
		$ASCII_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";

		$GRAPH.="Agent Ωρα Λεπτομέρειες                     $NOW_TIME\n";
		$GRAPH.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		}
	else
		{
		$file_output .= "Agent Ωρα Λεπτομέρειες                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>ΧΕΙΡΙΣΤΗΣTIME BREAKDOWN</caption><tr><th class='thgraph' scope='col'>ΚΑΤΑΣΤΑΣΗ</th>";
	...
	$AGENTTIME_graph=$graph_header."<th class='thgraph' scope='col'>ΧΕΙΡΙΣΤΗΣTIME</th></tr>";
	...
	# Εμφάνισηs park info - need to run regardless so HTML view will populate
	$park_array=array();
	...
	if ($file_download < 1)
		{
		$ASCII_text.="ΧΕΙΡΙΣΤΗΣTIME BREAKDOWN:\n";
		$ASCII_text.="+-----------------+----------+----------+------------+------------$park_HEADER_DIV+------------+------------+------------+------------+------------+------------+------------+------------+------------+------------+------------+   +$sub_statusesHEAD\n";
		$ASCII_text.="| <a href=\"$LINKbase&stage=NAME\">USER NAME</a>       | <a href=\"$LINKbase&stage=ID\">ID</a>       | <a href=\"$LINKbase&stage=LEADS\">CALLS</a>    | <a href=\"$LINKbase&stage=TCLOCK\">TIME CLOCK</a> | <a href=\"$LINKbase&stage=TIME\">ΧΕΙΡΙΣΤΗΣTIME</a> |$park_HEADER WAIT       | WAIT %     | TALK       | TALK TIME %| DISPO      | DISPOTIME %| PAUSE      | PAUSETIME %| DEAD       | DEAD TIME %| CUSTOMER   |   |$sub_statusesHTML\n";
		$ASCII_text.="+-----------------+----------+----------+------------+------------$park_HEADER_DIV+------------+------------+------------+------------+------------+------------+------------+------------+------------+------------+------------+   +$sub_statusesHEAD\n";
		}
	else
		{
		$file_output .= "USER,ID,CALLS,TIME CLOCK,ΧΕΙΡΙΣΤΗΣTIME$park_HEADER_CSV,WAIT,WAIT %,TALK,TALK TIME %,DISPO,DISPO TIME %,PAUSE,PAUSE TIME %,DEAD,DEAD TIME %,CUSTOMER$sub_statusesFILE\n";
		}
...
echo "</SELECT>\n";
echo "</TD><TD VALIGN=TOP>Ομάδες Χρήστη:<BR>";
echo "<SELECT SIZE=5 NAME=user_group[] multiple>\n";

if  (preg_match('/\-\-ALL\-\-/',$user_group_string))
	{echo "<option value=\"--ALL--\" selected>-- ALL ΟΜΑΔΕΣ ΧΡΗΣΤΗ --</option>\n";}
else
	{echo "<option value=\"--ALL--\">-- ALL ΟΜΑΔΕΣ ΧΡΗΣΤΗ --</option>\n";}
$o=0;
...
echo "<INPUT TYPE=Submit NAME=ΕΠΙΒΕΒΑΙΩΣΗ VALUE=ΥΠΟΒΑΛΛΩ>$NWB#agent_time_detail$NWE\n";
echo "</TD><TD VALIGN=TOP> &nbsp; &nbsp; &nbsp; &nbsp; ";

echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;\n";
echo " <a href=\"$LINKbase&stage=$stage&file_download=1\">ΛΗΨΗ</a> | \n";
