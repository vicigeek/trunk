...
$report_name='enumerar listas de Marcación Report';
...
echo "<TITLE>enumerar listas de Marcación Report</TITLE></HEAD><BODY BGCOLOR=WHITE marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
...
if (!$group)
	{
	echo "\n\n";
	echo "POR FAVOR SELECCIONE ARRIBA UNA CAMPAÑA Y CLICK EN REMITIR\n";
	}

else
	{
	echo "Enumerar Lista de Marcación Actualmente activa                      $NOW_TIME\n";
	...
	echo "Total de contactos en Lista de Marcación Actual:       $TOTALcalls\n";
	...
	echo "+------+--------+-----------+------------+------------+-------+--------+-------+--------+-------+-------+----------------------+\n";
	echo "|ORDER |PRIORIDAD| LEAD ID   | LISTA  ID    | PHONE NUM  | STATE | STATUS | COUNT | GMT    | ALT   | SOURCE| VENDOR LEAD CODE     |\n";
	echo "+------+--------+-----------+------------+------------+-------+--------+-------+--------+-------+-------+----------------------+\n";
	...
	while ($i < $users_to_print)
		...
		if ($LOGadmin_hide_lead_data != '0')
			{
			if ($DB > 0) {echo "OCULTARLEADDATA|$row[2]|$LOGadmin_hide_lead_data|\n";}
			if (strlen($row[2]) > 0)
