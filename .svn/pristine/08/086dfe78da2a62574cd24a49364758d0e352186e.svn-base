...
$report_name = 'Liste Esiti Campagna Report';
$db_source = 'M';
...
if (strlen($group[0]) < 1)
	...
	{
	$OUToutput = '';
	$OUToutput .= "Elenchi delle campagne di stato Statistiche                             $NOW_TIME\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "\n";
	$OUToutput .= "---------- STATUS FLAGS SUMMARY:    (and % of leads in selected lists)     <a href=\"$PHP_SELF?DB=$DB$groupQS&INVIA=$INVIA&file_download=2\">DOWNLOAD</a>\n";
	$OUToutput .= "+------------------+------------+----------+\n";
	$OUToutput .= "| Umani Risposta     | $HA_count |  $HA_percent% |\n";
	...
	$CSV_text2 .= "\"Umani Risposta\",\"$HA_count\",\"$HA_percent%\"\n";
	...
	$GRAPH.="  <tr>\n";
	$GRAPH.="	<td class=\"chart_td first\">Umani Risposta</td>\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- CUSTOM STATUS CATEGORIA STATS     <a href=\"$PHP_SELF?DB=$DB$groupQS&INVIA=$INVIA&file_download=3\">DOWNLOAD</a>\n";
	$OUToutput .= "+----------------------+------------+--------------------------------+\n";
	$OUToutput .= "| CATEGORIA             | CALLS      | DESCRIZIONE                    |\n";
	$OUToutput .= "+----------------------+------------+--------------------------------+\n";

	$CSV_text3.="\"CUSTOM STATUS CATEGORIA STATS\"\n";
	$CSV_text3.="\"CATEGORIA\",\"CALLS\",\"DESCRIZIONE\"\n";
	...
	$TOTALleads = 0;
	$OUToutput .= "\n";
	$OUToutput .= "---------- PER LIST DETAIL STATS     <a href=\"$PHP_SELF?DB=$DB$groupQS&INVIA=$INVIA&file_download=4\">DOWNLOAD</a>\n";
	$OUToutput .= "\n";
	...
	while ($i < $listids_to_print)
		...
		$STATUS_graph=$graph_header."<tr><th class='thgraph' scope='col'>ESITO</th><th class='thgraph' scope='col'>CUENTA</th></tr>";

		$CSV_text4.="\"ID LISTA: $LISTIDlists[$i]\",\"$LISTIDlist_names[$i]\",\"$LISTIDlist_active[$i]\"\n";
		...
		$OUToutput .= "|   Risposta Umana:       $HA_count    $HA_percent%                   |\n";
		$OUToutput .= "|   Sale:               $SALE_count    $SALE_percent%                   |\n";
		$OUToutput .= "|   DNC:                $DNC_count    $DNC_percent%                   |\n";
		$OUToutput .= "|   Customer Contact:   $CC_count    $CC_percent%                   |\n";
		$OUToutput .= "|   Not Interested:     $NI_count    $NI_percent%                   |\n";
		...
		$FLAGS_graph.="  <tr><td class='chart_td first'>RISPOSTA UMANA</td><td nowrap class='chart_td value first'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$HA_count, $max_flags))."' height='16' />$HA_count ($HA_percent%)</td></tr>";
		...
		$CSV_text4.="\"Risposta Umana:\",\"$HA_count\",\"$HA_percent%\"\n";
		...
		$CSV_text4.="\"Servicios repetidos Programar:\",\"$SC_count\",\"$SC_percent%\"\n\n";
