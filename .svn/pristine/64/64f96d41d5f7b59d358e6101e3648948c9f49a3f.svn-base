...
<title>QC AlterarLead: <?php echo "$lead_id - ".urldecode($lead_name).urldecode(" (Campanha: $campaign_id - $campaign_name)"); ?></title>
...
echo "<CENTER><FONT FACE='Courier' COLOR=BLACK SIZE=3><a href=\"../admin.php?ADD=881&campaign_id=$campaign_id\">QC CAMPANHA $campaign_id</a>: Lead $lead_id - ".urldecode($lead_name)."<BR>\n";
...
if ($end_call > 0) {
	...
	$rslt=mysql_to_mysqli($stmt, $link);
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        $row=mysqli_fetch_assoc($rslt);
        //QUALITY CONTROL CHANGE LOG BEGIN - CHANGE VERSÃO (VIEW VERSÃO IS BELOW, WHICH CREATES THE RECORD, THIS ONE MERELY MODIFIES THE EXISTING RECORD)
        $new_record=$row;
	...
	echo "<br>Informação Alterada<BR><BR>\n";
	echo "<i><small><a href=\"$PHP_SELF?lead_id=$lead_id&DB=$DB\">Go back to re-modify this QC lead</a></small></i><BR><BR><BR>\n";
        echo "<CENTER><B><FONT FACE='Courier' COLOR=BLACK SIZE=3><a href=\"../admin.php?ADD=881&campaign_id=$campaign_id\">Proceed to QC CAMPANHA $campaign_id Queue</a></B><BR><BR><B><I>Callback Information:</I></B>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	echo "<tr><td colspan=2>Fronter: <A HREF=\"user_stats.php?user=$tsr\">$tsr</A> &nbsp; &nbsp; ID da Lista: $list_id &nbsp; &nbsp; Called Count: $called_count</td></tr>\n";
	...
	while ($row_count > $o)
		...
		$rowx=mysqli_fetch_row($rslt);
             	echo "<tr><td align=right>$rowx[0] : </td><td align=left><hr>$rowx[1]<br><b>ID da Lista:</b> $rowx[2]; <b>ID da Campanha:</b> $rowx[3]<br>$rowx[4]</td></tr>\n";
		$o++;
		}
...
<input type=hidden name=viewtime value='<?php echo $STARTtime; ?>' />
	...
	echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
	echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=center><font size=2>OLD STATUS</td><td align=center><font size=2>NEW STATUS</td><td align=center><font size=2>QC USER</td><td align=center><font size=2>AGENT</td><td align=center><font size=2>CAMPANHA</td><td align=center><font size=2>LIST</td><td align=center><font size=2>ELAPSED</td><td align=center><font size=2>&nbsp;</td></tr>\n";
		echo "$qc_agent_log\n";
	echo "</TABLE>\n";
	echo "<BR><BR>\n";
