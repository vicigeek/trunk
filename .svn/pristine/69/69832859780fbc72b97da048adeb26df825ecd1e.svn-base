...
<title>ADMINISTRATION: Editar Registro Reloj Asistencia
<?php
...
<CENTER>
<TABLE WIDTH=720 BGCOLOR=#D9E6FE cellpadding=2 cellspacing=0><TR BGCOLOR=#015B91><TD ALIGN=LEFT><FONT FACE="ARIAL,HELVETICA" COLOR=WHITE SIZE=2><B>Editar Registro Reloj Asistencia for <?php echo $user ?></TD><TD ALIGN=RIGHT> &nbsp; </TD></TR>
...
if ( ($invalid_record < 1) or (strlen($timeclock_id)<1) )
	...
	if ($stage == "edit_TC_log")
		...
		if ( ($LOGINepoch <= $PREVevent_epoch) || ($LOGOUTepoch >= $NEXTevent_epoch) )
			{
			echo "ERROR-Hay un problema con los datos que ha introducido, por favor, volver<BR>\n";
			echo "Una sesión en Reloj de Asistencia no puede superponerse a otra sesion de Asistencia<BR>\n";
			echo "$LOGINepoch<BR>\n";
			echo "$LOGOUTepoch<BR>\n";
	...
	#	$LOGOUTevent_id =	$tcid_link;
		...
		echo "<input type=button name=go_submit id=go_submit value=REMITIR onclick=\"run_submit();\"><BR></form>\n";
		echo "</TD></TR></TABLE>\n";
		echo "<BR><BR>\n";
		}
