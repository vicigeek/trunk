...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten is niet geldig or queryCID $queryCID is niet geldig, Originate commando niet uitgevoerd\n";
		}
	else
		{
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten is niet geldig or queryCID $queryCID is niet geldig, Originate commando niet uitgevoerd\n";
		}
	else
		{
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR Je hebt geen rechten om te bellen naar andere agent sessies $exten\n";
			exit;
			}
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten is niet geldig or ext_context $ext_context or queryCID $queryCID is niet geldig, Hangup commando niet uitgevoerd\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
#			}
		if ( ($auto_dial_level > 0) and (strlen($CalLCID)>2) and (strlen($exten)>2) and ($secondS > 0))
			...
			if ($rowx[0]==0)
				{
				echo "Call $CalLCID $channel is niet live op $call_server_ip, Checking Live Kanaal...\n";
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference op $server_ip, Redirect commando niet uitgevoerd\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Kan geen lege conferentie vinden op $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel is niet live op $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel is niet live op $server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($rowx[0] < 1)
				{
				$channel_liveY=0;
				echo "No Local agent to send call to, Redirect commando niet uitgevoerd\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel op $call_server_ip, Hungup $extrachannel op $server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channel op $server_ip";}
			}
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $exten op $server_ip";}
					}
				else
					{
					$S='*';
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		...
		##### StopMonitorConf steps #####
			...
			# find and hang up all recordings going op in this conference # and extension = '$exten' 
		...
		#	$rec_count = intval(mysqli_num_rows($rslt) / 2);
			...
			echo "$ACTION commando verstuurd voor Kanaal $channel op $server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n De Opname loopt maximaal 60 minuten\n";
		}
	}
