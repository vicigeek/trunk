...
<title>ADMINISTRATION: Modificación de Registro de Contacto</title>
...
<?php 
echo "<a href=\"./admin.php?ADD=100\">ADMINISTRATION</a>: Modificación de Registro de Contacto<BR>\n";
...
if ($lead_id == 'NEW')
	...
	if ($affected_rows > 0)
		...
		echo "El prospecto se ha agregado: $lead_id<BR><BR>\n";
		$end_call=0;
		}
	else
		{echo "ERROR: Contacto no agregado, por favor, retornar y verificar la información incorporada<BR><BR>\n";}
	}
else
	{
...
if ($end_call > 0)
	...
	echo "Información modificada<BR><BR>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	if ($LOGadmin_hide_phone_data != '0')
		{
		if ($DB > 0) {echo "OCULTARPHONEDATA|$row[11]|$LOGadmin_hide_phone_data|\n";}
		$phone_temp = $row[11];
	...
	if ($LOGadmin_hide_lead_data != '0')
		{
		if ($DB > 0) {echo "OCULTARLEADDATA|$row[5]|$row[6]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";}
		if (strlen($row[5]) > 0)
	...
	if ($lead_id == 'NEW')
		{echo "<br><b>Agregar Contacto</B>\n";}
	else
		{echo "<br>Información de la llamada: $first_name $last_name - $phone_number\n";}
	...
	echo "<tr><td colspan=2>Lead ID: $lead_id &nbsp; &nbsp; ID de Lista: $list_id</td></tr>\n";
	...
	if ($LOGadmin_hide_lead_data != '0')
		...
		echo "<tr><td align=right>País    : </td><td align=left>$country_code</td></tr>\n";
		...
		echo "<tr><td align=right>País    : </td><td align=left><input type=text name=country_code size=3 maxlength=3 value=\"".htmlentities($country_code)."\"></td></tr>\n";
	...
	if ($lead_id != 'NEW') 
		...
		echo "<tr bgcolor=#B6D3FC><td align=left>Modificarvicidial log </td><td align=left><input type=checkbox name=modify_logs value=\"1\" CHECKED></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=left>Modificaragent log </td><td align=left><input type=checkbox name=modify_agent_logs value=\"1\" CHECKED></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=left>Modificarcloser log </td><td align=left><input type=checkbox name=modify_closer_logs value=\"1\"></td></tr>\n";
	...
	if ( ($LOGadmin_hide_lead_data == '0') or ($lead_id == 'NEW') )
		{
		echo "<tr><td colspan=2 align=center><input type=submit name=submit value=\"REMITIR\"></td></tr>\n";
		}
	echo "</table></form>\n";
	echo "<BR><BR><BR>\n";
	...
	if ($lead_id != 'NEW') 
		...
		echo "Callback Detalles:<BR><CENTER>\n";
		if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') or ($scheduled_callback == 'Y') )
			...
			if ($CB_to_print>0)
				...
				if ($rowx[9] == 'USERONLY')
					...
					echo "<input type=hidden name=lead_id value=\"$lead_id\">\n";
					echo "<input type=hidden name=callback_id value=\"$rowx[0]\">\n";
					echo "New Callback Owner UsuarioID: <input type=text name=CBuser size=18 maxlength=20 value=\"$rowx[8]\"> \n";
				...
				<TR BGCOLOR="#E6E6E6">
				<TD align=center colspan=2>
				<FONT FACE="ARIAL,HELVETICA">Comentarios: 
				...
				<input type=button value="REMITIR" name=smt id=smt onClick="submit_form()">
				</TD>
				</TR>
			...
			{
			echo "<BR>Si desea cambiar este contacto a una remarcación programada, primero cambie la disposición de CBHOLD, a continuación seleccione remitir, y usted será capaz de establecer la fecha y hora de remarcación.<BR>\n";
			}
		echo "</TD></TR></TABLE>\n";
		...
		if ($c > 0)
			{
			echo "<B>NUMEROS ALTERNOS EXTENDIDOS PARA ESTE CONTACTO:</B>\n";
		...
		if ($custom_fields_enabled > 0)
			...
			$stmt="MOSTRAR TABLAS LIKE \"custom_$CLlist_id\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_to_mysqli($stmt, $link);
			...
			if ($tablecount_to_print > 0) 
				...
				if ($fieldscount_to_print > 0) 
					...
					echo "<B>CAMPOS PERSONALIZADOS PARA ESTE CONTACTO:</B><BR>\n";
		...
		echo "<B>LLAMADAS A ESTE CONTACTO:</B>\n";
		echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
		echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=left><font size=2>LENGTH</td><td align=left><font size=2> STATUS</td><td align=left><font size=2> TSR</td><td align=right><font size=2> CAMPAÑA</td><td align=right><font size=2> LIST</td><td align=right><font size=2> LEAD</td><td align=right><font size=2> HANGUP REASON</td><td align=right><font size=2> PHONE</td></tr>\n";
		...
		echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
		echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=left><font size=2>LENGTH</td><td align=left><font size=2> STATUS</td><td align=left><font size=2> TSR</td><td align=right><font size=2> CAMPAÑA</td><td align=right><font size=2> LIST</td><td align=right><font size=2> LEAD</td><td align=right><font size=2> WAIT</td><td align=right><font size=2> HANGUP REASON</td></tr>\n";
		...
		echo "<tr><td><font size=1># </td><td align=left><font size=2> CAMPAÑA</td><td><font size=2>DATE/TIME </td><td align=left><font size=2>CALL MENU </td><td align=left><font size=2> &nbsp; ACTION</td></tr>\n";
	...
	##### vicidial agent outbound calls for this time period #####
		...
		if ($allow_emails>0) 
			...
			echo "<TABLE width=750 cellspacing=1 cellpadding=1>\n";
			echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=right><font size=2> USER</td><td align=right><font size=2> CAMPAÑA</td><td align=left><font size=2>EMAIL TO</td><td align=left><font size=2> MESSAGE</td><td align=right><font size=2> ATTACHMENTS</td></tr>\n";
		...
		if ($admin_display > 0)
			{
			echo "<a href=\"./admin.php?ADD=720000000000000&stage=$lead_id&category=LEADS\">Click here to see Lead Modificarchanges to this lead</a>\n";
			}
                //Desplegar link to QC if user has QC permissions
