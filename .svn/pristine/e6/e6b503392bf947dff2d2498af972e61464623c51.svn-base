...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten er ikke valid or queryCID $queryCID er ikke valid, Originate ordre ikke givet\n";
		}
	else
		{
		...
		echo "Originate ordre sent for Exten $exten Kanal $channel på $server_ip\n";
		}
	}
...
if ($ACTION=="OriginateName")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		{
		$channel_live=0;
		echo "En af disse variabler er ikke valid:\n";
		echo "Channel $channel skal være større end 2 tegn\n";
		echo "queryCID $queryCID skal være større end 14 tegn\n";
		echo "extenName $extenName skal stilles ind\n";
		echo "ext_context $ext_context skal stilles ind\n";
		echo "ext_priority $ext_priority skal stilles ind\n";
...
if ($ACTION=="OriginateNameVmail")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		echo "queryCID $queryCID skal være større end 14 tegn\n";
		echo "extenName $extenName skal stilles ind\n";
		echo "exten $exten skal stilles ind\n";
		echo "ext_context $ext_context skal stilles ind\n";
		echo "ext_priority $ext_priority skal stilles ind\n";
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten er ikke valid or queryCID $queryCID er ikke valid, Originate ordre ikke givet\n";
		}
	else
		{
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR Du har ikke tilladelse at ring til andre agenters sessioner $exten\n";
			exit;
			}
		...
		echo "Originate ordre sent for Exten $exten Kanal $channel på $server_ip |$account|$variable|\n";
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten er ikke valid or ext_context $ext_context or queryCID $queryCID er ikke valid, Hangup ordre ikke givet\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
if ($ACTION=="Hangup")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) )
		{
		$channel_live=0;
		echo "Channel $channel er ikke valid or queryCID $queryCID er ikke valid, Hangup ordre ikke givet\n";
		}
	else
		{
...
#			}
		if ( ($auto_dial_level > 0) and (strlen($CalLCID)>2) and (strlen($exten)>2) and ($secondS > 0))
			...
			if ($rowx[0]==0)
				{
				echo "Call $CalLCID $channel er ikke aktiv på $call_server_ip, Checking Live Kanal...\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02012',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if ($row[0]==0)
					{
					$channel_live=0;
					echo "Channel $channel er ikke aktiv på $call_server_ip, Hangup ordre ikke givet $rowx[0]\n$stmt\n";
					}
				else
					{
					echo "$stmt\n";
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02083',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			if ($row[0] > 0)
				{
				$channel_live=0;
				echo "Channel $channel in use by another agent på $call_server_ip, Hangup ordre ikke givet $rowx[0]\n$stmt\n";
		...
		if ($channel_live==1)
			...
			echo "Hangup ordre sent for Kanal $channel på $call_server_ip\n";
			}
		}
	}
...
if ($ACTION=="RedirectVD")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($campaign)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($uniqueid)<2) or (strlen($lead_id)<1) )
		...
		echo "ext_context $ext_context skal stilles ind\n";
		echo "ext_priority $ext_priority skal stilles ind\n";
		echo "auto_dial_level $auto_dial_level skal stilles ind\n";
		echo "campaign $campaign skal stilles ind\n";
		echo "uniqueid $uniqueid skal stilles ind\n";
		echo "lead_id $lead_id skal stilles ind\n";
...
if ($ACTION=="RedirectToPark")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($extenName)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($parkedby)<1) )
		...
		echo "ext_context $ext_context skal stilles ind\n";
		echo "ext_priority $ext_priority skal stilles ind\n";
		echo "parkedby $parkedby skal stilles ind\n";
...
if ($ACTION=="RedirectXtraCXNeW")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		echo "En af disse variabler er ikke valid:\n";
		echo "Channel $channel skal være større end 2 tegn\n";
		echo "ExtraChannel $extrachannel skal være større end 2 tegn\n";
		echo "queryCID $queryCID skal være større end 14 tegn\n";
		echo "exten $exten skal stilles ind\n";
		echo "ext_context $ext_context skal stilles ind\n";
		echo "ext_priority $ext_priority skal stilles ind\n";
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference på $server_ip, Redirect ordre ikke givet\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Kan ikke finde en tom konference på $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveX=0;
				echo "Channel $channel er ikke aktiv på $call_server_ip, Redirect ordre ikke givet\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel er ikke aktiv på $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_liveY=0;
				echo "Channel $channel er ikke aktiv på $server_ip, Redirect ordre ikke givet\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel er ikke aktiv på $server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($rowx[0] < 1)
				{
				$channel_liveY=0;
				echo "No Local agent to send call to, Redirect ordre ikke givet\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				echo "RedirectXtraCX ordre sent for Kanal $channel på $call_server_ip and \nHungup $extrachannel på $server_ip\n";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel på $call_server_ip, Hungup $extrachannel på $server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channel på $server_ip";}
			}
...
if ($ACTION=="RedirectXtraNeW")
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			echo "ext_context $ext_context skal stilles ind\n";
			echo "ext_priority $ext_priority skal stilles ind\n";
			echo "session_id $session_id skal stilles ind\n";
			echo "\nRedirect Action no enviado\n";
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename))
				{
				if ($WeBRooTWritablE > 0)
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					echo "RedirectXtra ordre sent for Kanal $channel and \nExtraChannel $extrachannel\n to $exten på $server_ip\n";
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $exten på $server_ip";}
					}
				else
					{
					$S='*';
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02059',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					echo "RedirectXtra ordre sent for Kanal $channel på $call_server_ip and \nExtraChannel $extrachannel\n to $exten på $server_ip\n";
...
if ($ACTION=="Redirect")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		if ($channel_live==1)
			...
			echo "Redirect ordre sent for Kanal $channel på $server_ip\n";
			}
		}
	}
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel er ikke valid or queryCID $queryCID er ikke valid or filename: $filename er ikke valid, $ACTION ordre ikke givet\n";
		}
	else
		{
		$stmt="SELECT count(*) FROM live_channels where server_ip = '$server_ip' and channel='$channel';";
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				{
				$channel_live=0;
				echo "Channel $channel er ikke aktiv på $server_ip, $ACTION ordre ikke givet\n";
				}	
			}
		if ($channel_live==1)
			{
			...
			echo "$ACTION ordre sent for Kanal $channel på $server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n";
			}
		}
	}
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		{
		$channel_live=0;
		echo "Channel $channel er ikke valid or exten $exten er ikke valid or filename: $filename er ikke valid, $ACTION ordre ikke givet\n";
		}
	else
		{
		$VDvicidial_id='';
		...
		#	$rec_count = intval(mysqli_num_rows($rslt) / 2);
			...
			echo "$ACTION ordre sent for Kanal $channel på $server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n INDSPILNING VARER OPTIL 60 MINUTER\n";
		}
	}
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "konference $exten, Stage $stage er ikke valid or queryCID $queryCID er ikke valid, Originate ordre ikke givet\n";
		}
	else
		{
		...
		echo "Volume ordre sent for konference $exten, Stage $stage Kanal $channel på $server_ip\n";
		}
	}
