...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<TABLE><TR><TD></TD>\n";
	echo "<!-- ILPA -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_br/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Brazil <img src=\"../agc/images/br.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- VERSÃO: $version     CONFIGURAÇÃO: $build      ADD: $ADD-->\n";

if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>astGUIclient web client: Login do Ramal</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<TABLE><TR><TD></TD>\n";
echo "<!-- ILPA -->\n";
...
echo "<BR><BR><BR><CENTER><TABLE WIDTH=360 CELLPADDING=0 CELLSPACING=0 BGCOLOR=\"#CCC2E0\"><TR BGCOLOR=WHITE>";
echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc/images/agc_tab_astguiclient.gif\" Border=0></TD>";
echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Login do Ramal </TD>";
echo "</TR>\n";
echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
echo "<TR><TD ALIGN=RIGHT>Login do Ramal: </TD>";
echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=phone_login SIZE=10 maxlength=20 VALUE=\"$phone_login\"></TD></TR>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_br/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Brazil <img src=\"../agc/images/br.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Login do Ramal </TD>";
	echo "</TR>\n";
	echo "<TR><TD ALIGN=CENTER COLSPAN=2><font size=1> &nbsp; <BR><FONT SIZE=3>Desculpe seu login de ramal e senha não estão ativos nesta campanha, por favor tente novamente: <BR> &nbsp; </TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Login do Ramal: </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=phone_login SIZE=10 maxlength=20 VALUE=\"$phone_login\"></TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Senha do Ramal:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 maxlength=20 VALUE=\"$phone_pass\"></TD></TR>\n";
...
?>
	...
	image_voicemail_OFF.src="../agc/images/agc_check_voicemail_OFF_ptbr.gif";
	var image_voicemail_ON = new Image();
	image_voicemail_ON.src="../agc/images/agc_check_voicemail_ON_ptbr.gif";
	var image_voicemail_BLINK = new Image();
	image_voicemail_BLINK.src="../agc/images/agc_check_voicemail_BLINK_ptbr.gif";
...
// ################################################################################
// ACTIVE EXTENSIONS LIST ATUALIZAR FUNCTIONS
...
// ################################################################################
// BUSY TRUNK LIST ATUALIZAR FUNCTIONS
...
// ################################################################################
// BUSY EXTENSIONS LIST ATUALIZAR FUNCTIONS
	...
	function busyhangup_send_redirect(taskvar,tasktrunkaction) 
		...
		if (tasktrunkaction == 'LISTEN')
			...
			if (listenvalue.match(regLa))
				...
				{
				bsr_continue=0;
				alert("Você só pode monitorar os canais ZAP:\n" + listenvalue);
				}
			}
		if (bsr_continue == '1')
			{
	...
	function liverecording_send_recording(taskvar,taskchan,taskspan,taskfile) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'Monitor')
				...
				var rec_start_html = "<a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + monitorchannelvalue + "','" + taskspan + "','" + filename + "');return false;\">Parar Gravação";
	...
	function conf_send_recording(taskconfrectype,taskconfspan,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			if (taskconfrectype == 'MonitorConf')
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfspan + "','" + taskconfrec + "','" + filename + "');return false;\">Parar Gravação</a>";
...
// ################################################################################
// Check to see if user's extension isligadoa call right now, if it is, format and print it
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	pause();
					...
					if (live_calls > 0)
						{
						var live_calls_HTML = "<font face=\"Arial,Helvetica\"><B>CHAMADAS ATIVAS NESTE RAMAL:</B></font><BR><table width=100%><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\">CANAL DO CLIENTE</td><td><font class=\"log_title\">CANAL REMOTO</td><td><font class=\"log_title\">RECORD</td><td><font class=\"log_title\">DESLIGAR</td><td><font class=\"log_title\">TRANFERÊNCIA</td><td><font class=\"log_title\">PARK</td></tr>";
						...
						while (loop_ct < live_calls)
							...
							if (recLIST.match(regx)) 
								{live_calls_HTML = live_calls_HTML + "<span id=\"recordlive" + loop_ct + "\"><a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + channelfieldBtrunk_array[1] + "','recordlive" + loop_ct + "');return false;\">Parar Gravação</span>";}
							else 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (out_calls > 0)
						{
						var out_log_HTML = "<table width=580><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> DATA/HORA DA CHAMADA</td><td><font class=\"log_title\">NÚMERO</td><td align=right><font class=\"log_title\">LENGTH (M:SS)</td><td><font class=\"log_title\"> </td></tr>"
					...
					if (in_calls > 0)
						{
						var in_log_HTML = "<table width=580><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> DATA/HORA DA CHAMADA</td><td><font class=\"log_title\">NÚMERO ENTRANTE</td><td COLSPAN=2><font class=\"log_title\">CALLERID</td><td align=right><font class=\"log_title\">LENGTH</td><td><font class=\"log_title\"> </td></tr>"
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (parked_count > 0)
						{
						var park_HTML = "<table width=600><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\">CHANNEL<BR>&nbsp; CHAMADA ID</td><td><font class=\"log_title\">EM ESPERA POR<BR>&nbsp; TEMPO EM ESPERA</td><td><font class=\"log_title\">DESLIGAR</td><td><font class=\"log_title\">TRANFERÊNCIA</td><td><font class=\"log_title\">CAPTURAR</td></tr>"
						while (loop_ct < parked_count)
							...
							park_HTML = park_HTML + "<tr bgcolor=\"" + row_color + "\"><td><font class=\"log_text\">" + loop_ct + "</td><td><font class=\"log_text\">" + park_channel + "<BR>&nbsp; " + park_call_id + "</td><td><font class=\"log_text\">" + parked_by + "<BR>&nbsp; " + parked_time + "</td><td><font class=\"log_text\"><a href=\"#\" onclick=\"livehangup_send_hangup('" + park_channel + "');return false;\">DESLIGAR</a></td><td><font class=\"log_text\"><a href=\"#\" onclick=\"showMainXfeR('MainXfeRBox','" + park_channel + "');return false;\">TRANFERÊNCIA</a></td><td><font class=\"log_text\"><a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + park_channel + "');return false;\">CAPTURAR</a></td></tr>";
...
// display general conference meetme room header info and refresh detail content
	function conference_header_display(taskconf,taskreg,taskrefresh)
		...
		var conf_head_HTML = "<font class=\"sh_text\">CONFERÊNCIA " + head_conf + "</b></font><font class=\"sb_text\">&nbsp; &nbsp; Registrado para: " + head_reg + " &nbsp; " + reglink + " &nbsp; &nbsp; &nbsp; &nbsp; <a href=\"#\" onclick=\"basic_originate_call('" + head_conf + "','NO','NO');return false;\">Entrar na conferência </a><BR><a href=\"#\" onclick=\"check_for_conf_calls('" + head_conf + "','1');return false;\">Atualizar </a> &nbsp; &nbsp; <span id=\"conf_rec_link\"><a href=\"#\" onclick=\"conf_send_recording('MonitorConf','conf_rec_link','" + head_conf + "');return false;\">Record</a></span> &nbsp; &nbsp; &nbsp; &nbsp; <input TYPE=TEXT SIZE=15 NAME=conf_dtmf STYLE=\"font-family : sans-serif; font-size : 10px\"> <A HREF=\"#\" onclick=\"SendConfDTMF(" + head_conf + ");\">Enviar DTMF</A> &nbsp; &nbsp; &nbsp; &nbsp; <input TYPE=TEXT SIZE=15 NAME=conf_dial STYLE=\"font-family : sans-serif; font-size : 10px\"> <A HREF=\"#\" onclick=\"SendManualDial('YES'," + head_conf + ");\">Discar da confêrencia</A><BR></font>";
				...
				//	alert(xmlhttp.responseText);
					...
					var conf_chan_array = check_conf_array[1].split(" ~");
					   if (live_conf_calls > 0)
						...
						var conv_start=-1;
						var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>CHAMADAS ATIVAS NESTA CONFERÊNCIA:</B></font><BR><TABLE WIDTH=500><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">CANAL REMOTO</TD><TD><font class=\"log_title\">DESLIGAR</TD><TD><font class=\"log_title\">TRANFERÊNCIA</TD></TR>";
...
// ################################################################################
// Send Originate command to manager to direct user to Caixa Postal
...
//('BuilD','ADD','REMOVE','UP','DOWN')
	...
	//		alert(favorites_listEDIT);
		...
		var favedit_HTML = "<center><table cellpadding=5 cellspacing=5 width=750><tr><td colspan=2 align=center bgcolor=\"#DDDD99\"><B>EXTENSÕES DISPONÍVEIS</B></td><td align=center bgcolor=\"#CCCC99\"><B>FAVORITOS</B></td></tr><tr><td bgcolor=\"#DDDD99\" height=380 width=200 valign=top><font class=\"ss_text\"><span id=FavSelectA>";
		loop_ct = 0;
		while (loop_ct < favlistCT)
			...
			if (favorites_listEDIT.match(regFAVlist))
				{
				// do not print extension, it'sligadothe favorites list
				}
			else
				{
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT()
		...
		document.getElementById("LogouTBoxLink").innerHTML = "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&user=" + user + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "\">CLIQUE AQUI PARA FAZER O LOGIN NOVAMENTE</a>\n";
...
<?php	echo "Bem vindo $LOGfullname, você está logado no ramal : $fullname - $protocol/$extensionligado$server_ip &nbsp; <a href=\"#\" onclick=\"LogouT();return false;\">SAIR</a><BR>\n"; ?>
</TD></TR>
...
<TD><A HREF="#" onclick="ActiveLinesPanelToFront();"><IMG SRC="../agc/images/agc_tab_active_lines_ptbr.gif" ALT="Linhas Ativas no Painel" WIDTH=139 HEIGHT=30 Border=0></A></TD>
<TD><A HREF="#" onclick="ConfereNcesPanelToFront();"><IMG SRC="../agc/images/agc_tab_conferences_ptbr.gif" ALT="Painel de Conferências" WIDTH=139 HEIGHT=30 Border=0></A></TD>
<TD><A HREF="#" onclick="SendCheckVoiceMail();"><IMG SRC="../agc/images/agc_check_voicemail_ON_ptbr.gif" NAME=voicemail ALT="Verificar Caixa Postal" WIDTH=170 HEIGHT=30 Border=0></A></TD>
<TD><IMG SRC="../agc/images/agc_live_call_OFF_ptbr.gif" NAME=livecall ALT="Chamadas Ativas" WIDTH=109 HEIGHT=30 Border=0></TD>
</TR></TABLE>
</SPAN>
...
<span style="position:absolute;left:0px;top:12px;z-index:29;" id="TrunkHangupBox">
	...
	<span id="TrunkHangup_ZMlink"><a href="#" onclick="busyhangup_send_redirect('Trunk','LISTEN');return false;">Ouvir o Canal</a> &nbsp; | &nbsp; </span>
	<a href="#" onclick="busytrunkhangup_force_refresh();return false;">Atualizar</a> &nbsp; | &nbsp; 
	<a href="#" onclick="hideTrunkHangup('TrunkHangupBox');">Voltar a página principal</a>
	</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:12px;z-index:28;" id="LocalHangupBox">
    <table border=1 bgcolor="#CDE0C2" width=600 height=500><TR><TD> DESLIGAR LOCAL <BR><BR>
	<span id="LocalHangupContent"> Menu Local Ativo </span><BR>
	<span id="LocalHangup_HUlink"><a href="#" onclick="busyhangup_send_hangup('Local');return false;">Desligar Local</a> &nbsp; | &nbsp; </span>
	<span id="LocalHangup_HJlink"><a href="#" onclick="busyhangup_send_redirect('Local');return false;">Capturar Local</a> &nbsp; | &nbsp; </span>
	<span id="LocalHangup_ZMlink"><a href="#" onclick="busyhangup_send_redirect('Local','LISTEN');return false;">Ouvir Local</a> &nbsp; | &nbsp; </span>
	<a href="#" onclick="busylocalhangup_force_refresh();return false;">Atualizar</a> &nbsp; | &nbsp; 
	<a href="#" onclick="hideLocalHangup('LocalHangupBox');">Voltar a página principal</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:80px;top:12px;z-index:42;" id="MainXfeRBox">
	...
	<tr><td>Extensões:<BR><span id="MainXfeRContent"> Menu de Extensões </span></td>
	<td>
	<BR>
	<a href="#" onclick="mainxfer_send_redirect('XfeR');return false;">Enviar para a extensão selecionada</a> <BR><BR>
	<a href="#" onclick="mainxfer_send_redirect('VMAIL');return false;">Enviar para a caixa postal selecionada</a> <BR><BR>
	<a href="#" onclick="mainxfer_send_redirect('ENTRY');return false;">Envie para este número</a>:<BR><input type=text name=extension_xfer_entry size=20 maxlength=50> <BR><BR>
	<a href="#" onclick="getactiveext('MainXfeRBox');return false;">Atualizar</a> <BR><BR><BR>
	<a href="#" onclick="hideMainXfeR('MainXfeRBox');">Voltar a página principal</a> <BR><BR>
	</TD>
	<TD>Conferências:<BR><font size=1>(clique no número abaixo para enviar a conferência)<BR><input type=checkbox name=MainXfeRconfXTRA size=1 value="1"> Enviar meu canal para<div class="scroll_list" id="MainXfeRconfContent"> Menu das conferências </div></td></TR></TABLE>
</span>
...
<span style="position:absolute;left:80px;top:12px;z-index:43;" id="LocalDialBox">
	...
	<tr><td>Extensões:<BR><span id="LocalDialContent"> Menu de Extensões </span></td>
	<td>
	<BR>
	<a href="#" onclick="mainxfer_send_originate('DiaL','','');return false;">Ligar para a extensão selecionada</a> <BR><BR>
	<a href="#" onclick="mainxfer_send_originate('VMAIL');return false;">Ligar para a caixa postal selecionada</a> <BR><BR>
	<a href="#" onclick="getactiveext('LocalDialBox');return false;">Atualizar</a> <BR><BR><BR>
	<a href="#" onclick="hideLocalDial('LocalDialBox');">Voltar a página principal</a> <BR><BR>
	</TD>
	<TD></td></TR></TABLE>
</span>

<span style="position:absolute;left:40px;top:12px;z-index:41;" id="ParkDisplayBox">
    <table border=0 bgcolor="#FFFFCC" width=640 height=500 cellpadding=3><TR><TD COLSPAN=3 ALIGN=CENTER><b> CHAMADAS EM ESPERA:</b> <div class="scroll_park" id="ParkDisplayContents"></div>
	<a href="#" onclick="hideParkDisplay('ParkDisplayBox');">Voltar a página principal</a> <BR><BR>
	</td></TR></TABLE>
</span>
...
<tr><td align=center><font face="Arial,Helvetica"><B>CAIXA POSTAL &nbsp; &nbsp; </B></font> NOVO: <span id="new_vmail_span"></span> &nbsp; &nbsp; VELHO: <span id="old_vmail_span"></span> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <font face="Arial,Helvetica"><B>DISCAGEM MANUAL &nbsp; &nbsp; &nbsp; </B></font><input TYPE=TEXT SIZE=20 NAME=manual_dial STYLE="font-family : sans-serif; font-size : 10px"> <A HREF="#" onclick="SendManualDial();">DIAL</A></td></tr>

<tr><td align=center><a href="#" onclick="showParkDisplay('ParkDisplayBox');return false;"><font face="Arial,Helvetica"><B>CHAMADAS EM ESPERA</B></a>:  <span id="parked_calls_count">0</span> &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a href="#" onclick="showLocalDial('LocalDialBox');return false;"><font face="Arial,Helvetica"><B>DISCAR A EXTENSÃO LOCAL</a></td></tr>

<tr><td align=center><font face="Arial,Helvetica"><B>CHAMADAS SAINTES:</B></font></td></tr>
...
<span style="position:absolute;left:5px;top:5px;z-index:34;" id="FavoriteSEdiT">
	...
	<TR VALIGN=BOTTOM><TD VALIGN=BOTTOM ALIGN=CENTER BGCOLOR="#FFFFCC"><a href="#" onclick="SubmiT_FavoritE_ChangEs();return false;">ENVIAR ALTERAÇÕES FAVORITAS - requer logout</a></TD></TR>
...
<tr><td colspan=3>
<a href="#" onclick="pause();return false;">STOP</a> | <a href="#" onclick="start();return false;">START</a> &nbsp; &nbsp; Taxa de Atualização: <span id="refresh_rate">1000 ms</span> <a href="#" onclick="faster();return false;">Mais rápido</a> | <a href="#" onclick="slower();return false;">Mais devagar</a></p>
	<div id="status"><em>Inicializando..</em></div>
</td></tr>
<tr><td>Extensões Ativas <BR> 
<a href="#" onclick="activeext_force_refresh();return false;">ATUALIZAR</a> | 
<span id="activeext_order"><a href="#" onclick="activeext_order_desc();return false;">ORDEM</a> | </span>
</td>

<td>Linhas externas <BR>
<a href="#" onclick="busytrunk_force_refresh();return false;">ATUALIZAR</a> | 
<span id="busytrunk_order"><a href="#" onclick="busytrunk_order_desc();return false;">ORDEM</a> | </span>
</td>

<td>Extensões Locais <BR>
<a href="#" onclick="busyext_force_refresh();return false;">ATUALIZAR</a> | 
<span id="busyext_order"><a href="#" onclick="busyext_order_desc();return false;">ORDEM</a> | </span>
</td></tr>

<tr><td VALIGN=TOP>
	<span id="activeext"><em>Os Dados vão aqui</em></span><BR><BR>
</td><td VALIGN=TOP>
	<span id="busytrunk"><em>Os Dados vão aqui</em></span><BR><BR><span id="TrunkHangupLink"><a href="#" onclick="showTrunkHangup('TrunkHangupBox');return false;">Atividade do Tronco</a></span>
</td><td VALIGN=TOP>
	<span id="busyext"><em>Os Dados vão aqui</em></span><BR><BR><span id="LocalHangupLink"><a href="#" onclick="showLocalHangup('LocalHangupBox');return false;">Atividade Local</a></span>
</td></tr>
...
<span id="ConfereNcesListContent">Lista das conferências </span>
</span>
...
<span style="width:540;height:400;" id="ConfereNceDetailContent">Clique no número da conferência, a esquerda, para entrar na conferência </span>
</span>
