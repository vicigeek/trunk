...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- バージョン: $version     ビルド: $build    MEETME: $conf_exten   server_ip: $server_ip-->\n";
	echo "<title>会議内線チェック";
	echo "</title>\n";
	echo "</head>\n";
...
if ($ACTION == 'refresh')
	...
	if (strlen($conf_exten)<1)
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 有効ではない\n";
		exit;
		}
	else
		{
		...
		#	$AexternalDEAD=$row[0];
			...
			##### BEGIN check オンcalls in queue, number of active calls in the campaign
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03007',$user,$server_ip,$session_name,$one_mysql_log);}
				$row=mysqli_fetch_row($rslt);
				$RingCalls=$row[0];
				if ($RingCalls > 0) {$RingCalls = "<font class=\"queue_text_red\">列になっている電話： $RingCalls</font>";}
				else {$RingCalls = "<font class=\"queue_text\">列になっている電話： $RingCalls</font>";}
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03008',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			##### END check オンcalls in queue, number of active calls in the campaign
			...
			if ($mel > 0) {$errno = mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03005',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				### find whether the call the agent is オンis hung up
...
if ($ACTION == 'register')
	...
	if ( (strlen($conf_exten)<1) || (strlen($exten)<1) )
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 有効ではない or Exten $exten 有効ではない\n";
		exit;
		}
	else
		{
	...
	echo "会議 $conf_exten 登録されています $exten\n";
	}
