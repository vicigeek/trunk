...
$HEADER.="	}\n";
$HEADER.=".chart_td\n";
$HEADER.="	{background-image: url(../vicidial/images/gridline58.gif); background-repeat: repeat-x; background-position: left top; border-left: 1px solid #e5e5e5; border-right: 1px solid #e5e5e5; padding:0; border-bottom: 1px solid #e5e5e5; background-color:transparent;}\n";
$HEADER.="\n";
$HEADER.="-->\n";
$HEADER.="</style>\n";
...
$MAIN.="<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <a href=\"$PHP_SELF?query_date=$query_date&end_date=$end_date$groupQS&shift=$shift&DB=$DB&INVIA=$INVIA&file_download=1\">DOWNLOAD</a> | <a href=\"./admin.php?ADD=3311&did_id=$group[0]\">MODIFICA</a> | <a href=\"./admin.php?ADD=999999\">REPORT</a> </FONT>\n";
$MAIN.="</TD></TR></TABLE>\n";
$MAIN.="</FORM>\n";
...
if (!$group)
	...
	###################################################
	### TOTALI DID SUMMARY SECTION ###
	if (strlen($extension[0]) > 0)
		{
		$ASCII_text.="DID Summary:\n";
		$ASCII_text.="+--------------------+--------------------------------+------------+------------+\n";
		$ASCII_text.="| DID                | DESCRIZIONE                    | ROUTE      | CALLS      |\n";
		$ASCII_text.="+--------------------+--------------------------------+------------+------------+\n";

		$CSV_text1.="\"DID Summary:\"\n";
		$CSV_text1.="\"DID\",\"DESCRIZIONE\",\"ROUTE\",\"CALLS\"\n";
		...
		while ($d < $stats_array_ct)
			...
			for ($d=0; $d<count($graph_stats); $d++) {
				...
				$GRAPH_text.="	<td nowrap class=\"chart_td value$class\"><img src=\"../vicidial/images/bar.png\" alt=\"\" width=\"".round(MathZDC(400*$graph_stats[$d][0], $max_calls))."\" height=\"16\" />".$graph_stats[$d][0]."</td>\n";
				$GRAPH_text.="  </tr>\n";
			}
			$GRAPH_text.="  <tr>\n";
			$GRAPH_text.="	<th class=\"thgraph\" scope=\"col\">TOTALE CHIAMATE:</th>\n";
		...
		$ASCII_text.="+--------------------+--------------------------------+------------+------------+\n";
		$ASCII_text.="|                                                           TOTALI | $FtotCALLS |\n";
		$ASCII_text.="+------------------------------------------------------------------+------------+\n";
		$CSV_text1.="\"\",\"\",\"TOTALI\",\"$FtotCALLS\"\n";
		#$MAIN.=$GRAPH;
	...
	for ($d=0; $d<count($graph_stats); $d++) {
		...
		$GRAPH_text.="  <tr>\n";
		$GRAPH_text.="	<td class=\"chart_td$class\">".$graph_stats[$d][1]."</td>\n";
		$GRAPH_text.="	<td nowrap class=\"chart_td value$class\" nowrap><img src=\"../vicidial/images/bar.png\" alt=\"\" width=\"".round(MathZDC(400*$graph_stats[$d][0], $max_calls))."\" height=\"16\" />".$graph_stats[$d][0]."</td>\n";
		$GRAPH_text.="  </tr>\n";
	}
	$GRAPH_text.="  <tr>\n";
	$GRAPH_text.="	<th class=\"thgraph\" scope=\"col\">TOTALE CHIAMATE:</th>\n";
	...
	$GRAPH_text.="<tr><th class='thgraph' scope='col'>TOTALI:</th><th class='thgraph' scope='col'>".trim($totCALLS)."</th></tr></table>";
