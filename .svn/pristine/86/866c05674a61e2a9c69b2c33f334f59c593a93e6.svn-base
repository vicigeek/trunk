...
if($reports_only_user > 0)
	...
	if ( (!preg_match("/$report_name/",$LOGallowed_reports)) and (!preg_match("/ALL REPORT/",$LOGallowed_reports)) )
		...
		Header("WWW-Authenticate: Basic realm=\"CONTACT-CENTER-ADMIN\"");
		Header("HTTP/1.0 401 Unauthorized");
		echo "Non sei autorizzato a visualizzare questa relazione: |$USER|$report_name|\n";
		exit;
		}
	else
		{
...
if ($action == "CALLCARD_DETAIL")
	...
	if ($details_to_print > 0) 
		...
		echo "<br>CallCard Dettagliata<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Esito: </td><td align=left><B>$status</B></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Gruppo: </td><td align=left><B>$inbound_group_id</B></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Note: </td><td align=left><B>$note_comments</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Create Utente/Date: </td><td align=left><B>$create_user / $create_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Activate Utente/Date: </td><td align=left><B>$activate_user / $activate_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Used Utente/Date: </td><td align=left><B>$used_user / $used_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Void Utente/Date: </td><td align=left><B>$void_user / $void_time</B></td></tr>\n";
		echo "</TABLE>\n";
		echo "<BR><BR><BR>\n";
		...
		echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=2 color=white>AGENT DATA</B></TD>";
...
if ($action == "CALLCARD_SUMMARY")
	...
	echo "<TR BGCOLOR=BLACK>";
	echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=1 color=white>ESITO</B></TD>";
	...
	if ($db_ct > 0)
		...
		echo "<b>Esistono $DBcount batches in the system\n";
		}
	echo "<BR><BR>\n";
...
if ($action == "GENERATE")
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>In-Gruppo: </td><td align=left><input type=text name=inbound_group_id size=20 maxlength=20></td></tr>\n";
...
<BR><font size=1>CallCard &nbsp; &nbsp; VERSIONE: <?php echo $version ?> &nbsp; &nbsp; BUILD: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
