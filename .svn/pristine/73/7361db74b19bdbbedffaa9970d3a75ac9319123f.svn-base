...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten não é válido or queryCID $queryCID não é válido, Originate comando não inserido\n";
		}
	else
		{
		...
		echo "Originate comando enviado para Exten $exten Canal $channelligado$server_ip\n";
		}
	}
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten não é válido or queryCID $queryCID não é válido, Originate comando não inserido\n";
		}
	else
		{
		...
		echo "Originate comando enviado para Exten $exten Canal $channelligado$server_ip |$account|$variable|\n";
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten não é válido or ext_context $ext_context or queryCID $queryCID não é válido, Hangup comando não inserido\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
#			}
		...
		if ($channel_live==1)
			...
			echo "Hangup comando enviado para Canal $channelligado$call_server_ip\n";
			}
		}
	}
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conferenceligado$server_ip, Redirect comando não inserido\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Não foi possível achar uma conferência vazialigado$server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel não está ativoligado$call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel não está ativoligado$server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channelligado$call_server_ip, Hungup $extrachannelligado$server_ip";}
				}
			}
		else
			{
			...
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Changed to Redirect: $channelligado$server_ip";}
			}
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02057',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel and $extrachannel to $extenligado$server_ip";}
					}
				else
					{
					$S='*';
...
if ($ACTION=="Redirect")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		if ($channel_live==1)
			...
			echo "Redirect comando enviado para Canal $channelligado$server_ip\n";
			}
		}
	}
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		...
		if ($channel_live==1)
			...
			echo "$ACTION comando enviado para Canal $channelligado$server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n";
			}
		}
	}
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
	...
	if ( (strlen($exten)<3) or (strlen($channel)<4) or (strlen($filename)<8) )
		...
		##### StopMonitorConf steps #####
			...
			# find and hang up all recordings goingligadoin this conference # and extension = '$exten' 
		...
		#	$rec_count = intval(mysqli_num_rows($rslt) / 2);
			...
			echo "$ACTION comando enviado para Canal $channelligado$server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n AS GRAVAÇÕES VÃO DURAR ATÉ 60 MINUTOS\n";
		}
	}
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "Conferência $exten, Stage $stage não é válido or queryCID $queryCID não é válido, Originate comando não inserido\n";
		}
	else
		{
		...
		echo "Volume comando enviado para Conferência $exten, Stage $stage Canal $channelligado$server_ip\n";
		}
	}
