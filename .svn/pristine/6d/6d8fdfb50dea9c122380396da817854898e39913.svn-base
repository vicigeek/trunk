...
function LoadLists(FromBox) {
	...
	for(i = 0; i < document.getElementById('group').options.length; i++) {
		if (document.getElementById('group').options[i].selected) {
			selectedCampanhas += document.getElementById('group').options[i].value+"|";
		} 
	}

	// Limpar Lista menu
	document.getElementById('list_ids').options.length=0;
	var new_list = new Opção();
	new_list.value = "--ALL--";
	...
	for (j=0; j<campaign_id_ary.length; j++) {
		...
		var campaign_matches = selectedCampanhas.match(campaignID);
		if (campaign_matches) {
...
echo "Listas: <font size=1>(opcional, possivelmente lento)</font><BR>\n";
echo $list_options;
echo "</TD><TD VALIGN=TOP>";
...
if (strlen($group[0]) > 1)
	{
	echo " <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">ALTERAR</a> | \n";
	echo " <a href=\"./admin.php?ADD=999999\">RELATÓRIOS</a> </FONT>\n";
	}
else
	{
	echo " <a href=\"./admin.php?ADD=10\">CAMPANHAS</a> | \n";
	echo " <a href=\"./admin.php?ADD=999999\">RELATÓRIOS</a> </FONT>\n";
	}
echo "</TD></TR></TABLE>";
echo "</FORM>\n\n";
...
if (strlen($group[0]) < 1)
	...
	$OUToutput .= "Average Call Length for all HA em segundo:    $average_ci_seconds     TotalTime: $CIsec\n";
	...
	$DROPRESPOSTApercent = (MathZDC($DROPcallsRAW, $RESPOSTAcalls) * 100);
	...
	if (preg_match("/YES/i",$include_rollover))
		{
		$inDROPRESPOSTApercent = (MathZDC($DROPcallsRAW, $CIcallsRAW) * 100);
	...
	$AVG_RESPOSTAagent_non_pause_sec = (MathZDC($TOTALanswers, $agent_non_pause_sec) * 60);
	...
	$ASCII_text .= "\n";
	$ASCII_text .= "---------- ESTATÍSTICA DE MOTIVO DE DESLIGAMENTO\n";
	$ASCII_text .= "+----------------------+------------+\n";
	...
	$GRAPH.="<caption align='top'>ESTATÍSTICA DE MOTIVO DE DESLIGAMENTO</caption>";
	$GRAPH.="<tr>\n";
	...
	while ($i < $reasons_to_print)
		...
		if (preg_match('/NONE/',$reason))	{$reason = 'NO RESPOSTA           ';}
	...
	$ASCII_text .= "| STATUS | DESCRIPTION          | CATEGORIA             | CALLS      | TOTAL TIME | AVG TIME |CALLS/HOUR|CALLS/HOUR|\n";
	$ASCII_text .= "+--------+----------------------+----------------------+------------+------------+----------+----------+----------+\n";
...
#	$ASCII_text .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	if ( ($carrier_logging_active > 0) and ($carrier_stats == 'YES') )
		...
		$ASCII_text .= "\n";
		$ASCII_text .= "---------- CARRIER CALL STATUS\n";
		$ASCII_text .= "+----------------------+------------+\n";
	...
	if ($presets_enabled > 0)
		...
		$GRAPH.="  <tr>\n";
		$GRAPH.="	<th class=\"thgraph\" scope=\"col\">TOTAL DE CHAMADAS:</th>\n";
	...
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	$ASCII_text .= "| CATEGORIA             | CALLS      | DESCRIPTION                    |\n";
	$ASCII_text .= "+----------------------+------------+--------------------------------+\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>AGENTE STATS</caption><tr><th class='thgraph' scope='col'>STATUS</th>";
	$CALLS_graph=$graph_header."<th class='thgraph' scope='col'>CALLS </th></tr>";
	$TOTALTIME_graph=$graph_header."<th class='thgraph' scope='col'>TOTAL TIME</th></tr>";
	$AVGTIME_graph=$graph_header."<th class='thgraph' scope='col'>AVG TIME</th></tr>";
	###########################
