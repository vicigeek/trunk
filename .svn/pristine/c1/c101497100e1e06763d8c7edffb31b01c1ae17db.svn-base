...
echo "<!-- VERSIONE: <?php echo $version ?>     BUILD: <?php echo $build ?> -->\n";
?>
...
window.onload = function() {
	...
	document.getElementById("enable_move_country_code").onclick = enableMoveStatoCode;
	document.getElementById("enable_move_vendor_lead_code").onclick = enableMoveVendorLeadCode;
	document.getElementById("enable_move_source_id").onclick = enableMoveStatoSourceId;
	...
	document.getElementById("enable_update_country_code").onclick = enableUpdateStatoCode;
	document.getElementById("enable_update_vendor_lead_code").onclick = enableUpdateVendorLeadCode;
	document.getElementById("enable_update_source_id").onclick = enableUpdateStatoSourceId;
	...
	document.getElementById("enable_delete_country_code").onclick = enableDeleteStatoCode;
	document.getElementById("enable_delete_vendor_lead_code").onclick = enableDeleteVendorLeadCode;
	document.getElementById("enable_delete_source_id").onclick = enableDeleteStatoSourceId;
...
if ($move_submit == "move" )
	...
	if (($enable_move_vendor_lead_code == "enabled") && ($move_vendor_lead_code != ''))
		...
		$move_parm = $move_parm . "&nbsp;&nbsp;&nbsp;&nbsp;co`digo del vendedor del Lead is like $move_vendor_lead_code<br />";
	...
	elseif ($enable_move_modify_date == "enabled")
		{
		blank_field('ModificareDate',true);
		}
...
if ($update_submit == "update" )
	...
	if ($update_list == '') { missing_required_field('ID Lista'); }
	...
	if (($enable_update_vendor_lead_code == "enabled") && ($update_vendor_lead_code != ''))
		...
		$update_parm = $update_parm . "&nbsp;&nbsp;&nbsp;&nbsp;co`digo del vendedor del Lead is like $update_vendor_lead_code<br />";
...
if ( ( $delete_submit == "delete" ) && ( $delete_lists > 0 ) )
	...
	if ($delete_list == '') { missing_required_field('ID Lista'); }
	...
	if (($enable_delete_vendor_lead_code == "enabled") && ($delete_vendor_lead_code != ''))
		...
		$delete_parm = $delete_parm . "&nbsp;&nbsp;&nbsp;&nbsp;;co`digo del vendedor del Lead is like $delete_vendor_lead_code<br />";
...
if (
	...
	if ( preg_match("/ALL\-CAMPAIGNS/i",$allowed_campaigns) )
		{
		if ($DB) { echo "|Elaborazione in corsoAll Campagnas|\n"; }
	...
	echo "<p>The following are advanced lead management tools.  They will only work on lista inattive with less than $list_lead_limit leads in them. This is to avoid data inconsistencies.</p>";
	...
	echo "<option value='%'>All Esiti</option>\n";
	...
	echo "</select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Stato Code</td></td><td align=left>\n";
	...
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>ModificareDate</td></td><td align=left>\n";
	...
	echo "<tr bgcolor=#015B91><td colspan=2 align=center><font color=white><b>Update Lead Esiti</b></font></td></tr>\n";
	...
	if ( $delete_lists > 0 )
		{
		# BEGIN EliminareLeads
		echo "</table></center>\n";
		echo "<br /><center><table width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#015B91><td colspan=2 align=center><font color=white><b>EliminareLeads</b></font></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>List</td><td align=left>\n";
