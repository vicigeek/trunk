...
if ($SUBMIT && $old_user && $new_user && $confirm_transfer) {
	...
	$stmt="INSERT INTO vicidial_admin_log set event_date=NOW(), user='$PHP_AUTH_USER', ip_address='$ip', event_section='BENUTZER', event_type='MODIFY', record_id='$new_user', event_code='ADMIN CALLBACK BULK CHANGE', event_sql=\"$SQL_log\", event_notes='Old user: $old_user';";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
}
?>
...
if ($SUBMIT && $old_user && $new_user && !$confirm_transfer) 
	...
	if ($DB) {echo "$stmt\n";}
	# <tr><td align='right'>
	echo "Bevollmächtigte with callbacks:<BR><select name='old_user' size=5>\n";
