...
$report_name = 'Λίστες Pass Report';
$db_source = 'M';
...
if ($use_lists > 0)
	{
	$MAIN.="</TD><TD VALIGN=TOP> Λίστες:<BR>";
	...
	$MAIN.="</SELECT>\n<BR>\n";
	$MAIN.="<a href=\"$PHP_SELF?use_lists=0&DB=$DB\">SWITCH TO ΕΚΣΤΡΑΤΕΙΕΣ</a>";
	}
else
	{
	$MAIN.="</TD><TD VALIGN=TOP> Εκστρατείες:<BR>";
	$MAIN.="<SELECT SIZE=5 NAME=group[] multiple>\n";
	if  (preg_match('/\-\-ALL\-\-/',$group_string))
		{$MAIN.="<option value=\"--ALL--\" selected>-- ALL ΕΚΣΤΡΑΤΕΙΕΣ --</option>\n";}
	else
		{$MAIN.="<option value=\"--ALL--\">-- ALL ΕΚΣΤΡΑΤΕΙΕΣ --</option>\n";}
	$o=0;
	while ($campaigns_to_print > $o)
		{
		if (preg_match("/$groups[$o]\|/i",$group_string)) {$MAIN.="<option selected value=\"$groups[$o]\">$groups[$o] - $group_names[$o]</option>\n";}
...
$MAIN.="<BR><BR>\n";
$MAIN.="<INPUT type=submit NAME=ΕΠΙΒΕΒΑΙΩΣΗ VALUE=ΥΠΟΒΑΛΛΩ>\n";
$MAIN.="</TD><TD VALIGN=TOP> &nbsp; &nbsp; &nbsp; &nbsp; ";
$MAIN.="<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
if (strlen($group[0]) > 1)
	{
	$MAIN.=" <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">ΤΡΟΠΟΠΟΙΗΣΗ</a> | \n";
	$MAIN.=" <a href=\"./admin.php?ADD=999999\">ΑΝΑΦΟΡΕΣ</a> </FONT>\n";
	}
else
	{
...
if (strlen($group[0]) < 1)
	{
	$MAIN.="\n\n";
	$MAIN.="ΠΑΡΑΚΑΛΩ ΕΠΙΛΕΞΤΕ ΜΙΑ ΕΚΣΤΡΑΤΕΙΑ ΚΑΙ ΜΙΑ ΗΜΕΡΟΜΗΝΙΑ ΑΝΩΤΕΡΩ, ΚΑΙ ΠΑΤΗΣΤΕ ΕΠΙΒΕΒΑΙΩΣΗ\n";
	}

else
	{
	$OUToutput = '';
	$OUToutput .= "Λίστες Pass Report                             $NOW_TIME\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- ID ΛΙΣΤΑΣ SUMMARY     <a href=\"$PHP_SELF?DB=$DB$groupQS&ΕΠΙΒΕΒΑΙΩΣΗ=$ΕΠΙΒΕΒΑΙΩΣΗ&file_download=1\">ΛΗΨΗ</a>\n";
	...
	$OUToutput .= "|  LOAD DATE | ID ΛΙΣΤΑΣ and NAME                         | CAMPAIGN | COUNT      | ACTIVE   |";
	...
	$GRAPH="</PRE><table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"white\" summary=\"ID ΛΙΣΤΑΣ Summary\" class=\"horizontalgraph\">\n";
	$GRAPH.="<caption align='top'>ID ΛΙΣΤΑΣ SUMMARY</caption>";
	$GRAPH.="<tr>\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>ID ΛΙΣΤΑΣ SUMMARY</caption><tr><th class='thgraph' scope='col'>LISTS</th>";
