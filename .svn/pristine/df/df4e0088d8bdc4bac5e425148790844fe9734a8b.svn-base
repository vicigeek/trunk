...
window.onload = function() {
	...
	document.getElementById("enable_move_country_code").onclick = enableMoveΧώραCode;
	document.getElementById("enable_move_vendor_lead_code").onclick = enableMoveVendorLeadCode;
	document.getElementById("enable_move_source_id").onclick = enableMoveΧώραSourceId;
	...
	document.getElementById("enable_update_country_code").onclick = enableUpdateΧώραCode;
	document.getElementById("enable_update_vendor_lead_code").onclick = enableUpdateVendorLeadCode;
	document.getElementById("enable_update_source_id").onclick = enableUpdateΧώραSourceId;
	...
	document.getElementById("enable_delete_country_code").onclick = enableDeleteΧώραCode;
	document.getElementById("enable_delete_vendor_lead_code").onclick = enableDeleteVendorLeadCode;
	document.getElementById("enable_delete_source_id").onclick = enableDeleteΧώραSourceId;
...
echo "<b> &nbsp; <a href=\"lead_tools.php\">Βασική Lead Tools</a> &nbsp; | &nbsp; Advanced Lead Tools</b>\n";
echo "</font>\n";
echo "</td>\n";
...
if ($move_submit == "move" )
	...
	if (($enable_move_vendor_lead_code == "enabled") && ($move_vendor_lead_code != ''))
		...
		$move_parm = $move_parm . "&nbsp;&nbsp;&nbsp;&nbsp;αυτόματος κωδικός καθοδήγησης is like $move_vendor_lead_code<br />";
	...
	elseif ($enable_move_security_phrase == "enabled")
		{
		blank_field('Φράση Ασφαλείας',true);
		}
...
if ($update_submit == "update" )
	...
	if ($update_list == '') { missing_required_field('ID Λίστας'); }
	...
	if (($enable_update_vendor_lead_code == "enabled") && ($update_vendor_lead_code != ''))
		...
		$update_parm = $update_parm . "&nbsp;&nbsp;&nbsp;&nbsp;αυτόματος κωδικός καθοδήγησης is like $update_vendor_lead_code<br />";
...
if ( ( $delete_submit == "delete" ) && ( $delete_lists > 0 ) )
	...
	if ($delete_list == '') { missing_required_field('ID Λίστας'); }
	...
	if (($enable_delete_vendor_lead_code == "enabled") && ($delete_vendor_lead_code != ''))
		...
		$delete_parm = $delete_parm . "&nbsp;&nbsp;&nbsp;&nbsp;;αυτόματος κωδικός καθοδήγησης is like $delete_vendor_lead_code<br />";
...
if (
	...
	echo "<p>The following are advanced lead management tools.  They will only work on Μη ενεργές λίστες with less than $list_lead_limit leads in them. This is to avoid data inconsistencies.</p>";
	...
	echo "</select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Χώρα    Code</td></td><td align=left>\n";
	...
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>ModifyDate</td></td><td align=left>\n";
	...
	echo "</td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Φράση Ασφαλείας</td></td><td align=left>\n";
