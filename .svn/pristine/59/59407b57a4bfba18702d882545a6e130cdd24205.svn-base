...
if ( (!$vendor_id) and (!$phone)  and (!$lead_id) and (!$log_phone)  and (!$log_lead_id) and ( (strlen($status)<1) and (strlen($list_id)<1) and (strlen($user)<1) and (strlen($owner)<1) ) and ( (strlen($first_name)<1) and (strlen($last_name)<1) )) 
	...
	echo "<TR>";
	echo "<TD colspan=3 align=center><b>Αναζήτηση Οδηγού Επιλογήs:</TD>";
	echo "</TR>";
	
	$archive_stmt="ΕΜΦΑΝΙΣΗ ΠΙΝΑΚΑΣS LIKE '%vicidial_list_archive%'";
	...
	echo "<TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>$label_vendor_lead_code(αυτόματος κωδικός καθοδήγησης): &nbsp; </TD><TD ALIGN=left><input type=text name=vendor_id size=10 maxlength=20></TD>";
	echo "<TD><input type=submit name=submit value=ΕΠΙΒΕΒΑΙΩΣΗ></TD>\n";
	echo "</TR><TR>";
	echo "<TD colspan=3 align=center> &nbsp; </TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";

	echo "<TD ALIGN=right>$label_phone_αριθμός: &nbsp; </TD><TD ALIGN=left><input type=text name=phone size=14 maxlength=18></TD>";
	echo "<TD rowspan=2><input type=submit name=submit value=ΕΠΙΒΕΒΑΙΩΣΗ></TD>\n";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	echo "<TD ALIGN=right>Κατάσταση:&nbsp; </TD><TD ALIGN=left><input type=text name=status size=7 maxlength=6></TD>";
	echo "<TD rowspan=4><input type=submit name=submit value=ΕΠΙΒΕΒΑΙΩΣΗ></TD>\n";
	echo "</TR><TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>ID Λίστας: &nbsp; </TD><TD ALIGN=left><input type=text name=list_id size=15 maxlength=14></TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>Χρήστης: &nbsp; </TD><TD ALIGN=left><input type=text name=user size=15 maxlength=20></TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>Ιδιοκτήτης:&nbsp; </TD><TD ALIGN=left><input type=text name=owner size=15 maxlength=50></TD>";
	echo "</TR><TR>";
	echo "<TD colspan=3 align=center> &nbsp; </TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	echo "<TD colspan=3 align=center> &nbsp; </TD>";
	echo "</TR><TR>";
	echo "<TD colspan=3 align=center><b>Log Search Επιλογήs:</TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	if ( (strlen($log_lead_id)>0) or (strlen($log_phone)>0) )
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			{
			echo "\n<br><br><center>\n";
			echo "<b>Υπάρχουν no outbound calls matching your search criteria</b><br><br>\n";
			echo "</center>\n";
			}
		else
			{
			...
			echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>CALL ΗΜΕΡΑ</B> &nbsp;</FONT></TD>\n";
			echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>ΚΑΤΑΣΤΑΣΗ</B> &nbsp;</FONT></TD>\n";
			echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>USER</B> &nbsp;</FONT></TD>\n";
			echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>ID ΛΙΣΤΑΣ</B> &nbsp;</FONT></TD>\n";
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			{
			echo "\n<br><br><center>\n";
			echo "<b>Υπάρχουν no inbound calls matching your search criteria</b><br><br>\n";
			echo "</center>\n";
			}
		else
			{
		...
		if (strlen($stmtC) > 10)
			...
			if ( ($results_to_print < 1) and ($results_to_printX < 1) )
				{
				echo "\n<br><br><center>\n";
				echo "<b>Υπάρχουν no inbound did calls matching your search criteria</b><br><br>\n";
				echo "</center>\n";
				}
			else
				{
	...
	if ( ($results_to_print < 1) and ($results_to_printX < 1) )
		...
		echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>ID ΛΙΣΤΑΣ</B> &nbsp;</FONT></TD>\n";
		echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>PHONE</B> &nbsp;</FONT></TD>\n";
		echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>ΟΝΟΜΑ</B> &nbsp;</FONT></TD>\n";
