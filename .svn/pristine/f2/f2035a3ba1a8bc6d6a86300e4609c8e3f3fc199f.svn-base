...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		{
		echo "ERROR Exten $exten nie jest prawidłowy or queryCID $queryCID nie jest prawidłowy, Originate komenda nie wstawiona\n";
		}
	else
		{
		if ( (preg_match('/MANUAL/i',$agent_dialed_type)) and ( (preg_match("/^\d860\d\d\d\d$/i",$exten)) or (preg_match("/^860\d\d\d\d$/i",$exten)) ) )
			{
			echo "ERROR Nie masz uprawnień do wybierania numeru inne sesje agenta $exten\n";
			exit;
			}
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "Nie mogę znaleźć wolnej konferencji na $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel nie jest aktywna na $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel nie jest aktywna na $server_ip";}
				}	
			}
