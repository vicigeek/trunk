...
<title>QC ModificarLead: <?php echo "$lead_id - ".urldecode($lead_name).urldecode(" (Campaña: $campaign_id - $campaign_name)"); ?></title>
...
if ($end_call > 0) {
	...
	$rslt=mysql_to_mysqli($stmt, $link);
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        $row=mysqli_fetch_assoc($rslt);
        //QUALITY CONTROL CHANGE LOG BEGIN - CHANGE VERSIÓN (VIEW VERSIÓN IS BELOW, WHICH CREATES THE RECORD, THIS ONE MERELY MODIFIES THE EXISTING RECORD)
        $new_record=$row;
	...
	echo "<br>Información modificada<BR><BR>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	echo "<tr><td colspan=2>Fronter: <A HREF=\"user_stats.php?user=$tsr\">$tsr</A> &nbsp; &nbsp; ID de Lista: $list_id &nbsp; &nbsp; Called Count: $called_count</td></tr>\n";
	...
	while ($row_count > $o)
		...
		$rowx=mysqli_fetch_row($rslt);
             	echo "<tr><td align=right>$rowx[0] : </td><td align=left><hr>$rowx[1]<br><b>ID de Lista:</b> $rowx[2]; <b>ID De la Campaña:</b> $rowx[3]<br>$rowx[4]</td></tr>\n";
		$o++;
		}
	...
	if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') || $scheduled_callback=='Y' )
		...
		if ($CB_to_print>0)
			...
			var o_cal = new tcal ({
				...
				'imgpath' : '../../vicidial/images/' // directory with calendar images
			});
...
<input type=hidden name=viewtime value='<?php echo $STARTtime; ?>' />
	...
	echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
	echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=center><font size=2>OLD STATUS</td><td align=center><font size=2>NEW STATUS</td><td align=center><font size=2>QC USER</td><td align=center><font size=2>AGENT</td><td align=center><font size=2>CAMPAIGN</td><td align=center><font size=2>LIST</td><td align=center><font size=2>ELAPSED</td><td align=center><font size=2>&nbsp;</td></tr>\n";
		echo "$qc_agent_log\n";
	echo "</TABLE>\n";
	echo "<BR><BR>\n";
