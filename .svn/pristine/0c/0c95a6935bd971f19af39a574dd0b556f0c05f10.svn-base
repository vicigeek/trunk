...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		$VDdisplayMESSAGE = "Użytkownika i hasło podane nie są aktywne w systemie<BR>Please try again:";
		...
		if ($vts_count > 0)
			...
			print "<!-- NOWY vicidial_timeclock_status record inserted for $user:   |$affected_rows| -->\n";
			}
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "Masz teraz zalogowanych";
				$LOGtimeMESSAGE = "Jesteś zalogowany na $NOW_TIME";
				...
				print "<!-- NOWY vicidial_timeclock_log record inserted for $user:   |$affected_rows|$timeclock_id| -->\n";
				...
				$rslt=mysql_to_mysqli($stmt, $link);
				$affected_rows = mysqli_affected_rows($link);
				print "<!-- NOWY vicidial_timeclock_audit_log record inserted for $user:   |$affected_rows| -->\n";
				}

			if ( ($status=='LOGIN') and ($stage=='logout') )
				{
				$VDdisplayMESSAGE = "Musisz się zalogować-out";
			...
			if ($referrer=='agent') 
				{$BACKlink = "<A HREF=\"./vicidial.php?pl=$phone_login&pp=$phone_pass&VD_login=$user\"><font color=\"#003333\">POWRÓT pełnomocnika do ekranu logowania</font></A>";}
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">POWRÓT do administracji</font></A>";}
			if ($referrer=='welcome') 
				{$BACKlink = "<A HREF=\"$welcomeURL\"><font color=\"#003333\">Powrót do ekranu powitalnego</font></A>";}
		...
		if ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') )
			...
			$log_action = 'login';
			$button_name = 'LOGIN';
			$LOGtimeMESSAGE = "Ostatniego zalogowanego obecnie pod adresem:$last_action_date<BR><BR>Kliknij poniżej, aby LOGOWANIE log-in";
			}
		if ($status=='LOGIN')
			{
