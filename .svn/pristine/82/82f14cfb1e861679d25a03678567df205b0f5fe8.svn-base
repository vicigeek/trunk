...
if ( (!$vendor_id) and (!$phone)  and (!$lead_id) and (!$log_phone)  and (!$log_lead_id) and ( (strlen($status)<1) and (strlen($list_id)<1) and (strlen($user)<1) and (strlen($owner)<1) ) and ( (strlen($first_name)<1) and (strlen($last_name)<1) )) 
	...
	$archive_stmt="MOSTRAR TABELAS LIKE '%vicidial_list_archive%'";
	...
	echo "<TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>$label_vendor_lead_code(código do fornecedor): &nbsp; </TD><TD ALIGN=left><input type=text name=vendor_id size=10 maxlength=20></TD>";
	...
	echo "<TD rowspan=4><input type=submit name=submit value=ENVIAR></TD>\n";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	if ( (strlen($log_lead_id)>0) or (strlen($log_phone)>0) )
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			{
			echo "\n<br><br><center>\n";
			echo "<b>Existem no outbound calls matching your search criteria</b><br><br>\n";
			echo "</center>\n";
			}
		else
			{
			...
			while ($results_to_print > $o)
				...
				if ($LOGadmin_hide_phone_data != '0')
					{
					if ($DB > 0) {echo "ESCONDERPHONEDATA|$row[1]|$LOGadmin_hide_phone_data|\n";}
					$phone_temp = $row[1];
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			{
			echo "\n<br><br><center>\n";
			echo "<b>Existem no inbound calls matching your search criteria</b><br><br>\n";
			echo "</center>\n";
			}
		else
			{
		...
		if (strlen($stmtC) > 10)
			...
			if ( ($results_to_print < 1) and ($results_to_printX < 1) )
				{
				echo "\n<br><br><center>\n";
				echo "<b>Existem no inbound did calls matching your search criteria</b><br><br>\n";
				echo "</center>\n";
				}
			else
				{
