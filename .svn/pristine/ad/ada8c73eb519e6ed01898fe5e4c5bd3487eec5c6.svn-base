...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- 版本: $version     版次: $build    ACTION: $ACTION   server_ip: $server_ip-->\n";
	echo "<title>管理者發送: ";
...
if ($ACTION=="SysCIDOriginate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or (strlen($queryCID)<1) )
		{
		echo "Exten $exten 是無效的 or queryCID $queryCID 是無效的, Originate 指令未插入\n";
		}
	else
		{
		...
		echo "Originate 指令發送  Exten $exten 渠道 $channel on $server_ip\n";
		}
	}
...
if ($ACTION=="OriginateName")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($extenName)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		echo "ext_priority $ext_priority 需被設定\n";
		echo "\nOriginateName Action 未送出\n";
		}
	else
		{
...
if ($ACTION=="Originate")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($ext_context)<1) or ( (strlen($queryCID)<10) and ($alertCID < 1) ) )
		...
		echo "Originate 指令發送  Exten $exten 渠道 $channel on $server_ip |$account|$variable|\n";
...
if ($ACTION=="HangupConfDial")
	...
	if ( (strlen($exten)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) )
		{
		$channel_live=0;
		echo "conference $exten 是無效的 or ext_context $ext_context or queryCID $queryCID 是無效的, Hangup 指令未插入\n";
		}
	else
		{
		$local_DEF = 'Local/';
...
#			}
		if ( ($auto_dial_level > 0) and (strlen($CalLCID)>2) and (strlen($exten)>2) and ($secondS > 0))
			...
			if ($rowx[0]==0)
				{
				echo "Call $CalLCID $channel 非可用 on $call_server_ip, Checking Live 渠道...\n";
		...
		if ($channel_live==1)
			...
			echo "Hangup 指令發送  渠道 $channel on $call_server_ip\n";
			}
		}
	}
...
if ($ACTION=="RedirectVD")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($campaign)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($uniqueid)<2) or (strlen($lead_id)<1) )
		...
		echo "ext_context $ext_context 需被設定\n";
		echo "ext_priority $ext_priority 需被設定\n";
		echo "auto_dial_level $auto_dial_level 需被設定\n";
		echo "campaign $campaign 需被設定\n";
		echo "uniqueid $uniqueid 需被設定\n";
		echo "lead_id $lead_id 需被設定\n";
		echo "\nRedirectVD Action 未送出\n";
		}
	else
		{
...
if ($ACTION=="RedirectToPark")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($extenName)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($parkedby)<1) )
		...
		echo "ext_context $ext_context 需被設定\n";
		echo "ext_priority $ext_priority 需被設定\n";
		echo "parkedby $parkedby 需被設定\n";
		echo "\nRedirectToPark Action 未送出\n";
		}
	else
		{
		if ($stage!="2NDXfeR")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
		...
		if (preg_match("/NEXTAVAILABLE/",$exten))
			...
			//	fclose($fp);
				...
				{
				$channel_liveX=0;
				echo "Cannot find empty vicidial_conference on $server_ip, Redirect 指令未插入\n|$stmt|";
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "找不到空會議 on $server_ip";}
				}
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel 非可用 on $call_server_ip";}
				}	
			}
		...
		if ($row[0]==0)
			...
			if ($rowx[0]==0)
				...
				if (preg_match("/SECOND|FIRST|DEBUG/",$filename)) {$DBout .= "$channel 非可用 on $server_ip";}
				}	
			}
		if ( ($channel_liveX==1) and ($channel_liveY==1) )
			...
			if ($rowx[0] < 1)
				{
				$channel_liveY=0;
				echo "No Local agent to send call to, Redirect 指令未插入\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02044',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				echo "RedirectXtraCX 指令發送  渠道 $channel on $call_server_ip and \nHungup $extrachannel on $server_ip\n";
...
if ($ACTION=="RedirectXtraNeW")
	...
	{$ACTION="Redirect";}
		...
		if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($session_id)<3) or ( ( (strlen($extrachannel)<3) or (strlen($exten)<1) ) and (!preg_match("/NEXTAVAILABLE/",$exten)) ) )
			...
			echo "ext_context $ext_context 需被設定\n";
			echo "ext_priority $ext_priority 需被設定\n";
			echo "session_id $session_id 需被設定\n";
			echo "\nRedirect Action 未送出\n";
			if (preg_match("/SECOND|FIRST|DEBUG/",$filename))
				{
				if ($WeBRooTWritablE > 0)
...
if ($ACTION=="Redirect")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15)  or (strlen($exten)<1)  or (strlen($ext_context)<1)  or (strlen($ext_priority)<1) )
		...
		if ($channel_live==1)
			...
			echo "Redirect 指令發送  渠道 $channel on $server_ip\n";
			}
		}
	}
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		...
		if ($channel_live==1)
			...
			echo "$ACTION 指令發送  渠道 $channel on $server_ip\nFilename: $filename\nRecorDing_ID: $recording_id\n";
			}
		}
	}
...
if ($ACTION=="VolumeControl")
	...
	if ( (strlen($exten)<1) or (strlen($channel)<1) or (strlen($stage)<1) or (strlen($queryCID)<1) )
		{
		echo "會議 $exten, Stage $stage 是無效的 or queryCID $queryCID 是無效的, Originate 指令未插入\n";
		}
	else
		{
		...
		echo "Volume 指令發送  會議 $exten, Stage $stage 渠道 $channel on $server_ip\n";
		}
	}
