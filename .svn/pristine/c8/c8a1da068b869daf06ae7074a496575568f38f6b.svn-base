...
if($reports_only_user > 0)
	...
	if ( (!preg_match("/$report_name/",$LOGallowed_reports)) and (!preg_match("/ALL RELATÓRIOS/",$LOGallowed_reports)) )
		...
		Header("WWW-Authenticate: Basic realm=\"CONTACT-CENTER-ADMIN\"");
		Header("HTTP/1.0 401 Unauthorized");
		echo "Você não tem permissão para ver este relatório: |$USER|$report_name|\n";
		exit;
		}
	else
		{
...
<title>ADMINISTRATION: CallCard Administrador
<?php
...
if ($action == "CALLCARD_DETAIL")
	...
	if ($details_to_print > 0) 
		...
		echo "<br>CallCard Detalhes<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Comentários: </td><td align=left><B>$note_comments</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Create Usuário/Date: </td><td align=left><B>$create_user / $create_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Activate Usuário/Date: </td><td align=left><B>$activate_user / $activate_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Used Usuário/Date: </td><td align=left><B>$used_user / $used_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Void Usuário/Date: </td><td align=left><B>$void_user / $void_time</B></td></tr>\n";
		echo "</TABLE>\n";
		echo "<BR><BR><BR>\n";
		...
		echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=2 color=white>AGENTE DATA</B></TD>";
...
if ($action == "CALLCARD_SUMMARY")
	...
	echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=1 color=white>QUANTIDADE</B></TD>";
	echo "</TR>\n";
	...
	if ($db_ct > 0)
		...
		echo "<b>Existem $DBcount batches in the system\n";
		}
	echo "<BR><BR>\n";
...
<BR><font size=1>CallCard &nbsp; &nbsp; VERSÃO: <?php echo $version ?> &nbsp; &nbsp; BUILD: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
