...
<title>ΔΙΟΙΚΗΣΗ: Εκστρατεία Multi-Alt-Εκκινήσεις
<?php 
...
if ($camp_multi < 1)
	{
	echo "Αυτή η καμπάνια δεν έχει οριστεί σε Αυτόματη-Alt-Dial MULTI_LEAD\n";
	exit;
	}
...
if ($action == "ALT_MULTI_ΕΠΙΒΕΒΑΙΩΣΗ")
	{
	$settings_updated=0;
...
if ($action == "BLANK")
	...
	echo "<br>Multi-μολύβδου Ρυθμίσεις κλήσης Αυτόματη Alt Έντυπο<form action=$PHP_SELF method=POST>\n";
	echo "<input type=hidden name=DB value=\"$DB\">\n";
	echo "<input type=hidden name=campaign_id value=\"$campaign_id\">\n";
	echo "<input type=hidden name=action value=ALT_MULTI_ΕΠΙΒΕΒΑΙΩΣΗ>\n";
	echo "<center><TABLE width=$section_width cellspacing=3>\n";
	echo "<tr><td align=center colspan=2>\n";


	echo "<br><b>PHONE TYPES WITHIN THE LISTS FOR THIS ΕΚΣΤΡΑΤΕΙΑ: <a href=\"./admin.php?ADD=31&campaign_id=$campaign_id\">$campaign_id</a></b>\n";
	echo "<TABLE width=700 cellspacing=3>\n";
	echo "<tr><td>PHONE NUMBER TYPE</td><td>ΚΛΗΘΕΝΤΑ</td><td>ΜΗ ΚΛΗΘΕΝΤΑ</td><td>SELECTED</td><td>OLD RANK</td><td>NEW RANK</td></tr>\n";
	...
	echo "<tr bgcolor=\"#9BB9FB\"><td><font size=1>ΣΥΝΟΛΙΚΑ</td><td colspan=3 align=center><font size=1>$lead_list[count]</td><td align=center><font size=1>$o</td></tr>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Παραγγελία Λίστα Τυχαία: </td><td align=left><select size=1 name=lead_order_randomize><option>Y</option><option>N</option><option SELECTED>$lead_order_randomize</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Παραγγελία Λίστα Δευτεροβάθμια: </td><td align=left><select size=1 name=lead_order_secondary><option>LEAD_ASCEND</option><option>LEAD_DESCEND</option><option>CALLTIME_ASCEND</option><option>CALLTIME_DESCEND</option><option SELECTED>$lead_order_secondary</option></select></td></tr>\n";
