...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		$VDdisplayMESSAGE = "入力したユーザ名とパスワードはこのシステム内では有効ではありません。<BR>再度、お試しください。:";

		echo"<HTML><HEAD>\n";
		echo"<TITLE>Agent タイムクロック</TITLE>\n";
		...
		echo "<TD ALIGN=CENTER VALIGN=MIDDLE><B> タイムクロック </B></TD>";
		echo "</TR>\n";
		...
		if ( ($last_action_sec < 30) and ($status != 'START') )
			...
			$VDdisplayMESSAGE = "最後にログインまたはログアウトの30秒以内にログインまたはログアウトすることができません。";
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "今、ログインしました。";
				$LOGtimeMESSAGE = "でログインしました。 $NOW_TIME";
			...
			if ( ($status=='LOGIN') and ($stage=='logout') )
				{
				$VDdisplayMESSAGE = "今、ログアウトしました。";
				$LOGtimeMESSAGE = "でログアウトしました。$NOW_TIME<BR>ログインしていた時間：$totTIME_HMS";
			...
			if ( ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='logout') ) or ( ($status=='LOGIN') and ($stage=='login') ) )
				{echo "エラー：タイムクロックログ入力がすでに行われました：$status|$stage";  exit;}

			if ($referrer=='agent') 
				{$BACKlink = "<A HREF=\"./vicidial.php?pl=$phone_login&pp=$phone_pass&VD_login=$user\"><font color=\"#003333\">エージェントログイン画面に戻る</font></A>";}
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">管理に戻る</font></A>";}
			if ($referrer=='welcome') 
				{$BACKlink = "<A HREF=\"$welcomeURL\"><font color=\"#003333\">ようこそ画面に戻る</font></A>";}
		...
		if ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') )
			{
			$VDdisplayMESSAGE = "最後にログインしてからの時間：$totTIME_HMS";
			$log_action = 'login';
			$button_name = 'LOGIN';
			$LOGtimeMESSAGE = "以下の時間に最後にログアウトしました：$last_action_date<BR><BR>ログインするために以下のログインをクリックする";
			}
		if ($status=='LOGIN')
			{
			$VDdisplayMESSAGE = "ログインしていた時間：$totTIME_HMS";
			$log_action = 'logout';
			$button_name = 'LOGOUT';
			$LOGtimeMESSAGE = "以下の時間にログインしました： $last_action_date<BR>ログインしていた時間：$totTIME_HMS<BR><BR>ログアウトするために以下のログアウトをクリックする";
			}
