...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<TABLE><TR><TD></TD>\n";
	echo "<!-- ILPA -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pl/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Polski <img src=\"../agc/images/pl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	function busyhangup_send_redirect(taskvar,tasktrunkaction) 
		...
		if (tasktrunkaction == 'LISTEN')
			...
			if (listenvalue.match(regLa))
				...
				{
				bsr_continue=0;
				alert("Możesz tylko monitorować kanały Zap:\n" + listenvalue);
				}
			}
		if (bsr_continue == '1')
			{
	...
	function liverecording_send_recording(taskvar,taskchan,taskspan,taskfile) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'Monitor')
				...
				var rec_start_html = "<a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + monitorchannelvalue + "','" + taskspan + "','" + filename + "');return false;\">Zakończ nagrywanie";
	...
	function conf_send_recording(taskconfrectype,taskconfspan,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			if (taskconfrectype == 'MonitorConf')
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfspan + "','" + taskconfrec + "','" + filename + "');return false;\">Zakończ nagrywanie</a>";
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	pause();
					...
					if (live_calls > 0)
						{
						var live_calls_HTML = "<font face=\"Arial,Helvetica\"><B>AKTYWNE ROZMOWY Z TEGO TELEFONU:</B></font><BR><table width=100%><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\">KANAŁ KLIENCKI</td><td><font class=\"log_title\">KANAŁ ZDALNY</td><td><font class=\"log_title\">RECORD</td><td><font class=\"log_title\">ROZŁĄCZ</td><td><font class=\"log_title\">TRANSFER</td><td><font class=\"log_title\">PARK</td></tr>";
						...
						while (loop_ct < live_calls)
							...
							if (recLIST.match(regx)) 
								{live_calls_HTML = live_calls_HTML + "<span id=\"recordlive" + loop_ct + "\"><a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + channelfieldBtrunk_array[1] + "','recordlive" + loop_ct + "');return false;\">Zakończ nagrywanie</span>";}
							else 
...
// display general conference meetme room header info and refresh detail content
	function conference_header_display(taskconf,taskreg,taskrefresh)
		...
		var conf_head_HTML = "<font class=\"sh_text\">KONFERENCJA " + head_conf + "</b></font><font class=\"sb_text\">&nbsp; &nbsp; Zarejestrowany na: " + head_reg + " &nbsp; " + reglink + " &nbsp; &nbsp; &nbsp; &nbsp; <a href=\"#\" onclick=\"basic_originate_call('" + head_conf + "','NO','NO');return false;\">Dołącz do konferencji </a><BR><a href=\"#\" onclick=\"check_for_conf_calls('" + head_conf + "','1');return false;\">Odswież </a> &nbsp; &nbsp; <span id=\"conf_rec_link\"><a href=\"#\" onclick=\"conf_send_recording('MonitorConf','conf_rec_link','" + head_conf + "');return false;\">Record</a></span> &nbsp; &nbsp; &nbsp; &nbsp; <input TYPE=TEXT SIZE=15 NAME=conf_dtmf STYLE=\"font-family : sans-serif; font-size : 10px\"> <A HREF=\"#\" onclick=\"SendConfDTMF(" + head_conf + ");\">Wyślij tonowo</A> &nbsp; &nbsp; &nbsp; &nbsp; <input TYPE=TEXT SIZE=15 NAME=conf_dial STYLE=\"font-family : sans-serif; font-size : 10px\"> <A HREF=\"#\" onclick=\"SendManualDial('YES'," + head_conf + ");\">Rozmowa z konferencji</A><BR></font>";
...
<span style="position:absolute;left:0px;top:12px;z-index:28;" id="LocalHangupBox">
	...
	<a href="#" onclick="busylocalhangup_force_refresh();return false;">Odswież</a> &nbsp; | &nbsp; 
	<a href="#" onclick="hideLocalHangup('LocalHangupBox');">Wróć do Głównego okna</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:80px;top:12px;z-index:42;" id="MainXfeRBox">
	...
	<a href="#" onclick="mainxfer_send_redirect('VMAIL');return false;">Przełącz do poczty głosowej</a> <BR><BR>
	<a href="#" onclick="mainxfer_send_redirect('ENTRY');return false;">Przełącz do numeru</a>:<BR><input type=text name=extension_xfer_entry size=20 maxlength=50> <BR><BR>
	<a href="#" onclick="getactiveext('MainXfeRBox');return false;">Odswież</a> <BR><BR><BR>
	<a href="#" onclick="hideMainXfeR('MainXfeRBox');">Wróć do Głównego okna</a> <BR><BR>
	</TD>
	<TD>Konferencje:<BR><font size=1>(wybierz poniższy numer w celu przełączenia do konferencji)<BR><input type=checkbox name=MainXfeRconfXTRA size=1 value="1"> Przełącz również mój kanał<div class="scroll_list" id="MainXfeRconfContent"> Menu konferencji </div></td></TR></TABLE>
</span>
...
<span id="ConfereNcesListContent">Lista konferencji </span>
</span>
...
<span style="width:540;height:400;" id="ConfereNceDetailContent">Wybierz numer konferencji w celu uzyskania o niej informacji </span>
</span>
