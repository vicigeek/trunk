...
<title>ADMINISTRAÇÃO: campanha multi-Alt-Leads
<?php 
...
if ($camp_multi < 1)
	{
	echo "Esta campanha não está definido para Auto-Alt-Dial MULTI_LEAD\n";
	exit;
	}
...
if ($action == "ALT_MULTI_ENVIAR")
	...
	if ( (strlen($lead_order_randomize) > 0) and (strlen($lead_order_secondary) > 0) )
		...
		if ($affected_rows > 0)
			...
			$stmt="INSERT INTO vicidial_admin_log set event_date='$NOW_TIME', user='$PHP_AUTH_USER', ip_address='$ip', event_section='CAMPANHAS', event_type='MODIFY', record_id='$campaign_id', event_code='MODIFY CAMPANHA MULTI LEAD', event_sql=\"$SQL_log\", event_notes='';";
			$rslt=mysql_to_mysqli($stmt, $link);
...
if ($action == "BLANK")
	...
	echo "<br>Multi-chumbo Auto definições de marcação Alt Formulário<form action=$PHP_SELF method=POST>\n";
	echo "<input type=hidden name=DB value=\"$DB\">\n";
	echo "<input type=hidden name=campaign_id value=\"$campaign_id\">\n";
	echo "<input type=hidden name=action value=ALT_MULTI_ENVIAR>\n";
	echo "<center><TABLE width=$section_width cellspacing=3>\n";
	echo "<tr><td align=center colspan=2>\n";


	echo "<br><b>PHONE TYPES WITHIN THE LISTS FOR THIS CAMPANHA: <a href=\"./admin.php?ADD=31&campaign_id=$campaign_id\">$campaign_id</a></b>\n";
	echo "<TABLE width=700 cellspacing=3>\n";
	echo "<tr><td>PHONE NUMBER TYPE</td><td>CHAMADO</td><td>NÃO CHAMADO</td><td>SELECTED</td><td>OLD RANK</td><td>NEW RANK</td></tr>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Ordem Randomize Lista: </td><td align=left><select size=1 name=lead_order_randomize><option>Y</option><option>N</option><option SELECTED>$lead_order_randomize</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Ordem lista secundária: </td><td align=left><select size=1 name=lead_order_secondary><option>LEAD_ASCEND</option><option>LEAD_DESCEND</option><option>CALLTIME_ASCEND</option><option>CALLTIME_DESCEND</option><option SELECTED>$lead_order_secondary</option></select></td></tr>\n";
