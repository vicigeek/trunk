...
if ($admin_viewable_groupsALL > 0)
	{$UUgroups_list .= "<option value=\"---ALL---\">Alle Admin Benutzer-Gruppen</option>\n";}
...
<title>ADMINISTRATION: Dialer Email Konto Felder
<?php 
...
if ($SSemail_enabled < 1)
	{
	echo "ERROR: Inbound E-Mails sind auf diesem System nicht aktiv\n";
	exit;
	}
...
if (($SUBMIT=="SUBMIT" || $SUBMIT=="UPDATE") && $email_account_id) 
	{
	$error_msg="";
	if (!$list_id) {$error_msg.="- Default list ID is unwirksam oder null<BR/>";}
	if (!$email_account_id) {$error_msg.="- Email account ID is unwirksam oder null<BR/>";}
	if (!$email_account_name) {$error_msg.="- Email account name is unwirksam oder null<BR/>";}
	if (!$email_account_server) {$error_msg.="- Email account server is unwirksam oder null<BR/>";}
	if (!$email_account_user) {$error_msg.="- Email account user is unwirksam oder null<BR/>";}
	if (!$email_account_pass) {$error_msg.="- Email account password is unwirksam oder null<BR/>";}
	if (!filter_var($email_replyto_address, FILTER_VALIDATE_EMAIL)) {$error_msg.="- Email reply-to address is unwirksam oder null<BR/>";}
	...
	if (!$error_msg) 
		...
		if ($SUBMIT=="SUBMIT") 
			...
			if ($add_copy_disabled > 0)
				{
				echo "<br>Sie haben keine Berechtigung, um Datensätze auf diesem System hinzufügen -system_settings-\n";
				}
			else
				{
				...
				if (mysqli_affected_rows($link)==0) 
					...
					{
					$message="NEUES KONTO $email_account_id Erfolgreich erstellt";
					$eact="";
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="ACCOUNT $email_account_id ERFOLGREICH VERÄNDERTER";
				# $eact="";
	...
	if (mysqli_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysqli_affected_rows($link)==0) 
				{
				$error_msg.="- Es war ein unbekannter Fehler beim Versuch, das neue Konto kopieren<BR/>";
				if($DB>0) {$error_msg.="<B>$ins_stmt</B><BR>";}
				}
			else
				{
				$message="NEUES KONTO $new_account_id Erfolgreich von KOPIERT $source_email_account<BR/>";
				$eact="";
		...
		{
		$error_msg="- Error - Quelle E-Mail existiert nicht<BR/>";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "Konto NICHT KOPIERT<BR/>Die Daten, die Sie eingereicht hat folgende Fehler:<BR/>$error_msg";}
		echo "<br>Kopieren New Account aus bestehenden Konto<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_SUBMIT>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Konto-ID zu kopierenden: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** Es sind keine bestehende Konten zu kopierenden ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "Account not INSERTED<BR/>Die Daten, die Sie eingereicht hat folgende Fehler:<BR/>$error_msg";}
		echo "<br>ADD NEW INBOUND-Mail-Konto<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto-ID: </td><td align=left><input type=text name=email_account_id size=15 maxlength=20 value='$email_account_id'>$NWB#email_accounts-email_account_id$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Account Name: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Aktiv:</td><td align=left><select size=1 name=active><option>Y</option><option SELECTED>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailKonto Beschreibung: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Account: </td><td align=left><select size=1 name=email_account_type><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Admin Benutzer-Gruppe: </td><td align=left><select size=1 name=user_group>\n";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Alle Admin Benutzer-Gruppen</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto-Protokoll: </td><td align=left><select size=1 name=protocol><option SELECTED>IMAP</option><option>POP3</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailReply-to Adresse: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto Server: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailKonto Benutzername: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Konto Passwort: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailFrequency Check Rate (mins): </td><td align=left><select name='email_frequency_check_mins'>";
		$i=5;
		while ($i<=60) 
			{
		...
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">In-Gruppen-ID</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
		echo "</select>$NWB#email_accounts-in_group$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">Standard Liste ID</a>: </td><td align=left><input type=text name=default_list_id size=20 maxlength=255 value='$default_list_id'>$NWB#email_accounts-default_list_id$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>In-Group Call Griff Methode:</td><td align=left><select size=1 name=call_handle_method><option>EMAIL</option><option>EMAILLOOKUP</option><option>EMAILLOOKUPRL</option><option>EMAILLOOKUPRC</option><option SELECTED>$call_handle_method</option></select>$NWB#email_accounts-call_handle_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Group-Agent Suche Methode:</td><td align=left><select size=1 name=agent_search_method><option value=\"LB\">LB - Load Balanced</option><option value=\"LO\">LO - Load Balanced Overflow</option><option value=\"SO\">SO - Server Only</option><option SELECTED>$agent_search_method</option></select>$NWB#email_accounts-agent_search_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Group US-ID:</td><td align=left><input type=text name=list_id size=14 maxlength=14 value=\"$list_id\">$NWB#email_accounts-ingroup_list_id$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Group Kampagnen-ID:</td><td align=left><select size=1 name=campaign_id>\n";
		echo "$campaigns_list";
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>Bestätigen Sie das Löschen der eingehenden E-Mail Konto $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Klicken Sie auf Konto löschen $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_to_mysqli($stmt, $link);
		...
		if ($error_msg) {echo "Konto nicht aktualisiert<BR/>Die Daten, die Sie eingereicht hat folgende Fehler:<BR/>$error_msg";}
		echo "<br>Update einer bestehenden INBOUND-Mail-Konto<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto-ID: </td><td align=left><input type=hidden name=email_account_id value='$email_account_id'><B>$email_account_id$NWB#email_accounts-email_account_id$NWE</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Account Name: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Aktiv:</td><td align=left><select size=1 name=active>";
		echo "<option value='$active' selected>$active</option>";
		echo "<option>Y</option><option>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailKonto Beschreibung: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Account: </td><td align=left><select size=1 name=email_account_type><option value='$email_account_type' selected>$email_account_type</option><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Admin Benutzer-Gruppe: </td><td align=left><select size=1 name=user_group>\n";
		echo "<option value='$user_group' selected>$user_group</option>";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Alle Admin Benutzer-Gruppen</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto-Protokoll: </td><td align=left><select size=1 name=protocol><option>IMAP</option><option>POP3</option><option SELECTED>$protocol</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailReply-to Adresse: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail-Konto Server: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=rightEmailKonto Benutzername: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-Mail Konto Passwort: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "</select>$NWB#email_accounts-email_frequency_check_mins$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=3811&group_id=$group_id\">In-Gruppen-ID</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
...
###############
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Un-E-Mails behandelt: </td><td align=left><B>$unhandled_emails</B></td></tr>\n";
################
		...
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Klicken Sie hier um Admin Änderungen an diesem Datensatz zu sehen</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>Eingehende E-Mails Kontoeinträge:\n";
	...
	echo "<TD><font size=1 color=white>BESCHREIBUNG</TD>\n";
	...
	echo "<TD><font size=1 color=white>AKTIV</TD>\n";
	echo "<TD><font size=1 color=white>UNREAD EMAIL</TD>\n";
	echo "<TD><font size=1 color=white>ÄNDERN Sie</TD></tr>\n";
	...
	while ($accounts_to_print > $o) 
		...
		echo "<td><font size=1><a href=\"$PHP_SELF?eact=UPDATE&email_account_id=$row[email_account_id]\">ÄNDERN Sie</a></font></td></tr>\n";
		$o++;
		}
