...
if ( ($invalid_record < 1) or (strlen($timeclock_id)<1) )
	...
	if ($stage == "edit_TC_log")
		...
		if ( ($LOGINepoch <= $PREVevent_epoch) || ($LOGOUTepoch >= $NEXTevent_epoch) )
			{
			echo "ERROR-Es gibt ein Problem mit den Daten, die Sie eingegeben haben, gehen Sie bitte zurück<BR>\n";
			echo "Ein timeclock Tagung können sich nicht ein anderes timeclock Sitzung<BR>\n";
			echo "$LOGINepoch<BR>\n";
			echo "$LOGOUTepoch<BR>\n";
	...
	#	$LOGOUTevent_id =	$tcid_link;
		...
		echo "<TR><TD ALIGN=RIGHT>User Gruppe:</TD><TD ALIGN=RIGHT>$LOGINuser_group</TD></TR>\n";
		...
		echo "<TR><TD ALIGN=RIGHT>User Gruppe:</TD><TD ALIGN=RIGHT>$LOGOUTuser_group</TD></TR>\n";
