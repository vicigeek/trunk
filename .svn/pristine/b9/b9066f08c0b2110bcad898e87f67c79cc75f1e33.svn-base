...
<title>ADMINISTRATION: Modification de l'enregistrement de la fiche</title>
...
<?php 
echo "<a href=\"./admin.php?ADD=100\">ADMINISTRATION</a>: Modification de l'enregistrement de la fiche<BR>\n";
...
if ($lead_id == 'NEW')
	...
	if ($affected_rows > 0)
		...
		echo "Le plomb a été ajouté: $lead_id<BR><BR>\n";
		$end_call=0;
		}
	else
		{echo "ERREUR: pas encore ajouté de plomb, s'il vous plaît revenir en arrière et regarder ce que vous avez entré<BR><BR>\n";}
	}
else
	{
...
if ($end_call > 0)
	...
	echo "Informations modifiées<BR><BR>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	if ($LOGadmin_hide_phone_data != '0')
		{
		if ($DB > 0) {echo "CACHERPHONEDATA|$row[11]|$LOGadmin_hide_phone_data|\n";}
		$phone_temp = $row[11];
	...
	if ($LOGadmin_hide_lead_data != '0')
		{
		if ($DB > 0) {echo "CACHERLEADDATA|$row[5]|$row[6]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";}
		if (strlen($row[5]) > 0)
	...
	if ($lead_id == 'NEW')
		{echo "<br><b>Ajouter une nouvelle piste</B>\n";}
	else
		{echo "<br>Informations sur l'appel: $first_name $last_name - $phone_number\n";}
	...
	echo "<tr><td colspan=2>Lead ID: $lead_id &nbsp; &nbsp; ID de la liste: $list_id</td></tr>\n";
	...
	if ($LOGadmin_hide_lead_data != '0')
		...
		echo "<tr><td align=right>Pays   : </td><td align=left>$country_code</td></tr>\n";
		...
		echo "<tr><td align=right>Pays   : </td><td align=left><input type=text name=country_code size=3 maxlength=3 value=\"".htmlentities($country_code)."\"></td></tr>\n";
	...
	if ($lead_id != 'NEW') 
		...
		echo "<tr bgcolor=#B6D3FC><td align=left>Modifiervicidial log </td><td align=left><input type=checkbox name=modify_logs value=\"1\" CHECKED></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=left>Modifieragent log </td><td align=left><input type=checkbox name=modify_agent_logs value=\"1\" CHECKED></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=left>Modifiercloser log </td><td align=left><input type=checkbox name=modify_closer_logs value=\"1\"></td></tr>\n";
	...
	if ( ($LOGadmin_hide_lead_data == '0') or ($lead_id == 'NEW') )
		{
		echo "<tr><td colspan=2 align=center><input type=submit name=submit value=\"VALIDER\"></td></tr>\n";
		}
	echo "</table></form>\n";
	echo "<BR><BR><BR>\n";
	...
	if ($lead_id != 'NEW') 
		...
		echo "Callback Détails:<BR><CENTER>\n";
		if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') or ($scheduled_callback == 'Y') )
			...
			if ($CB_to_print>0)
				...
				if ($rowx[9] == 'USERONLY')
					...
					echo "<input type=hidden name=lead_id value=\"$lead_id\">\n";
					echo "<input type=hidden name=callback_id value=\"$rowx[0]\">\n";
					echo "New Callback Owner UtilisateurID: <input type=text name=CBuser size=18 maxlength=20 value=\"$rowx[8]\"> \n";
				...
				<TR BGCOLOR="#E6E6E6">
				<TD align=center colspan=2>
				<FONT FACE="ARIAL,HELVETICA">Commentaires: 
				...
				<input type=button value="VALIDER" name=smt id=smt onClick="submit_form()">
				</TD>
				</TR>
			...
			{
			echo "<BR>Si vous voulez changer ce fil à un rappel prévu, d'abord changer la disposition à CBHOLD, puis le soumettre et vous serez en mesure de fixer la date et l'heure de rappel.<BR>\n";
			}
		echo "</TD></TR></TABLE>\n";
		...
		if ($c > 0)
			{
			echo "<B>EXTENSION SUPPLEANT DE NUMEROS DE TELEPHONE POUR CE PLOMB:</B>\n";
		...
		if ($custom_fields_enabled > 0)
			...
			$stmt="MONTRER TABLES LIKE \"custom_$CLlist_id\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_to_mysqli($stmt, $link);
			...
			if ($tablecount_to_print > 0) 
				...
				if ($fieldscount_to_print > 0) 
					...
					echo "<B>CHAMPS SUR MESURE POUR CE FIL:</B><BR>\n";
		...
		echo "<B>APPELS VERS CETTE FICHE:</B>\n";
		echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
		echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=left><font size=2>LENGTH</td><td align=left><font size=2> STATUS</td><td align=left><font size=2> TSR</td><td align=right><font size=2> CAMPAGNE</td><td align=right><font size=2> LIST</td><td align=right><font size=2> LEAD</td><td align=right><font size=2> HANGUP REASON</td><td align=right><font size=2> PHONE</td></tr>\n";
		...
		echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
		echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=left><font size=2>LENGTH</td><td align=left><font size=2> STATUS</td><td align=left><font size=2> TSR</td><td align=right><font size=2> CAMPAGNE</td><td align=right><font size=2> LIST</td><td align=right><font size=2> LEAD</td><td align=right><font size=2> WAIT</td><td align=right><font size=2> HANGUP REASON</td></tr>\n";
		...
		echo "<tr><td><font size=1># </td><td align=left><font size=2> CAMPAGNE</td><td><font size=2>DATE/TIME </td><td align=left><font size=2>CALL MENU </td><td align=left><font size=2> &nbsp; ACTION</td></tr>\n";
	...
	##### vicidial agent outbound calls for this time period #####
		...
		if ($allow_emails>0) 
			...
			echo "<TABLE width=750 cellspacing=1 cellpadding=1>\n";
			echo "<tr><td><font size=1># </td><td><font size=2>DATE/TIME </td><td align=right><font size=2> USER</td><td align=right><font size=2> CAMPAGNE</td><td align=left><font size=2>EMAIL TO</td><td align=left><font size=2> MESSAGE</td><td align=right><font size=2> ATTACHMENTS</td></tr>\n";
		...
		if ($admin_display > 0)
			{
			echo "<a href=\"./admin.php?ADD=720000000000000&stage=$lead_id&category=LEADS\">Click here to see Lead Modifierchanges to this lead</a>\n";
			}
                //Afficher link to QC if user has QC permissions
