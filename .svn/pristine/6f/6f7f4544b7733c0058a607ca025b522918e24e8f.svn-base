...
<title>ADMINISTRATION: Dialer campi dell'account e-mail
<?php 
...
if ($SSemail_enabled < 1)
	{
	echo "ERRORE: messaggi di posta elettronica in entrata non sono attivi su questo sistema\n";
	exit;
	}
...
if (($INVIA=="INVIA" || $INVIA=="UPDATE") && $email_account_id) 
	{
	$error_msg="";
	if (!$list_id) {$error_msg.="- Default list ID is invalido o nullo<BR/>";}
	if (!$email_account_id) {$error_msg.="- Email account ID is invalido o nullo<BR/>";}
	if (!$email_account_name) {$error_msg.="- Email account name is invalido o nullo<BR/>";}
	if (!$email_account_server) {$error_msg.="- Email account server is invalido o nullo<BR/>";}
	if (!$email_account_user) {$error_msg.="- Email account user is invalido o nullo<BR/>";}
	if (!$email_account_pass) {$error_msg.="- Email account password is invalido o nullo<BR/>";}
	if (!filter_var($email_replyto_address, FILTER_VALIDATE_EMAIL)) {$error_msg.="- Email reply-to address is invalido o nullo<BR/>";}
		...
		if ($INVIA=="INVIA") 
			...
			if ($add_copy_disabled > 0)
				...
				if (mysqli_affected_rows($link)==0) 
					...
					{
					$message="NUOVO CONTO $email_account_id SUCCESSO CREATO";
					$eact="";
			...
			if (mysqli_affected_rows($link)==0) 
				...
				{
				$message="ACCOUNT $email_account_id SUCCESSO MODIFICATI";
				# $eact="";
	...
	if (mysqli_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysqli_affected_rows($link)==0) 
				{
				$error_msg.="- Si è verificato un errore sconosciuto nel tentativo di copiare il nuovo account<BR/>";
				if($DB>0) {$error_msg.="<B>$ins_stmt</B><BR>";}
				}
			else
				{
				$message="NUOVO CONTO $new_account_id SUCCESSO copiato da $source_email_account<BR/>";
				$eact="";
		...
		{
		$error_msg="- Error - email origine non esiste<BR/>";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "CONTO NON COPIATO<BR/>I dati da Lei presentato ha i seguenti errori:<BR/>$error_msg";}
		echo "<br>Copiare un nuovo account da account esistente<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_INVIA>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>ID account da cui copiare: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nuovo ID account: </td><td align=left><input type=text name=new_account_id value='$new_account_id' size=10 maxlength=20></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nuovo nome account: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100>$NWB#email_accounts-carrier_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=INVIA value='COPY'></td></tr>\n";
		echo "</TABLE></center>\n";
		echo "</TD></TR></TABLE>\n";
		}
	else 
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** Non ci sono conti esistenti per copiare da ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "CONTO NON INSERITA<BR/>I dati da Lei presentato ha i seguenti errori:<BR/>$error_msg";}
		echo "<br>AGGIUNGI NUOVO ACCOUNT EMAIL INBOUND<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>Email ID account: </td><td align=left><input type=text name=email_account_id size=15 maxlength=20 value='$email_account_id'>$NWB#email_accounts-email_account_id$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Nome account: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Attivo: </td><td align=left><select size=1 name=active><option>Y</option><option SELECTED>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Descrizione: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Tipo di account: </td><td align=left><select size=1 name=email_account_type><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account protocollo: </td><td align=left><select size=1 name=protocol><option SELECTED>IMAP</option><option>POP3</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Indirizzo di risposta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Utente: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Password account: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">ID In-Group</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
		echo "</select>$NWB#email_accounts-in_group$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">Predefinito ID List</a>: </td><td align=left><input type=text name=default_list_id size=20 maxlength=255 value='$default_list_id'>$NWB#email_accounts-default_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Nel gruppo di agente Metodo di ricerca:</td><td align=left><select size=1 name=agent_search_method><option value=\"LB\">LB - Load Balanced</option><option value=\"LO\">LO - Load Balanced Overflow</option><option value=\"SO\">SO - Server Only</option><option SELECTED>$agent_search_method</option></select>$NWB#email_accounts-agent_search_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Nel gruppo Elenco ID:</td><td align=left><input type=text name=list_id size=14 maxlength=14 value=\"$list_id\">$NWB#email_accounts-ingroup_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=INVIA VALUE=INVIA><input type=hidden name='eact' value='ADD'></td></tr>\n";
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>Confermare l'eliminazione INBOUND email dell'account $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Fare clic per eliminare l'account $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_to_mysqli($stmt, $link);
		...
		if ($error_msg) {echo "CONTO NON AGGIORNATO<BR/>I dati da Lei presentato ha i seguenti errori:<BR/>$error_msg";}
		echo "<br>Aggiornare una esistente INBOUND email dell'account<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right>Email ID account: </td><td align=left><input type=hidden name=email_account_id value='$email_account_id'><B>$email_account_id$NWB#email_accounts-email_account_id$NWE</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Nome account: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Attivo: </td><td align=left><select size=1 name=active>";
		echo "<option value='$active' selected>$active</option>";
		echo "<option>Y</option><option>N</option></select>$NWB#email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Descrizione: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Tipo di account: </td><td align=left><select size=1 name=email_account_type><option value='$email_account_type' selected>$email_account_type</option><option>INBOUND</option></select>$NWB#email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Admin Gruppo Utente: </td><td align=left><select size=1 name=user_group>\n";
		echo "<option value='$user_group' selected>$user_group</option>";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Tutti i gruppi di utenti Admin</option>\n";
		echo "</select>$NWB#email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account protocollo: </td><td align=left><select size=1 name=protocol><option>IMAP</option><option>POP3</option><option SELECTED>$protocol</option></select>$NWB#email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Indirizzo di risposta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Server: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Utente: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Password account: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "</select>$NWB#email_accounts-email_frequency_check_mins$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=3811&group_id=$group_id\">ID In-Group</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
...
###############
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Messaggi di posta elettronica non-trattati: </td><td align=left><B>$unhandled_emails</B></td></tr>\n";
################
		
		echo "<tr bgcolor=#B6D3FC><td align=center colspan=2><input type=submit name=INVIA VALUE=UPDATE><input type=hidden name='eact' value='UPDATE'></td></tr>\n";
		echo "</TABLE><BR><BR>";
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Clicca qui per vedere i cambiamenti di amministratore per questo record</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>INBOUND EMAIL Annunci account:\n";
	...
	echo "<TD><font size=1 color=white>MODIFICA</TD></tr>\n";
	...
	while ($accounts_to_print > $o) 
		...
		echo "<td><font size=1><a href=\"$PHP_SELF?eact=UPDATE&email_account_id=$row[email_account_id]\">MODIFICA</a></font></td></tr>\n";
		$o++;
		}
