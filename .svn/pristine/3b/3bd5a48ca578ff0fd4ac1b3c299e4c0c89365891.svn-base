...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<title>χειριστής astGUIclient: Login</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<TABLE><TR><TD></TD>\n";
	echo "<!-- ILPA -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_el/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Ελληνικά <img src=\"../agc/images/el.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- ΕΚΔΟΣΗ: $version     ΔΗΜΙΟΥΡΓΙΑ: $build      ADD: $ADD-->\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_el/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Ελληνικά <img src=\"../agc/images/el.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Σύνδεση Τηλεφώνου </TD>";
	echo "</TR>\n";
	echo "<TR><TD ALIGN=CENTER COLSPAN=2><font size=1> &nbsp; <BR><FONT SIZE=3>Συγγνώμη, αλλά η τηλεφωνική σύνδεση και ο κωδικός πρόσβασής σας δεν είναι ενεργά σε αυτό το σύστημα, παρακαλώ προσπαθήστε πάλι: <BR> &nbsp; </TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Σύνδεση Τηλεφώνου: </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=phone_login SIZE=10 maxlength=20 VALUE=\"$phone_login\"></TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Κωδικός πρόσβασης Τηλεφώνου:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 maxlength=20 VALUE=\"$phone_pass\"></TD></TR>\n";
...
?>
	...
	image_voicemail_ON.src="../agc/images/agc_check_voicemail_ON_el.gif";
	var image_voicemail_BLINK = new Image();
	image_voicemail_BLINK.src="../agc/images/agc_check_voicemail_BLINK_el.gif";
...
// ################################################################################
// ACTIVE EXTENSIONS LIST ΑΝΑΝΕΩΣΗ FUNCTIONS
...
// ################################################################################
// BUSY TRUNK LIST ΑΝΑΝΕΩΣΗ FUNCTIONS
...
// ################################################################################
// BUSY EXTENSIONS LIST ΑΝΑΝΕΩΣΗ FUNCTIONS
	...
	function busyhangup_send_redirect(taskvar,tasktrunkaction) 
		...
		if (tasktrunkaction == 'LISTEN')
			...
			if (listenvalue.match(regLa))
				...
				{
				bsr_continue=0;
				alert("Μπορείτε μόνο να ελέγξετε τα κανάλια Zap:\n" + listenvalue);
				}
			}
		if (bsr_continue == '1')
			{
	...
	function liverecording_send_recording(taskvar,taskchan,taskspan,taskfile) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'Monitor')
				...
				var rec_start_html = "<a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + monitorchannelvalue + "','" + taskspan + "','" + filename + "');return false;\">Τερματισμός Ηχογράφησης";
	...
	function conf_send_recording(taskconfrectype,taskconfspan,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			if (taskconfrectype == 'MonitorConf')
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfspan + "','" + taskconfrec + "','" + filename + "');return false;\">Τερματισμός Ηχογράφησης</a>";
...
// ################################################################################
// Check to see if user's extension is στο a call right now, if it is, format and print it
					...
					if (live_calls > 0)
						...
						while (loop_ct < live_calls)
							...
							if (recLIST.match(regx)) 
								{live_calls_HTML = live_calls_HTML + "<span id=\"recordlive" + loop_ct + "\"><a href=\"#\" onclick=\"liverecording_send_recording('StopMonitor','" + channelfieldBtrunk_array[1] + "','recordlive" + loop_ct + "');return false;\">Τερματισμός Ηχογράφησης</span>";}
							else 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (parked_count > 0)
						{
						var park_HTML = "<table width=600><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\">CHANNEL<BR>&nbsp; ΚΛΗΣΗ ID</td><td><font class=\"log_title\">ΣΤΑΘΜΕΥΜΕΝΕΣ ΑΠΟ<BR>&nbsp; ΧΡΟΝΟΣ ΣΤΑΘΜΕΥΣΗΣ</td><td><font class=\"log_title\">ΚΛΕΙΣΙΜΟ</td><td><font class=\"log_title\">XFER</td><td><font class=\"log_title\">ΕΠΑΝΑΛΕΙΨΗ</td></tr>"
...
// ################################################################################
// Send Originate command to manager to direct user to Κουτί φωνητικού ταχυδρομείου
...
//('BuilD','ADD','REMOVE','UP','DOWN')
	...
	//		alert(favorites_listEDIT);
		...
		var favedit_HTML = "<center><table cellpadding=5 cellspacing=5 width=750><tr><td colspan=2 align=center bgcolor=\"#DDDD99\"><B>ΔΙΑΘΕΣΙΜΕΣ ΕΠΕΚΤΑΣΕΙΣ</B></td><td align=center bgcolor=\"#CCCC99\"><B> ΑΓΑΠΗΜΕΝΑ</B></td></tr><tr><td bgcolor=\"#DDDD99\" height=380 width=200 valign=top><font class=\"ss_text\"><span id=FavSelectA>";
		loop_ct = 0;
		while (loop_ct < favlistCT)
			...
			if (favorites_listEDIT.match(regFAVlist))
				{
				// do not print extension, it's στο the favorites list
				}
			else
				{
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT()
		...
		document.getElementById("LogouTBoxLink").innerHTML = "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&user=" + user + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "\">ΕΠΙΛΕΞΤΕ ΕΔΩ ΓΙΑ ΝΑ ΣΥΝΔΕΘΕΙΤΕ ΠΑΛΙ</a>\n";
...
<span style="position:absolute;left:0px;top:12px;z-index:29;" id="TrunkHangupBox">
    <table border=1 bgcolor="#CDE0C2" width=600 height=500><TR><TD> TRUNK ανωμαλίας <BR><BR>
...
<span style="position:absolute;left:0px;top:12px;z-index:28;" id="LocalHangupBox">
    <table border=1 bgcolor="#CDE0C2" width=600 height=500><TR><TD> ΤΟΠΙΚΟ ΚΛΕΙΣΙΜΟ ΤΗΛΕΦΩΝΟΥ <BR><BR>
	<span id="LocalHangupContent"> Ενεργές Τοπικές Επιλογές </span><BR>
	<span id="LocalHangup_HUlink"><a href="#" onclick="busyhangup_send_hangup('Local');return false;">Κλείσιμο τοπικού</a> &nbsp; | &nbsp; </span>
...
<span style="position:absolute;left:80px;top:12px;z-index:42;" id="MainXfeRBox">
	...
	<td>
	<BR>
	<a href="#" onclick="mainxfer_send_redirect('XfeR');return false;">Στείλε στην επιλεγμένη τηλ.σύνδεση</a> <BR><BR>
...
<tr><td align=center><font face="Arial,Helvetica"><B>ΕΞΕΡΧΟΜΕΝΕΣ ΚΛΗΣΕΙΣ:</B></font></td></tr>
...
<span style="position:absolute;left:5px;top:5px;z-index:34;" id="FavoriteSEdiT">
    <table border=0 bgcolor="#DDDDFF" width=800 height=450 cellpadding=2 ALIGN=TOP><TR HEIGHT=95%><TD align=center HEIGHT=95%><span id="FavoriteSEditContent"> ΑΓΑΠΗΜΕΝΑ</span></TD></TR>
	<TR><TD ALIGN=CENTER><BR> &nbsp; </TD></TR>
	<TR VALIGN=BOTTOM><TD VALIGN=BOTTOM ALIGN=CENTER BGCOLOR="#FFFFCC"><a href="#" onclick="SubmiT_FavoritE_ChangEs();return false;">ΥΠΟΒΑΛΤΕ τις ΑΛΛΑΓΕΣ ΣΥΜΠΑΘΕΙΩΝ - απαιτεί την αποσύνδεση</a></TD></TR>
	<TR VALIGN=BOTTOM><TD VALIGN=BOTTOM ALIGN=CENTER BGCOLOR="#FFFFCC"><a href="#" onclick="hideDiv('FavoriteSEdiT');return false;">ΠΙΣΩ στο ΚΥΡΙΟ ΠΑΡΑΘΥΡΟ - αγνοήστε τις αλλαγές που γίνονται</a></TD></TR>
	</TABLE>
</span>
...
<a href="#" onclick="activeext_force_refresh();return false;">ΑΝΑΝΕΩΣΗ</a> | 
<span id="activeext_order"><a href="#" onclick="activeext_order_desc();return false;">ΤΑΞΙΝ</a> | </span>
</td>

<td>Εξωτερικές γραμμές <BR>
<a href="#" onclick="busytrunk_force_refresh();return false;">ΑΝΑΝΕΩΣΗ</a> | 
<span id="busytrunk_order"><a href="#" onclick="busytrunk_order_desc();return false;">ΤΑΞΙΝ</a> | </span>
</td>

<td>Τοπικές τηλ. συνδέσεις <BR>
<a href="#" onclick="busyext_force_refresh();return false;">ΑΝΑΝΕΩΣΗ</a> | 
<span id="busyext_order"><a href="#" onclick="busyext_order_desc();return false;">ΤΑΞΙΝ</a> | </span>
</td></tr>
...
<span id="ConfereNcesListContent">Κατάλογος διασκέψεων </span>
</span>
...
<span style="width:540;height:400;" id="ConfereNceDetailContent">Επιλέξτε έναν αριθμό δωματίου συνδιάσκεψης στα αριστερά για τις πληροφορίες σε εκείνη την συνδιάσκεψη </span>
</span>
