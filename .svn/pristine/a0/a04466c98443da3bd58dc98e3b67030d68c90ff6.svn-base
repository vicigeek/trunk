...
if ($function == 'version')
	{
	$data = "ΕΚΔΟΣΗ: $version|ΔΗΜΙΟΥΡΓΙΑ: $build|DATE: $NOW_TIME|EPOCH: $StarTtime";
	$result = 'SUCCESS';
	echo "$data\n";
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if(strlen($source)<2)
		...
		if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0) or ($auth_api==0))
			{
			$result = 'ERROR';
			$result_reason = "Ακυρο Ονομα Χρήστη/Κωδικός Πρόσβασης";
...
if ($format=='debug')
	...
	echo "<html>\n";
	echo "<head>\n";
	echo "<!-- ΕΚΔΟΣΗ: $version     ΔΗΜΙΟΥΡΓΙΑ: $build    USER: $user\n";
