...
if (strlen($group[0]) < 1)
	...
	if ($file_download < 1)
		{
		$ASCII_text.="Agent Jours Etat Report: $user                     $NOW_TIME\n";
		$ASCII_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		$ASCII_text.="---------- AGENT Détails -------------\n\n";
		$GRAPH_text.="Agent Jours Etat Report: $user                     $NOW_TIME\n";
		$GRAPH_text.="Time range: $query_date_BEGIN to $query_date_END\n\n";
		$GRAPH_text.="---------- AGENT Détails -------------\n";
		}
	else
		{
		$file_output .= "Agent Jours Etat Report: $user                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>LEAD STATS BREAKDOWN</caption><tr><th class='thgraph' scope='col'>ETAT</th>";
...
echo "</TD><TD VALIGN=TOP>Utilisateur:<BR>";
...
if (strlen($user) > 1)
	...
	{echo " <a href=\"./admin.php?ADD=0A\">UTILISATEURS</a> | \n";}
