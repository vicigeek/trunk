...
<title>ADMINISTRACIÓN: Campaña Multi-Alt-Leads
<?php 
...
if ($camp_multi < 1)
	{
	echo "Esta campaña no está establecido en automarcacion de Alternos Auto-Alt-Dial MULTI_LEAD\n";
	exit;
	}
...
if ($action == "BLANK")
	...
	echo "<br>Form de Config AutoMarcacion Alterna Multi-Prospecto <form action=$PHP_SELF method=POST>\n";
	echo "<input type=hidden name=DB value=\"$DB\">\n";
	echo "<input type=hidden name=campaign_id value=\"$campaign_id\">\n";
	echo "<input type=hidden name=action value=ALT_MULTI_REMITIR>\n";
	echo "<center><TABLE width=$section_width cellspacing=3>\n";
	echo "<tr><td align=center colspan=2>\n";


	echo "<br><b>PHONE TYPES WITHIN THE LISTS FOR THIS CAMPAÑA: <a href=\"./admin.php?ADD=31&campaign_id=$campaign_id\">$campaign_id</a></b>\n";
	echo "<TABLE width=700 cellspacing=3>\n";
	echo "<tr><td>PHONE NUMBER TYPE</td><td>LLAMADO</td><td>NO LLAMADO</td><td>SELECTED</td><td>OLD RANK</td><td>NEW RANK</td></tr>\n";
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>Orden de lista Aleatoria: </td><td align=left><select size=1 name=lead_order_randomize><option>Y</option><option>N</option><option SELECTED>$lead_order_randomize</option></select></td></tr>\n";
	echo "<tr bgcolor=#B6D3FC><td align=right>Lista Orden Secundario: </td><td align=left><select size=1 name=lead_order_secondary><option>LEAD_ASCEND</option><option>LEAD_DESCEND</option><option>CALLTIME_ASCEND</option><option>CALLTIME_DESCEND</option><option SELECTED>$lead_order_secondary</option></select></td></tr>\n";
