...
if($reports_only_user > 0)
	...
	if ( (!preg_match("/$report_name/",$LOGallowed_reports)) and (!preg_match("/ALL INFORMES/",$LOGallowed_reports)) )
		...
		Header("WWW-Authenticate: Basic realm=\"CONTACT-CENTER-ADMIN\"");
		Header("HTTP/1.0 401 Unauthorized");
		echo "No tienes permiso para ver este informe: |$USER|$report_name|\n";
		exit;
		}
	else
		{
...
<title>ADMINISTRATION: Administración de CallCard
<?php
...
if ($action == "CALLCARD_DETAIL")
	...
	if ($details_to_print > 0) 
		...
		echo "<br>CallCard Detalle<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Estado: </td><td align=left><B>$status</B></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>In-Grupo: </td><td align=left><B>$inbound_group_id</B></td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Comentarios: </td><td align=left><B>$note_comments</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Create Usuario/Date: </td><td align=left><B>$create_user / $create_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Activate Usuario/Date: </td><td align=left><B>$activate_user / $activate_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Used Usuario/Date: </td><td align=left><B>$used_user / $used_time</B></td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Void Usuario/Date: </td><td align=left><B>$void_user / $void_time</B></td></tr>\n";
		echo "</TABLE>\n";
		echo "<BR><BR><BR>\n";
		...
		echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=2 color=white>AGENTE FECHA</B></TD>";
		echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=2 color=white>AGENTE DISPO</B></TD>";
		echo "</TR>\n";
...
if ($action == "CALLCARD_SUMMARY")
	...
	echo "<TR BGCOLOR=BLACK>";
	echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=1 color=white>ESTADO</B></TD>";
	echo "<TD><B><FONT FACE=\"Arial,Helvetica\" size=1 color=white>CUENTA</B></TD>";
	echo "</TR>\n";
	...
	if ($db_ct > 0)
		...
		echo "<b>Hay  $DBcount batches in the system\n";
		}
	echo "<BR><BR>\n";
...
if ($action == "GENERATE")
	...
	echo "<tr bgcolor=#B6D3FC><td align=right>In-Grupo: </td><td align=left><input type=text name=inbound_group_id size=20 maxlength=20></td></tr>\n";
...
<BR><font size=1>CallCard &nbsp; &nbsp; VERSIÓN: <?php echo $version ?> &nbsp; &nbsp; CONSTRUCCION: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</TD></TR></TABLE>
