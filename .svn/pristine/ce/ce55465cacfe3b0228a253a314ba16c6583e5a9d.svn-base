...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		$VDdisplayMESSAGE = "Užívateľa a heslo, ktoré ste zadali, nie sú aktívne v systéme<BR>Prosím, skúste to znova:";
		...
		if ($vts_count > 0)
			...
			print "<!-- NOVÁ vicidial_timeclock_status record inserted for $user:   |$affected_rows| -->\n";
			}
		if ( ($last_action_sec < 30) and ($status != 'START') )
			{
			### You cannot log in or out within 30 sekúnd of your last login/logout
			$VDdisplayMESSAGE = "Nemôžete sa prihlásiť alebo sa do 30 sekúnd od vášho posledného prihlásenia alebo odhlásenie";
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='login') )
				{
				$VDdisplayMESSAGE = "Máte teraz prihlásených";
				$LOGtimeMESSAGE = "Ste prihlásení na $NOW_TIME";
				...
				print "<!-- NOVÁ vicidial_timeclock_log record inserted for $user:   |$affected_rows|$timeclock_id| -->\n";
				...
				$rslt=mysql_to_mysqli($stmt, $link);
				$affected_rows = mysqli_affected_rows($link);
				print "<!-- NOVÁ vicidial_timeclock_audit_log record inserted for $user:   |$affected_rows| -->\n";
				}

			if ( ($status=='LOGIN') and ($stage=='logout') )
				{
				$VDdisplayMESSAGE = "Teraz ste prihlásený-out";
				$LOGtimeMESSAGE = "Ste odhlásený na$NOW_TIME<BR>Množstvo času, ste prihlásený-in:$totTIME_HMS";
			...
			if ( ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') ) and ($stage=='logout') ) or ( ($status=='LOGIN') and ($stage=='login') ) )
				{echo "ERROR: timeclock log vstupe už bolo dosiahnuté:$status|$stage";  exit;}

			if ($referrer=='agent') 
				{$BACKlink = "<A HREF=\"./vicidial.php?pl=$phone_login&pp=$phone_pass&VD_login=$user\"><font color=\"#003333\">SPÄŤ na Agent Prihlásiť obrazovky</font></A>";}
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"/$admin_web_directory/admin.php\"><font color=\"#003333\">SPÄŤ na administratívne</font></A>";}
			if ($referrer=='welcome') 
				{$BACKlink = "<A HREF=\"$welcomeURL\"><font color=\"#003333\">SPÄŤ na úvodnú obrazovku</font></A>";}
		...
		if ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') or ($status=='TIMEOUTLOGOUT') )
			{
			$VDdisplayMESSAGE = "Čas od Vášho posledného prihlásenia-in:$totTIME_HMS";
			$log_action = 'login';
			$button_name = 'LOGIN';
			$LOGtimeMESSAGE = "Ty posledný prihlásený-out na:$last_action_date<BR><BR>Kliknite nižšie k LOGIN LOG-IN";
			}
		if ($status=='LOGIN')
			{
			$VDdisplayMESSAGE = "Množstvo času, ktoré ste vykáceli-in:$totTIME_HMS";
			$log_action = 'logout';
			$button_name = 'LOGOUT';
			$LOGtimeMESSAGE = "Ste prihlásený-in na adrese: $last_action_date<BR>Množstvo času, ktoré ste vykáceli-in:$totTIME_HMS<BR><BR>Kliknite na Logout nižšie log-out";
			}
