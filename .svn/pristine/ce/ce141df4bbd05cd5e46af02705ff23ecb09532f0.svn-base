...
$report_name = 'Agent Status Dettagliata';
$db_source = 'M';
...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		{
		$ASCII_text.="<BR><BR>Agent Status Dettagliata Report                     $NOW_TIME\n";
		$ASCII_text.="Intervallo Di Tempo: $query_date_BEGIN to $query_date_END\n\n";
		$ASCII_text.="---------- AGENT Dettagliatas -------------\n\n";
		$GRAPH.="<BR><BR>Agent Status Dettagliata Report                     $NOW_TIME\n";
		$GRAPH.="Intervallo Di Tempo: $query_date_BEGIN to $query_date_END\n\n";
		$GRAPH.="---------- AGENT Dettagliatas -------------";
		}
	else
		{
		$file_output .= "Agent Status Dettagliata Report                     $NOW_TIME\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>CALL STATS BREAKDOWN</caption><tr><th class='thgraph' scope='col'>ESITO</th>";
	...
	if ($file_download < 1)
		{
		$ASCII_text.="+-----------------+----------+--------+--------+--------+$statusesHEAD\n";
		$ASCII_text.="|  TOTALI        AGENTS:$TOT_AGENTS | $TOTcalls| $CIScountTOT| $DNCcountPCT%|$SUMstatusesHTML\n";
		$ASCII_text.="+----------------------------+--------+--------+--------+$statusesHEAD\n";
		...
		{
		$file_output .= "TOTALI,$TOT_AGENTS,$TOTcalls,$CIScountTOT,$DNCcountPCT%,$SUMstatusesFILE\n";
		}
	}
...
echo "<INPUT TYPE=Submit NAME=INVIA VALUE=INVIA>$NWB#agent_status_detail$NWE\n";
