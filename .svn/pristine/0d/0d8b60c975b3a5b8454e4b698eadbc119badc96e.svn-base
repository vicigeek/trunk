...
$report_name = 'Reporte Detalle Usuario Reloj Asistencia';
$db_source = 'M';
...
if (strlen($user_group[0]) < 1)
	...
	if ($file_download < 1)
		{
		echo "Usuario Time-Clock Detalle                     $NOW_TIME\n";

		echo "Time range: $query_date_BEGIN to $query_date_END\n\n";
		}
	else
		{
		$file_output .= "Usuario Time-Clock Detalle                     $NOW_TIME\n";
	...
	if ($file_download < 1)
		{
		$ASCII_text.="AGENTE TIME-CLOCK DETAIL:\n";
		$ASCII_text.="+-----------------+----------+----------------------+------------+--------------------\n";
		$ASCII_text.="| <a href=\"$LINKbase&stage=NAME\">USER NAME</a>       | <a href=\"$LINKbase&stage=ID\">ID</a>       | <a href=\"$LINKbase&stage=GROUP\">GRUPO DE USUARIOS</a>           | <a href=\"$LINKbase&stage=TCLOCK\">TIME CLOCK</a> | TIME CLOCK PUNCHES\n";
		$ASCII_text.="+-----------------+----------+----------------------+------------+--------------------\n";

		$GRAPH.="<table cellspacing=\"0\" cellpadding=\"0\" summary=\"AGENTE TIME-CLOCK DETAIL\" class=\"horizontalgraph\">\n";
		$GRAPH.="  <caption align=\"top\">AGENTE TIME-CLOCK DETAIL</caption>\n";
		$GRAPH.="  <tr>\n";
		$GRAPH.="	<th class=\"thgraph\" scope=\"col\">USER/GRUPO DE USUARIOS</th>\n";
...
#		echo "$Suser[$m]|$Sname[$m]|$Swait[$m]|$Stalk[$m]|$Sdispo[$m]|$Spause[$m]|$Scalls[$m]\n";
	...
	##### END loop through each user formatting data for output
		for ($i=0; $i<count($graph_stats); $i++) {
			...
			$GRAPH.="	<th class=\"thgraph value$class\" scope=\"col\" nowrap><img src=\"../vicidial/images/bar.png\" alt=\"\" width=\"".round(MathZDC(200*$graph_stats[$i][1], $max_time))."\" height=\"16\" />".sec_convert($graph_stats[$i][1], 'H').$graph_stats[$i][2]."</th>\n";
