...
<title>ADMINISTRATION: Alteração de Ponto
<?php
...
<CENTER>
<TABLE WIDTH=720 BGCOLOR=#D9E6FE cellpadding=2 cellspacing=0><TR BGCOLOR=#015B91><TD ALIGN=LEFT><FONT FACE="ARIAL,HELVETICA" COLOR=WHITE SIZE=2><B>Alteração de Ponto for <?php echo $user ?></TD><TD ALIGN=RIGHT> &nbsp; </TD></TR>
...
if ( ($invalid_record < 1) or (strlen($timeclock_id)<1) )
	...
	if ($stage == "edit_TC_log")
		...
		if ( ($LOGINepoch <= $PREVevent_epoch) || ($LOGOUTepoch >= $NEXTevent_epoch) )
			{
			echo "ERRO - Existe um problema com os dados digitados, por favor retorne<BR>\n";
