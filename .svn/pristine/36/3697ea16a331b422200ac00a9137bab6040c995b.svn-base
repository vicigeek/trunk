...
$report_name = 'Listen Pass Report';
$db_source = 'M';
...
if ($use_lists > 0)
	{
	$MAIN.="</TD><TD VALIGN=TOP> Listen:<BR>";
	...
	$MAIN.="</SELECT>\n<BR>\n";
	$MAIN.="<a href=\"$PHP_SELF?use_lists=0&DB=$DB\">SWITCH TO KAMPAGNEN</a>";
	}
else
	{
	$MAIN.="</TD><TD VALIGN=TOP> Kampagnen:<BR>";
	$MAIN.="<SELECT SIZE=5 NAME=group[] multiple>\n";
	if  (preg_match('/\-\-ALL\-\-/',$group_string))
		{$MAIN.="<option value=\"--ALL--\" selected>-- ALL KAMPAGNEN --</option>\n";}
	else
		{$MAIN.="<option value=\"--ALL--\">-- ALL KAMPAGNEN --</option>\n";}
	$o=0;
	while ($campaigns_to_print > $o)
		{
		if (preg_match("/$groups[$o]\|/i",$group_string)) {$MAIN.="<option selected value=\"$groups[$o]\">$groups[$o] - $group_names[$o]</option>\n";}
...
if (strlen($group[0]) > 1)
	{
	$MAIN.=" <a href=\"./admin.php?ADD=34&campaign_id=$group[0]\">ÄNDERN Sie</a> | \n";
...
if (strlen($group[0]) < 1)
	{
	$MAIN.="\n\n";
	$MAIN.="WÄHLEN Sie BITTE Eine KAMPAGNE Vor UND DATUM OBEN UND KLICKEN REICHTEin\n";
	}

else
	{
	$OUToutput = '';
	$OUToutput .= "Listen Pass Report                             $NOW_TIME\n";

	$OUToutput .= "\n";

	##############################
	#########  LISTE IDENTIFIKATION BREAKDOWN STATS

	$TOTALleads = 0;

	$OUToutput .= "\n";
	$OUToutput .= "---------- LISTE IDENTIFIKATION SUMMARY     <a href=\"$PHP_SELF?DB=$DB$groupQS&SUBMIT=$SUBMIT&file_download=1\">DOWNLOAD</a>\n";
	...
	$OUToutput .= "|  LOAD DATE | LISTE IDENTIFIKATION and NAME                         | CAMPAIGN | COUNT      | ACTIVE   |";
	...
	$CSV_text1.="\"LISTE IDENTIFIKATION SUMMARY\"\n";
	...
	$GRAPH="</PRE><table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"white\" summary=\"LISTE IDENTIFIKATION Summary\" class=\"horizontalgraph\">\n";
	$GRAPH.="<caption align='top'>LISTE IDENTIFIKATION SUMMARY</caption>";
	$GRAPH.="<tr>\n";
	...
	$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>LISTE IDENTIFIKATION SUMMARY</caption><tr><th class='thgraph' scope='col'>LISTS</th>";
